...
def test_home_world_is_one_bounded_physical_declaration() -> None:
    ...
    assert len(snapshot.objects) == 64   # 63 declared items + her room's nightlight <= 64 ceiling
    ...
    assert any(item.object_id == "mailbox" for item in snapshot.objects)
    assert any(item.object_id == "tree-apple" for item in snapshot.objects)
    assert any(item.object_id == "garden-apple" for item in snapshot.objects)
    assert any(item.object_id == "garden-ladder" for item in snapshot.objects)
    assert any(item.object_id == "tv-remote" for item in snapshot.objects)
    assert any(item.object_id == "bottle-milk" for item in snapshot.objects)
    assert any(item.object_id == "bread-slice" for item in snapshot.objects)
    assert any(item.object_id == "stroller-carriage" for item in snapshot.objects)
