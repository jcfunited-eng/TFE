...
def test_exact_w1_mount_commits_one_bounded_two_ear_l5_experience():
    ...
    assert "source_sample_commitment_sha256" not in authority
    assert all(
        len(value.source_sample_commitment_sha256) == 64
        for value in experience.compact_full_field.components
    )
...
def test_l5_prepare_and_commit_failures_leave_no_partial_transaction(
    ...
    def fail_prepare(_settlement, **_verified_context):
