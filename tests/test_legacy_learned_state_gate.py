...
def test_gate_translates_exact_owner_and_accounts_every_source_byte(
    tmp_path,
):
    runtime = Guala()
    try:
        identity, tick = _source(tmp_path, runtime)
        ...
        verified = verify_legacy_learned_state_plan(
            plan.record(),
            authority_key=AUTHORITY,
        )
        assert plan.cutover_allowed is True
        assert verified["unresolved_member_count"] == 0
        assert sum(verified["category_bytes"].values()) == (
            verified["learned_source_bytes"]
        )
        assert verified["category_bytes"][DIRECT] > 0
        assert verified["category_bytes"][ESCROW] > 0
        assert verified["category_bytes"][UNRESOLVED] == 0
        assert verified["direct_owner_authentication"] == (
            "current_runtime_restore_and_canonical_target_resnapshot"
        )
        assert set(plan.direct_owner_bodies) == {
            "owner_state/causal_action_cycle.json"
        }
        direct_members = {
            value["member"]
            for value in verified["members"]
            if value["category"] == DIRECT
        }
        assert direct_members == {
            "causal_action_cycle",
            "causal_action_cycle_pending_review",
        }
    finally:
...
def test_gate_translates_authenticated_empty_v1_anonymous_av_owner(
    tmp_path,
):
    runtime = Guala()
    try:
        ...
        identity = runtime._guala_identity
        tick = 17
        source_value = _legacy_anonymous_av_genesis(runtime)
        (tmp_path / "guala_teaching.json").write_bytes(
            _legacy_envelope(
                identity,
                tick,
                {
                    "anonymous_audiovisual_continuity": source_value,
                },
            )
        )
        ...
        assert plan.cutover_allowed is True
        assert set(plan.direct_owner_bodies) == {target_path}
        target = json.loads(plan.direct_owner_bodies[target_path])
        assert target["payload"] == {
            "generation": 0,
            "latest": None,
            "schema": (
                "guala.w1."
                "anonymous_audiovisual_continuity.snapshot.v2"
            ),
            "settled": 0,
            "transition_capacity": 64,
            "transitions": [],
        ...
        assert member["category"] == DIRECT
        assert member["target_path"] == target_path
        assert member["target_body_sha256"] == hashlib.sha256(
            plan.direct_owner_bodies[target_path]
        ).hexdigest()
        assert "zero learned observations" in member["reason"]
    finally:
        runtime.strict_shutdown(timeout=30.0)
...
def test_gate_refuses_unresolved_physical_or_learned_custody(tmp_path):
    runtime = Guala()
    try:
        identity, tick = _source(tmp_path, runtime, unresolved=True)
        with pytest.raises(
            LegacyLearnedStateUnresolved,
            match="guala_teaching.json:unmapped_learned_custody",
        ) as captured:
            build_legacy_learned_state_plan(
                tmp_path,
                identity=identity,
        ...
        plan = captured.value.plan
        assert plan.cutover_allowed is False
        assert plan.body["unresolved_members"] == [
            "guala_teaching.json:unmapped_learned_custody"
        ]
        assert sum(plan.body["category_bytes"].values()) == (
            plan.body["learned_source_bytes"]
        )
        assert plan.body["category_bytes"][UNRESOLVED] > 0
    finally:
        runtime.strict_shutdown(timeout=30.0)


def test_gate_authentication_and_escrow_ceiling_fail_closed(tmp_path):
    runtime = Guala()
    try:
        identity, tick = _source(tmp_path, runtime)
        with pytest.raises(
            LegacyLearnedStateGateError,
            match="escrow exceeds",
        ):
            build_legacy_learned_state_plan(
                tmp_path,
                identity=identity,
        ...
        tampered["body"]["learned_source_bytes"] += 1
        with pytest.raises(
            LegacyLearnedStateGateError,
