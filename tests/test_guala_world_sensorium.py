...
def test_compact_retinal_capture_matches_spectral_values_without_signal_objects(monkeypatch) -> None:
    ...
    assert len(before) == len(after) == 19335
    ...
    repeated = ((Fraction(0), edge, collision, -edge, edge, Fraction(0)),) * 19335
...
def test_focal_optics_preserve_old_apertures_and_tile_the_sixty_degree_center() -> None:
    ...
    assert len(FOCAL_RETINAL_SITE_GEOMETRY) == 19200
    assert tuple(site[0] for site in FOCAL_RETINAL_SITE_GEOMETRY) == tuple(range(135, 19335))
    assert all(site[3:] == (Fraction(375, 2), Fraction(375, 2)) for site in FOCAL_RETINAL_SITE_GEOMETRY)
    first_row = FOCAL_RETINAL_SITE_GEOMETRY[:160]
    ...
    first_column = FOCAL_RETINAL_SITE_GEOMETRY[::160]
