...
def test_v3_groups_canonical_32_mount_as_16_typed_independent_pairs() -> None:
    ...
    experience.verify()
    assert AUDITORY_L5_SCHEMA == "guala.auditory_l5.full_field.v3"
...
def test_v3_receipts_bind_both_complete_l4_sequences_and_each_pair() -> None:
    ...
    assert AUDITORY_L5_COMPONENT_SCHEMA.endswith(".v1")
    assert AUDITORY_L5_PAIR_SCHEMA.endswith(".v1")
    assert tuple(
        ...
        (
            lambda values: (
                ...
                replace(
                    ...
                    normalized_signal=(
                        0.125,
                        *values[1].normalized_signal[1:],
                    ),
                ),
                *values[2:],
            ),
            "normalization differs from provider binary64",
        ),
        (
            lambda values: (
                values[0],
