...
def test_live_transport_is_decoupled_from_one_physical_vocal_action() -> None:
    assert PCM_CHUNK_SAMPLES == 32_000
    assert PCM_RING_SAMPLES == 128_000
    assert PCM_TRANSPORT_UNITS == 4
    assert PCM_RING_SAMPLES == PCM_TRANSPORT_UNITS * PCM_CHUNK_SAMPLES
    assert MAX_VOCAL_SAMPLE_COUNT == 80_000
    assert MAX_EMITTED_PCM_SAMPLES == MAX_VOCAL_SAMPLE_COUNT
    assert PCM_CHUNK_SAMPLES != MAX_VOCAL_SAMPLE_COUNT
