...
def test_equal_tick_pruning_retains_exact_immediate_causal_predecessor(
    tmp_path: Path,
) -> None:
    root = tmp_path / "same-tick-store"

    def revision(generation) -> int:
        return int(generation.payload("brain.json")["revision"])
    ...
    store = AuthoritativeColdGenerationStore(
        ...
        required_files=REQUIRED_FILES,
        max_encoded_generation_bytes=CAPACITY,
        pre_publish_validator=lambda _generation: True,
        generation_revision=revision,
    )

    def revision_files(revision_value: int) -> dict[str, object]:
        binary = tmp_path / f"same-tick-{revision_value}.bin"
        binary.write_bytes(
            f"same-tick-organism-{revision_value}".encode("ascii")
        )
        return {
            "brain.json": json.dumps({
                "tick": 9,
    ...
    oldest = store.commit(
        tick=9,
        state_revision=1,
        files=revision_files(1),
        generation_uuid=_uuid(900),
    ).current
    baseline = store.commit(
        tick=9,
        state_revision=2,
        files=revision_files(2),
    ...
    sealed = store.commit(
        tick=9,
        state_revision=3,
        files=revision_files(3),
        generation_uuid=_uuid(500),
        allow_equal_tick_schema_migration=True,
    )

    assert sealed.current.generation_uuid == _uuid(500)
    assert sealed.predecessor is not None
    assert sealed.predecessor.generation_uuid == baseline.generation_uuid
    assert {
        path.name
        for path in (root / GENERATIONS_DIRECTORY).iterdir()
    } == {
        baseline.generation_uuid,
        sealed.current.generation_uuid,
    }
...
def test_exclusive_audit_lock_is_write_open_for_nfs_semantics(
    tmp_path: Path,
    ...
    def nfs_semantic_flock(descriptor: int, operation: int) -> None:
        if operation & fcntl.LOCK_EX:
            ...
            if access_mode != os.O_RDWR:
                raise OSError(
                    errno.EBADF,
                    "NFS exclusive locks require write-open descriptors",
                )
        native_flock(descriptor, operation)

    monkeypatch.setattr(fcntl, "flock", nfs_semantic_flock)
    before = {
    ...
    with store.exclusive_read_only_transaction() as state:
...
def test_sealed_boot_retires_interrupted_unpublished_candidate(
        tmp_path: Path) -> None:
    root = tmp_path / "sealed-boot-interrupted"
    store = _store(root)
    for tick in (1, 2):
        store.commit(
            tick=tick,
    ...
    state = _store(root).inspect_sealed_boot()

    assert state.current.generation_uuid == _uuid(2)
    assert state.predecessor is not None
    assert state.predecessor.generation_uuid == _uuid(1)
    assert {
        path.name
        for path in (root / GENERATIONS_DIRECTORY).iterdir()
    } == {_uuid(1), _uuid(2)}
...
def test_legacy_transition_refuses_changed_current_after_restore(
    ...
    brain_record = next(
        ...
        if record["relative_path"] == "brain.json"
    )
    digest = brain_record["chunks"][0]["sha256"]
    os.chmod(
        root / CONTENT_CHUNKS_DIRECTORY / digest[:2] / digest,
        0o644,
    )
    with pytest.raises(
        AuthoritativeColdGenerationError,
