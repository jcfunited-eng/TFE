...
RETIRED_WINDOW_WAL_DIRNAME = "guala_windows_wal"
    ...
    for name in tool.REQUIRED_STATE_FILES:
        with open(os.path.join(state_dir, name), "w") as handle:
            json.dump({"guala_identity": f"identity-{marker}",
                       "data": marker}, handle)
    wal = os.path.join(state_dir, RETIRED_WINDOW_WAL_DIRNAME)
...
def _stage_real_backup(staging_dir):
    """Build a verifiable staged vintage with inert retired window files."""
    ...
    with open(os.path.join(staging_dir, "guala_windows.json"), "w") as handle:
        json.dump({"retired": "archive-only"}, handle)


def test_verify_runs_against_staging_and_leaves_live_dir_alone(tmp_path):
    state = str(tmp_path / "state")
    live_seg = _make_state(state)
    live_before = sorted(os.listdir(os.path.join(
        state, RETIRED_WINDOW_WAL_DIRNAME)))
    ...
    assert report["transient_causal_experience_state"] == "not_restorable"
    assert sorted(os.listdir(os.path.join(
        state, RETIRED_WINDOW_WAL_DIRNAME))) == live_before
...
def test_swap_preserves_displaced_state_and_installs_staged(tmp_path):
    ...
    assert os.path.exists(
        os.path.join(displaced_dir, RETIRED_WINDOW_WAL_DIRNAME,
    ...
    assert os.path.exists(os.path.join(state, "guala_windows.json"))
