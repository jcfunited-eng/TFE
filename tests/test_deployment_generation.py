...
def test_v2_causal_seal_remains_one_way_boot_compatible(tmp_path):
    result = stage_commit_upload(
        store_root=tmp_path / "store",
        identity=IDENTITY,
        tick=89,
    ...
    certificate = result.seal_certificate()
    certificate.update({
        "schema": deployment.LEGACY_DEPLOYMENT_SEAL_SCHEMA,
        "state_revision": 7,
        "causal_state_sha256": "1" * 64,
        "operational_metadata_sha256": "2" * 64,
        "attempt_operational_metadata_sha256": "3" * 64,
    })
    unsigned = {
        key: value
    ...
    certificate["seal_hmac_sha256"] = deployment.hmac.new(
        deployment._hmac_key(HMAC_KEY),
        deployment._canonical_json(unsigned),
        hashlib.sha256,
    ).hexdigest()

    assert verify_deployment_seal(
        certificate,
        hmac_key=HMAC_KEY,
        expected_nonce=NONCE,
    ) == certificate
...
def test_authoritative_stage_seals_dynamic_contracts_and_retains_exact_two(
    ...
    } == retained_local | {"content-chunks"}
    assert third.version_aware_remote_reconciliation is True
    assert set(third.remote_retained_generation_uuids) == retained_local
    assert third.remote_retired_generation_uuids == (
...
def test_authoritative_current_post_swap_failure_preserves_recoverable_state(
    ...
    } == {current.generation_uuid, "content-chunks"}
...
def test_authoritative_restart_retires_first_boot_prepublication_crash(
    ...
    assert {
        key.split("/")[2]
        for _, key in fake.objects
    } == {result.generation.generation_uuid, "content-chunks"}
