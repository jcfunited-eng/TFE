...
def test_multi_component_settlement_is_atomic_at_state_byte_boundary() -> None:
    tokens = AuditoryTokenSequenceAuthority(authority_secret=TOKEN_SECRET)
    probe = CausalLanguageConstructionAuthority(
        authority_key=CONSTRUCTION_SECRET
    )
    evidence = (
        (
            ("red", "appears"),
            _settlement("atomic-red", sight="red", touch="still"),
            "atomic-red",
        ),
        (
            ("blue", "appears"),
            _settlement("atomic-blue", sight="blue", touch="still"),
            "atomic-blue",
        ),
        (
            ("it", "is", "soft"),
            _settlement("atomic-soft", sight="still", touch="soft"),
            "atomic-soft",
        ),
        (
            ("it", "is", "hard"),
            _settlement("atomic-hard", sight="still", touch="hard"),
            "atomic-hard",
        ),
    )
    for forms, settlement, label in evidence:
        _admit(probe, tokens, forms, settlement, label)
    before = probe.snapshot()
    components = probe._contrast_components(
        probe._verified_consolidation_pool_locked()
    )
    assert tuple(len(value) for value in components) == (2, 2)
    probe._learn_locked(
        components[0],
        allow_structural_subsumption=True,
        reject_unresolved_overlap=True,
    )
    first_size = len(_canonical(probe.snapshot()))
    probe._learn_locked(
        components[1],
        allow_structural_subsumption=True,
        reject_unresolved_overlap=True,
    )
    complete_size = len(_canonical(probe.snapshot()))
    assert len(_canonical(before)) < first_size < complete_size

    authority = CausalLanguageConstructionAuthority(
        authority_key=CONSTRUCTION_SECRET,
        max_state_bytes=(first_size + complete_size) // 2,
    )
    authority.restore(before)

    with pytest.raises(RuntimeError, match="state capacity is full"):
        authority.settle_complete_constructions()

    assert authority.snapshot() == before
    assert authority.working_count == 4
    assert authority.construction_count == 0
