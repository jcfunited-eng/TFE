...
def test_live_validator_uses_one_bounded_child_process(
    ...
    assert "--allow-authenticated-legacy-pickle" not in command
    child_environment = values.pop("env")
    assert values == {
        "check": False,
        "capture_output": True,
        "text": True,
    ...
    expected_root = str(
        Path(app_module.__file__).resolve().parents[1]
    )
    pythonpath = child_environment["PYTHONPATH"].split(os.pathsep)
    assert pythonpath[0] == expected_root
