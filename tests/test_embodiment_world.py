...
def _legacy_snapshot(key: str, *, body=None, objects=None, revision=0) -> bytes:
    ...
    legacy_body_record = legacy_body.as_record()
    del legacy_body_record["active_contact"]
    del legacy_body_record["receptor_geometry"]
    payload_value = {
        "limits": {
...
def test_another_body_action_preserves_self_contact() -> None:
    authority = EmbodimentWorldAuthority(
        authority_key="embodiment-contact-continuity-test-key"
    )
    moved = _execute(
        authority,
        MoveCommand(
            PoseMM(PositionMM(1_150, 1_000, 0), 0),
            200_000,
        ),
        intent_number=1,
    )
    ...
    assert contact.disposition == "applied"
    self_contact = _body(contact.after).active_contact
    assert self_contact is not None

    pressure = b"\x01\x00" * 160
    vocal = authority.execute_port_command(
        ...
        expected_revision=contact.after.revision,
    )
...
def test_valid_move_pick_place_are_exact_authenticated_transitions() -> None:
    ...
    assert tuple(item.object_id for item in initial.objects[:6]) == tuple(
        f"W1-object-{number}" for number in range(1, 7)
    )
    assert len(initial.objects) == 42
    assert all(
        item.optical_surface is not None
        for item in initial.objects[6:]
    )
    ...
    moved = _execute(
        authority,
        MoveCommand(moved_pose, 200_000),
        intent_number=2,
    )
    assert moved.disposition == "applied"
    ...
    placed = _execute(
        authority,
        PlaceCommand("W1-object-1", placed_position, 200_000),
        intent_number=3,
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            "move_path_intersects_object",
        ),
        (PickCommand("missing-object", 200_000), "pick_unknown_object"),
        (
            PlaceCommand(
                "W1-object-1",
                PositionMM(1700, 1000, 0),
                200_000,
            ),
            "place_object_not_held",
        ),
    ),
)
...
def test_pick_rejects_a_reachable_object_behind_another_object() -> None:
    authority = EmbodimentWorldAuthority(
        authority_key="embodiment-pick-path-key",
        initial_objects=(
            EmbodiedObject(
                object_id="blocker",
                radius_mm=50,
                mass_grams=100,
                position=PositionMM(1000, 1600, 0),
            ),
            EmbodiedObject(
                object_id="target",
                radius_mm=50,
                mass_grams=100,
                position=PositionMM(1000, 1750, 0),
            ),
        ),
    )
    before = authority.encoded_snapshot()
    ...
    blocked = _execute(
        authority,
        PickCommand("target", 200_000),
        intent_number=23,
    )

    assert blocked.disposition == "rejected"
    assert blocked.reason == "pick_path_intersects_object"
    assert blocked.before == blocked.after
    assert authority.encoded_snapshot() == before


def test_pick_rejects_a_reachable_object_behind_another_body() -> None:
    authority = EmbodimentWorldAuthority(
        authority_key="embodiment-pick-body-path-key",
        bodies=(
            EmbodiedBody(
                body_id="guala-body-1",
                pose=PoseMM(PositionMM(1000, 1000, 0), 0),
            ...
            EmbodiedBody(
                body_id="w1-body-2",
                pose=PoseMM(PositionMM(1000, 1500, 0), 180_000),
                radius_mm=250,
                reach_mm=800,
            ),
        ),
        initial_objects=(
            EmbodiedObject(
                ...
                radius_mm=50,
                mass_grams=100,
                position=PositionMM(1000, 1800, 0),
            ),
        ),
    )
    before = authority.encoded_snapshot()
    ...
    assert blocked.disposition == "rejected"
    assert blocked.reason == "pick_path_intersects_body"
    assert blocked.before == blocked.after
    assert authority.encoded_snapshot() == before


def test_place_rejects_a_nonoverlapping_target_reached_through_an_object() -> None:
    authority = EmbodimentWorldAuthority(
        authority_key="embodiment-place-path-key",
        initial_objects=(
            EmbodiedObject(
                object_id="carried",
                radius_mm=50,
                mass_grams=100,
                position=PositionMM(1000, 1400, 0),
            ),
            EmbodiedObject(
                object_id="blocker",
                radius_mm=50,
    ...
    assert blocked.disposition == "rejected"
    assert blocked.reason == "place_path_intersects_object"
    assert blocked.before == blocked.after
    assert authority.encoded_snapshot() == before


def test_place_rejects_a_nonoverlapping_target_reached_through_a_body() -> None:
    authority = EmbodimentWorldAuthority(
        authority_key="embodiment-place-body-path-key",
        bodies=(
            EmbodiedBody(
                body_id="guala-body-1",
                pose=PoseMM(PositionMM(1000, 1000, 0), 0),
        ...
        initial_objects=(
            EmbodiedObject(
                ...
                radius_mm=50,
                mass_grams=100,
                position=PositionMM(1400, 1000, 0),
            ),
        ),
    )
    picked = _execute(
    ...
    assert blocked.disposition == "rejected"
    assert blocked.reason == "place_path_intersects_body"
    assert blocked.before == blocked.after
    assert authority.encoded_snapshot() == before
...
def test_command_language_is_typed_canonical_and_bounded() -> None:
    move = MoveCommand(
        PoseMM(PositionMM(1100, 1200, 0), 180_000),
        200_000,
    )
...
def test_revision_binding_makes_successful_replay_fail_closed() -> None:
    ...
    payload = encode_command(PickCommand("W1-object-1", 200_000))
    ...
    def run(number: int, y: int):
        ...
        return authority.execute_port_command(
            ...
            command_payload=encode_command(
                MoveCommand(
                    PoseMM(PositionMM(1000, y, 0), number * 1000),
                    200_000,
                )
            ),
...
def test_persistence_round_trip_rejects_tamper_wrong_key_and_noncanonical_envelope() -> None:
    ...
    _execute(
        authority,
        MoveCommand(
            PoseMM(PositionMM(1200, 1200, 0), 90_000),
            200_000,
        ),
        intent_number=61,
    )
...
def test_full_curriculum_world_retains_exact_receipt_window_without_surface_duplication() -> None:
    authority = EmbodimentWorldAuthority(authority_key="alphabet-window-key")
    pick = _execute(
        authority,
        PickCommand("W1-object-1", 200_000),
        intent_number=630,
    ...
    for number in range(1, 16):
        ...
        receipt = _execute(
            authority,
            MoveCommand(
                PoseMM(PositionMM(coordinate, 1000, 0), 0),
                200_000,
            ),
            intent_number=630 + number,
        )
        assert receipt.disposition == "applied"
    status = authority.status()
    assert status["retained_applied_receipts"] == 16
    encoded = authority.encoded_snapshot()
    assert len(encoded) <= DEFAULT_MAX_ENCODED_STATE_BYTES
    envelope = json.loads(encoded)
    payload = json.loads(
        base64.b64decode(envelope["payload_base64"], validate=True)
    )
    catalog = payload["optical_surface_catalog"]
    assert len(catalog) == 36
    references = [
        item.get("optical_surface")
        for item in payload["world"]["objects"]
        if item.get("optical_surface") is not None
    ]
    assert len(references) == 36
    assert all(set(item) == {"content_sha256"} for item in references)
    restored = EmbodimentWorldAuthority(authority_key="alphabet-window-key")
    restored.restore_encoded(encoded)
    assert restored.encoded_snapshot() == encoded
    assert tuple(item.optical_surface for item in restored.observation_snapshot().objects) == tuple(item.optical_surface for item in authority.observation_snapshot().objects)


def test_authenticated_unresolved_optical_surface_reference_is_rejected_atomically() -> None:
    key = "alphabet-reference-tamper-key"
    authority = EmbodimentWorldAuthority(authority_key=key)
    encoded = authority.encoded_snapshot()
    envelope = json.loads(encoded)
    payload = json.loads(base64.b64decode(envelope["payload_base64"], validate=True))
    referenced = [
        item
        for item in payload["world"]["objects"]
        if item.get("optical_surface") is not None
    ...
    assert referenced
    referenced[0]["optical_surface"]["content_sha256"] = "0" * 64
    canonical = lambda value: json.dumps(
        value,
        allow_nan=False,
        ensure_ascii=False,
...
def test_object_inventory_and_retained_receipts_are_strictly_bounded_long_run() -> None:
    ...
    long_run_object = EmbodiedObject(
        object_id="W1-object-1",
        radius_mm=100,
        mass_grams=500,
        position=PositionMM(1500, 1000, 0),
    ...
    authority = EmbodimentWorldAuthority(
        ...
        receipt_capacity=4,
        initial_objects=(long_run_object,),
    )
    _execute(
        authority,
        PickCommand("W1-object-1", 200_000),
    ...
    capacity_probe = EmbodimentWorldAuthority(
        authority_key="embodiment-byte-capacity-probe-key",
        receipt_capacity=1,
        initial_objects=(long_run_object,),
    )
    ...
    derived_capacity = probe_genesis_bytes + probe_retained_receipt_bytes
    assert derived_capacity > probe_retained_receipt_bytes
    ...
    for number in range(1, 20):
        ...
        assert len(byte_bounded.encoded_snapshot()) <= derived_capacity
...
def test_actor_specific_ports_move_only_their_owned_physical_body() -> None:
    ...
    result = authority.execute_port_command(
        ...
        command_payload=encode_command(
            MoveCommand(
                PoseMM(PositionMM(4000, 4000, 0), 90_000),
                200_000,
            )
        ),
    ...
    rejected = authority.execute_port_command(
        ...
        command_payload=encode_command(
            MoveCommand(
                PoseMM(PositionMM(1100, 1000, 0), 0),
                200_000,
            )
        ),
...
def test_multi_body_collision_and_holding_reciprocity_fail_closed() -> None:
    ...
    collision = _execute(
        authority,
        MoveCommand(
            PoseMM(PositionMM(3000, 1000, 0), 0),
            200_000,
        ),
        intent_number=910,
    )
...
def test_authenticated_v1_state_migrates_once_without_rewriting_prior_world() -> None:
    ...
    assert json.loads(migrated)["schema"] == ENVELOPE_SCHEMA
    restored = EmbodimentWorldAuthority(authority_key=key)
