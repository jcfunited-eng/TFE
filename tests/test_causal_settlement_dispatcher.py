...
SPEECH_KEY = "dispatcher-test-speech-authority"
...
def _dispatcher(cycle, speech, body):
    return CausalSettlementDispatcher(
        cycle=cycle,
        authority_key=DISPATCHER_KEY,
        speech_executor=speech,
        speech_executor_id="speech.primary",
        speech_executor_authority_key=SPEECH_KEY,
...
def test_capacity_one_speech_dispatch_is_idempotent_and_closes_only_bound_outcome():
    ...
    _teach(cycle, taught, ActionCommand.speech("hello"))
    speech = _Executor("speech.primary", SPEECH_KEY)
    body = _Executor("body.primary", BODY_KEY)
    dispatcher = _dispatcher(cycle, speech, body)
...
def test_unknown_ambiguous_and_authenticated_rejection_are_explicit():
    ...
    unknown_dispatcher = _dispatcher(
        unknown_cycle,
        _Executor("speech.primary", SPEECH_KEY),
    ...
    _teach(ambiguous_cycle, trigger, ActionCommand.speech("one"))
    ambiguous_cycle.teach(
        trigger_reference=trigger.event_id,
        action=ActionCommand.speech("two"),
        source="joe",
    ...
    _teach(rejected_cycle, trigger, ActionCommand.speech("no"))
    rejecting = _Executor("speech.primary", SPEECH_KEY, disposition="rejected")
    rejected_dispatcher = _dispatcher(
        rejected_cycle, rejecting, _Executor("body.primary", BODY_KEY)
    )
...
def test_executor_failure_stays_pending_without_implicit_retry_and_accepts_later_ack():
    ...
    _teach(cycle, trigger, ActionCommand.speech("later"))
    requests = []
    ...
    dispatcher = _dispatcher(
        cycle, unavailable, _Executor("body.primary", BODY_KEY)
    )
    ...
    acknowledgement = authenticate_executor_acknowledgement(
        request=requests[0],
        executor_id="speech.primary",
        authority_key=SPEECH_KEY,
...
def test_authenticated_pending_state_restores_and_tampering_fails_closed():
    ...
    _teach(cycle, trigger, ActionCommand.speech("persist"))
    speech = _Executor("speech.primary", SPEECH_KEY)
    body = _Executor("body.primary", BODY_KEY)
    dispatcher = _dispatcher(cycle, speech, body)
    pending = dispatcher.dispatch(repeated)
    ...
    restored = _dispatcher(restored_cycle, speech, body)
...
def test_authenticated_bound_outcome_restores_and_closes_exact_settlement():
    ...
    _teach(cycle, trigger, ActionCommand.speech("bound"))
    speech = _Executor("speech.primary", SPEECH_KEY)
    body = _Executor("body.primary", BODY_KEY)
    dispatcher = _dispatcher(cycle, speech, body)
    pending = dispatcher.dispatch(repeated)
...
def test_wrong_executor_acknowledgement_cannot_advance_live_intent():
    ...
    _teach(cycle, trigger, ActionCommand.speech("authenticate"))
    requests = []
