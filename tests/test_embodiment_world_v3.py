...
def _v2_snapshot() -> bytes:
    ...
    payload = {
        "actor_ports": [
            item.as_record() for item in source.actor_ports
        ],
        "limits": {
            "max_bodies": 4,
...
def test_v3_manifest_is_neutral_bounded_physics() -> None:
    ...
    assert [item.object_id for item in observation.objects] == [
...
def test_portals_govern_move_pick_place_without_remote_action() -> None:
    ...
    rejected = _execute(
        world,
        MoveCommand(PoseMM(PositionMM(16000, 2500, 0), 0)),
        1,
    )
    ...
    for number, point in enumerate(
        (
            ...
            PositionMM(6000, 2500, 0),
            PositionMM(7500, 1000, 0),
        ),
        start=2,
    ):
        assert _execute(
            world, MoveCommand(PoseMM(point, 0)), number
        ).disposition == "applied"
    assert _execute(world, PickCommand("W1-object-4"), 5).disposition == "applied"
    for number, point in enumerate(
        (
    ...
    placed = _execute(
        world,
        PlaceCommand("W1-object-4", PositionMM(16500, 2500, 0)),
        8,
    )
    assert placed.disposition == "applied"
    assert placed.after.room_id == "W1-region-C"
    object_4 = next(
        item for item in placed.after.objects
...
def test_sight_is_local_or_exact_direct_portal_line_of_sight() -> None:
    world = EmbodimentWorldAuthority(authority_key=KEY)
    assert _execute(
        world,
        MoveCommand(PoseMM(PositionMM(1000, 2500, 0), 0)),
        1,
    ).disposition == "applied"
    ...
    sight_ids = {
        item.substream_id for item in observed[PhysicalSense.SIGHT]
    }

    assert "W1-visible-object-W1-object-5" in sight_ids
    assert "W1-visible-object-W1-object-4" not in sight_ids
    assert "W1-visible-object-W1-object-6" not in sight_ids
    assert "W1-visible-body-w1-body-2" in sight_ids
    assert "W1-visible-portal-W1-portal-1" in sight_ids
    assert "W1-visible-portal-W1-portal-2" not in sight_ids
...
def test_authenticated_v2_state_migrates_once_and_round_trips() -> None:
    ...
    assert len(observation.objects) == 6
...
def test_v3_hard_limits_reject_expansion() -> None:
    ...
    with pytest.raises(ValueError, match="object capacity"):
        EmbodimentWorldAuthority(authority_key=KEY, max_objects=17)
    ...
    with pytest.raises(ValueError, match="command byte capacity"):
        EmbodimentWorldAuthority(
            authority_key=KEY, max_command_bytes=4097
        )
...
def test_state_byte_capacity_rejects_without_evicting_or_mutating() -> None:
    ...
    receipt = _execute(
        world,
        MoveCommand(PoseMM(PositionMM(1000, 1200, 0), 0)),
        1,
    )
...
def test_ten_thousand_transitions_remain_bounded() -> None:
    ...
    for number in range(1, 10_001):
        ...
        receipt = _execute(
            world,
            MoveCommand(PoseMM(PositionMM(1000, y_mm, 0), 0)),
            number,
        )
