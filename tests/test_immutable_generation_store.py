...
def test_binary_source_same_size_mutation_cannot_commit_mixed_state(
    ...
    def mutate_after_first_binary_block(fd, data):
        nonlocal mutated
        real_write_all(fd, data)
        if len(data) == 1024 * 1024 and not mutated:
            mutated = True
    ...
    monkeypatch.setattr(store_module, "_write_all", mutate_after_first_binary_block)

    with pytest.raises(
        GenerationValidationError,
        match="changed while copied",
    ):
        store.commit(
            tick=5,
            files={"core.json": b'{"tick":5}', "organism.bin": source},
        )

    assert not (store.root / CURRENT_NAME).exists()
    assert not tuple(store.generations_directory.iterdir())


def test_json_source_same_size_mutation_cannot_commit_mixed_state(
        tmp_path, monkeypatch):
    source = tmp_path / "changing.json"
    source.write_text('{"data":"' + "A" * (2 * 1024 * 1024) + '"}')
    store = ImmutableGenerationStore(
        tmp_path / "store",
        identity=IDENTITY,
    ...
    def mutate_after_first_source_block(fd, size):
        nonlocal mutated
        ...
        if len(data) == 1024 * 1024 and not mutated:
            ...
            with source.open("r+b") as handle:
                handle.write(
                    ('{"data":"' + "B" * (2 * 1024 * 1024) + '"}').encode()
                )
                handle.flush()
                os.fsync(handle.fileno())
        return data

    monkeypatch.setattr(store_module.os, "read", mutate_after_first_source_block)

    with pytest.raises(
        GenerationValidationError,
        match="changed while read",
    ):
        store.commit(tick=6, files={"core.json": source})
...
def test_current_replace_failure_preserves_prior_pointer_and_removes_temp(
    ...
    assert len(complete_generations) == 1
