...
def test_native_body_feedback_joins_the_next_world_consequence(
    ...
    world = World()
    body_source = SimpleNamespace(occurrence_count=2)
    restore_calls = []
    ...
    monkeypatch.setattr(
        ...
        lambda *args: (restore_calls.append(args), body_source)[1],
    )
...
def test_native_root_discharge_turns_world_and_returns_typed_direction(
    ...
    assert result is not None
    _, _, episode, admissions, _, vestibular, signed_root_yaw, signed_root_x, signed_root_y = result
...
def test_native_root_translation_moves_world_and_returns_actual_displacement(
    monkeypatch,
) -> None:
    from dsf_ai_service.substrate.embodiment_world import (
        PoseMM,
    ...
    after_body = SimpleNamespace(
        body_id="guala",
        pose=PoseMM(PositionMM(13, 18, 0), 100),
    )
    before = SimpleNamespace(
        revision=7,
        state_sha256="10" * 32,
    ...
    class World:
        ...
        @staticmethod
        def discard_prepared_action(_prepared) -> None:
            raise AssertionError("lawful root translation was discarded")

    root_source = SimpleNamespace(occurrence_count=1)
    built_sources = []
    monkeypatch.setattr(production, "_world", lambda: World())
    monkeypatch.setattr(production, "_world_displacement", lambda *_args: (3, -2, 0, 0))
    monkeypatch.setattr(
        production,
        "_action_consequence_episode",
        lambda *_args, **_kwargs: ("world-source", [(1, 1_000)], {"world": True}),
    ...
    monkeypatch.setattr(
        ...
        lambda *, predecessor_heading_millidegrees,
        signed_displacement_millidegrees,
        duration_microseconds: (predecessor_heading_millidegrees, ()),
    )
    monkeypatch.setattr(
        production,
    ...
    result = production._prepare_continuous_native_action_consequence(
        ...
        root_yaw_unit_recruitments=(),
        root_translation_unit_recruitments=(
            ("40" * 16, 3, 3, "x", "positive"),
            ("41" * 16, 4, 2, "y", "negative"),
        ),
        body_effector_bindings=(),
        articulated_body_consequences=(),
        body_proprioceptive_sources=(),
    ...
    assert result is not None
    _, _, episode, admissions, _, vestibular, yaw, dx, dy = result
    assert episode == ("world-source", root_source)
    assert admissions == ([(1, 1_000)], [(1, 1_000)])
    assert vestibular is None
    assert (yaw, dx, dy) == (0, 3, -2)
    assert built_sources == [
        {
            "source_tick": 8,
            "signed_x_millimetres": 3,
            "signed_y_millimetres": -2,
        }
    ]
