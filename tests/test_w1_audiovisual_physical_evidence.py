...
from dsf_ai_service.substrate.w1_physical_receptors import RETINA_SUBSTREAM_COUNT
...
def _last_sample_impulse_pcm(sample_count: int = 960) -> bytes:
    return struct.pack(
        f"<{sample_count}h",
...
def test_mount_retains_full_fields_but_no_perceptual_identity_or_raw_media():
    ...
    assert set(observed) == {"sight", "sound", "body"}
    assert set(unavailable) == {"touch", "smell", "taste"}
    assert all(
        interpretation.substreams == ()
        for interpretation in unavailable.values()
    )
    assert len(observed["sight"].substreams) == 162
...
def test_ambiguous_visual_source_does_not_erase_heard_pressure():
    ...
    assert result.binaural_auditory_l5 is not None
    assert result.binaural_receptor_settlement is not None
    assert result.anonymous_av_correspondence is None
    assert result.anonymous_av_continuity is None
    assert (
        result.evidence_receipt
        .anonymous_av_correspondence_receipt_sha256 is None
    )
...
def test_action_outcome_settles_anonymous_body_contact_without_fake_sound():
    ...
    assert unavailable == {"sound", "touch", "smell", "taste"}
...
def test_static_current_field_does_not_invent_temporal_dsf_displacement():
    world = _world()
    ...
    assert initial_fingerprint == held_fingerprint == replaced_fingerprint
    assert (
        initial.evidence_receipt.somatic_series_sha256
        == held.evidence_receipt.somatic_series_sha256
        == replaced.evidence_receipt.somatic_series_sha256
    )
    for mount in (initial, held, replaced):
        body = next(
            value for value in mount.causal_settlement.interpretations
            if value.sense == "body"
        )
        assert body.state == "sensor_unavailable"
        touch = next(
            value for value in mount.causal_settlement.interpretations
            if value.sense == "touch"
        )
        assert touch.state == "sensor_unavailable"
...
def test_rejected_world_command_cannot_be_mounted_as_an_action_outcome():
    ...
    rejected = world.execute_port_command(
        port_id=PORT_ID,
        command_payload=encode_command(
            PickCommand("missing-object", 200_000)
        ),
...
def test_visual_transport_capacity_equals_exact_W1_inventory() -> None:
    assert MAX_NATIVE_SIGHT_SUBSTREAMS == RETINA_SUBSTREAM_COUNT == 162
...
def test_max_inventory_mount_preserves_all_sight_ports_and_full_fields():
    ...
    assert len(sight.substreams) == 162
    assert all(
        tuple(name for name, _value in field_tuple.fields)
        == DSF_FIELD_ORDER
...
def test_visual_appearance_disappearance_is_exact_anonymous_and_fail_closed(
    ...
    def probe(before, after):
        ...
        observation = _world().observation_snapshot()
...
def test_visual_order_crossing_retains_the_physical_field_without_attribution():
    accepted = []
    world = EmbodimentWorldAuthority(
        authority_key=WORLD_KEY,
        bodies=(
    ...
    assert result.evidence_receipt is not None
    assert result.causal_settlement is not None
    result.causal_settlement.verify()
    assert accepted == [result.causal_settlement]
    assert owner.status()["settled"] == 1
    assert owner.status()["prepared_reservation"] == 0
    assert authority.status()["active_epochs"] == 0
