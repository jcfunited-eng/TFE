...
def test_browser_audio_is_transport_not_recognition_or_meaning() -> None:
    ...
    assert "pcm_s16le_base64:pcm_b64" in page
...
def test_local_capture_and_organism_acceptance_are_distinct_states() -> None:
    page = _page()
    assert "Local microphone active" in page
    assert "no organism acceptance yet" in page
    assert "organism accepted generation" in page
