...
def test_syllables_come_from_speech_record_and_grow_sequences_by_measured_worth() -> None:
    ...
    assert len(SYLLABLES) == 220 and len(set(SYLLABLE_DRIVES.values())) == 220
...
def test_what_followed_a_moment_is_counted_within_the_window_the_next_moment_and_a_bite() -> None:
    ...
    from dsf_ai_service.guala_functional_organism import FOLLOW_CAPACITY, FOLLOW_WINDOW_BEATS
    ...
    assert all(int(count) >= 1 and len(following) <= FOLLOW_CAPACITY for _key, following in followed for count in following.values())
...
def test_pool_shock_moment_consolidates_on_single_trial_in_organism() -> None:
    """Cognitive Asset 1 (The Pool Shock Principle):
    A high-salience somatic moment (Gamma_salience >= 0.65) consolidates immediately into meanings
    during sleep even with count == 1, whereas low-salience events require recurrence.
    """
    organism = FunctionalOrganism.genesis(identity=IDENTITY, organism_tick=1)
    moments = organism._state["moments"]

    # Acute shock episode: single exposure, high salience
    moments["acute_shock"] = {
        "count": 1,
        "tick": 10,
        "held": "none",
    ...
    # Low-salience background moment: single exposure, zero salience
    moments["background_noise"] = {
        "count": 1,
        "tick": 11,
        "held": "none",
        "source": "heard",
    ...
    # Night consolidation
    organism._dream_moment(100)
    organism._dream_moment(101)

    meanings = organism._state["meanings"]
    assert "acute_shock" in meanings, "Acute shock failed to consolidate on single trial"
    assert "background_noise" not in meanings, "Low-salience event should not consolidate on single trial"
    encoded = organism.encoded()
    assert FunctionalOrganism.restore(encoded).encoded() == encoded


def test_anticipatory_consequence_vetoes_damaging_act_in_organism() -> None:
    """Cognitive Asset 1 (Anticipatory Trajectory Reactivation):
    When meanings contain a past episode of severe damage (< -0.35) for an action under the
    active cue, _choose vetoes that candidate action.
    """
    organism = FunctionalOrganism.genesis(identity=IDENTITY, organism_tick=1)
    # Store prior traumatic consequence
    organism._state["meanings"]["trauma_cue"] = {
        "count": 2,
        "tick": 1,
        "figure": "hot_pan",
        "held": "none",
        "source": "heard",
        "context": [0, 0, 0],
        "acts": {"touch": [1, -0.90]},  # Severe pain from touching
        "fed": 0,
        "salience": 0.90,
    }

    # Set current cue matching the traumatic episode
    organism._state["sight_figure"] = "hot_pan"

    # Candidate acts include both touch and rest
    chosen_act, reason = organism._choose(
        key="0123456789abcdef",
        situation="S0000",
        acts=["touch", "rest"],
    )

    # Touch must be vetoed; rest chosen
    assert chosen_act != "touch", f"Touch should be vetoed by anticipated trauma, got {chosen_act}"
    encoded = organism.encoded()
    assert FunctionalOrganism.restore(encoded).encoded() == encoded


def test_affordance_plan_integrated_in_organism() -> None:
    """Cognitive Asset 2: When hungry with multi-step plan active, organism follows
    affordance plan steps sequentially."""
    from dsf_ai_service.affordance_planner import AffordancePlan, PlanStep
    organism = FunctionalOrganism.genesis(identity=IDENTITY, organism_tick=1)
    organism._state["reserve_micrograms"] = 100_000
    organism._state["feeding"] = True
    ...
    steps = (
        ...
        PlanStep(2, "bite", "apple-1", "held", "fed"),
    )
    plan = AffordancePlan("plan-test", "quench_hunger", "apple-1", steps, 1, 0.8)
    organism._state["affordance_plan"] = plan.to_dict()

    act, reason = organism._choose("0123456789abcdef", "S0000", ["toward_food", "rest"])
    assert act == "toward_food"
    assert "affordance plan step 0" in reason

    act, reason = organism._choose("0123456789abcdef", "S0000", ["grasp", "rest"])
    assert act == "grasp"
    assert "affordance plan step 1" in reason

    encoded = organism.encoded()
    assert FunctionalOrganism.restore(encoded).encoded() == encoded


def test_auditory_motor_spectral_reafference_resonance() -> None:
    """Cognitive Asset 3: Auditory-Motor Reafference & Spectral Babbling Resonance.
    Verifies that continuous 32-channel physical cochlear spectral cosine similarity
    drives vocal formant imitation without static lookup tables, ML, or heuristics."""
    from dsf_ai_service.guala_functional_organism import (
        SYLLABLE_DRIVES, SYLLABLES, cochlear_profile, spectral_cosine_similarity, syllable_pcm,
    )
    from dsf_ai_service.guala_cochlea import one_self_hearing_hop

    # 1. Physical spectral cosine similarity invariants
    p_ones = (1.0,) * 32
    assert spectral_cosine_similarity(p_ones, p_ones) == 1.0
    assert spectral_cosine_similarity(None, p_ones) == 0.0
    assert spectral_cosine_similarity((0.0,) * 32, p_ones) == 0.0

    # 2. Formant discrimination across acoustic airway waveforms
    dah_pcm = syllable_pcm(SYLLABLE_DRIVES['dah0'], 1)
    _, _, dah_coch, _ = one_self_hearing_hop(dah_pcm)
    dah_profile = cochlear_profile(dah_coch)

    dee_pcm = syllable_pcm(SYLLABLE_DRIVES['dee0'], 1)
    _, _, dee_coch, _ = one_self_hearing_hop(dee_pcm)
    dee_profile = cochlear_profile(dee_coch)

    # Identical vowel formants resonate; disparate formants diverge
    rho_self = spectral_cosine_similarity(dah_profile, dah_profile)
    rho_diff = spectral_cosine_similarity(dah_profile, dee_profile)
    assert rho_self > 0.99
    assert rho_diff < 0.40, f'Expected disparate formants to diverge, got {rho_diff}'

    # 3. Closed-loop auditory imitation in organism
    organism = FunctionalOrganism.genesis(identity=IDENTITY, organism_tick=1)
    context = 'reson_sit:start'
    organism._state['speech'][context] = {
        'syllables': {'dah0': [1, 0.0], 'dee0': [1, 0.0]},
        'tick': 1,
    }
    # Caregiver speaks dah0
    organism._state['heard_speech_target'] = {
        'tick': 1,
        'key': 'target_word',
        'profile': list(dah_profile),
    }

    # Guala chooses next syllable under acoustic resonance target
    drive, name, chosen_ctx, reason = organism._choose_syllable('reson_sit', None)
    assert name == 'dah0', f'Expected resonant choice dah0, got {name}'
    assert drive == SYLLABLE_DRIVES['dah0']
    assert 'resonant answer' in reason and 'rho=' in reason

    # 4. Reafference reinforcement upon settling
    # Guala produces dah0 and hears her own voice back
    organism._state['pending_act'] = {
        'key': 'test_key',
        'act': 'say',
        'syllable': 'dah0',
        'context': context,
        ...
        'refused': False,
        'burn': 0,
        'self_profile': list(dah_profile),
    }
    organism._settle('next_key', False, 0.5, 0.0, 2)
    syl_record = organism._state['speech'][context]['syllables']['dah0']
    # Total credit should include baseline sound_value (0.5) + biological resonance reinforcement (round(rho * 1.5, 4))
    assert syl_record[1] > 1.5, f'Expected biological resonance credit, got {syl_record[1]}'
    assert organism._state['speech'][context]['resonance']['dah0'] >= 0.99

    # 5. Deterministic serialization / restoration
    encoded = organism.encoded()
