# TFE 5.3 Current Implementation Plan v2.3
**Document ID:** TFE-5.3-PLAN-2.2  
**Status:** Controlled implementation plan for the current TFE instantiation  
**Scope:** Current TFE instantiation only. This plan is an implementation plan, not the normative TFE specification. It is aligned to `TFE_Specification_v2_3` and its internal review.
...
2. align the implementation with the strengthened v2.3 specification;
3. implement the practical consequences of resolved specification issues 1--3 plus the v2.3 next-pass requirements:
   ...
   - short-side strategy families, pattern/setup rule families, insider-transaction/news governance, and advanced epoch-trading intelligence;
...
### Objective
Implement the v2.3 normative threshold / strategy scaffolding in code and registry form.
...
### Required actions
1. Materialize the normative baseline thresholds from the v2.3 spec into a machine-readable registry.
...
8. Materialize additional controlled registries for:
   - short-side strategy families and blockers;
   - pattern/setup families (double bottom/top, pivots, RSI extremes, Bollinger setups, moving-average/crossover states);
   - insider transaction / ownership signal policy;
   - company-specific news and catalyst policy;
   - epoch tactical classes and source reliability for event/news-driven trade intelligence.
9. Ensure long-side and short-side outputs remain research states unless a later operating profile explicitly authorizes executable trade semantics.
...
### Objective
Implement the v2.3 computational-discipline requirements so the system can prove whether it is operating on structural events and shared state rather than quietly reverting to row-centric behavior.
...
## 6B. Phase B3 — Efficient L5 Computational Profile Prototypes

### Objective
Prototype and instrument lower-overhead L5 realizations that remain faithful to the event-tape/shared-state design.

### Required outputs
- `l5_computational_profile_registry_latest.json`
- `l5_profile_comparison_latest.json`
- `l5_profile_energy_overhead_latest.json`

### Required actions
1. Implement or stub the following implementation profiles behind feature flags:
   - sparse low-rank shared-state profile
   - selective state-space style profile
   - fixed-dynamics reservoir profile
2. Compare each profile against the current pragmatic serialized approximation on a gold slice.
3. Record CPU time, memory, event count, and approximate energy/overhead proxies.
4. Reject any profile that breaks determinism, auditability, or event-tape semantics.

### Required tests
- profile determinism test
- shared-state equivalence sanity check
- overhead comparison test
- structural trace preservation test
...
### Objective
Implement the v2.3 external comparator methodology so benchmark claims become artifact-backed.
...
4. Run the v2.3 structural-geometry vs predictive-analytics traceability audit to prove whether the implementation is consuming shared state or only row-like artifacts.
...
### Objective
Translate the stronger v2.3 financial-governance and epoch-governance specification into explicit controlled registries and executable rules.
