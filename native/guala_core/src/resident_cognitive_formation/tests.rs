...
fn sparse_contact_components_preserve_canonical_breadth_first_order() {
    let contacts = [(2, 3), (0, 2), (1, 3)]
        .into_iter()
        .map(|(left, right)| {
            ElectricalContactAnatomy::new(left, right, ExactRational::integer(1), 5).unwrap()
        })
        .collect();
    let anatomy = SparseElectricalAnatomy::new(5, contacts).unwrap();

    assert_eq!(
        contact_components(5, &anatomy),
        vec![vec![0, 2, 3, 1], vec![4]]
    );
}
...
fn explicit_optical_seed_for_occurrence(
    ...
    let shared = prepare_complete_joint_field_admitted_fixture(source, occurrence_index).unwrap();
...
fn palmar_contact_source_site() -> NeuronSourceSite {
    NeuronSourceSite::from_source_port(&crate::joint_source_episode::JointSourcePortView {
...
fn rejected_w1_retirement_removes_only_the_duplicate_retina_and_integrators() {
    ...
    let corrected = state.retire_rejected_w1_spectral_retina().unwrap().unwrap();
...
fn mount_body_regulation_fixture(
    ...
    let receptor_site =
        NeuronSourceSite::from_source_port(&source.joint_source_ports()[terminal.ordinal()])
            .unwrap();
...
fn singular_palmar_contact_mounts_only_the_two_closing_grip_reflexes() {
    ...
    let (palmar_regulation, palmar_receptor, palmar_site) = mount_body_regulation_from_site_fixture(
        &mut cohorts,
        &mut population,
        &mut next_lineage,
        &mut fabric,
    ...
    for axis in [BodyAxis::LeftGripAperture, BodyAxis::RightGripAperture] {
        let closing = BodyEffectorTerminal::new(axis, BodyEffectorDirection::TowardMinimum);
        let opening = BodyEffectorTerminal::new(axis, BodyEffectorDirection::TowardMaximum);
    ...
    for axis in [BodyAxis::LeftGripAperture, BodyAxis::RightGripAperture] {
        ...
        assert!(
            exact_articulated_body_preparation_regulations(closing, &paths, &[], &[]).is_empty()
        );
        assert!(paths.iter().any(|path| {
            path.receptor_lineage == palmar_receptor && path.receptor_site == palmar_site
        }));
    }
}
...
fn gustatory_contact_mounts_only_the_single_closing_glottal_reflex() {
    ...
    let (taste_regulation, taste_receptor, taste_site) = mount_body_regulation_from_site_fixture(
        &mut cohorts,
        &mut population,
        &mut next_lineage,
        &mut fabric,
    ...
    .unwrap();
    let regulations =
        exact_articulated_body_preparation_regulations(closing, &paths, &[], &[taste_receptor]);
    ...
    assert!(exact_articulated_body_preparation_regulations(closing, &paths, &[], &[]).is_empty());
...
fn terminal_observation_is_derived_by_the_canonical_seal() {
    ...
    let expected_relation_count = prepared.successor.mosaic_of_mosaics_count().unwrap();
...
fn complete_body_source_mounts_bounded_receptor_integration_and_regulation_once() {
    ...
    assert_eq!(
        first.observation.externally_perturbed_body_receptor_count,
        0
    );
    ...
    assert_eq!(
        repeated.successor.retained_neuron_lineages(),
        first_lineages
    );
}
...
fn load_feedback_reenters_after_complete_body_and_repeats() {
    ...
    let first_source = admit_articulated_body_consequence_source(1, &[consequence]).unwrap();
    let first = complete_body
        .prepare(&first_source, 16_000_000)
        .unwrap()
        .successor;
    let second_source = admit_articulated_body_consequence_source(2, &[consequence]).unwrap();
...
fn dormant_four_field_lineage_seed_is_consumed_once_by_exact_current_source() {
    ...
    .unwrap();
    let state =
        ResidentCognitiveFormationState::from_genesis_parts(0, 8, Vec::new(), vec![seed]).unwrap();
...
fn explicit_growth_dna_contacts_survive_restart_and_express_once() {
    ...
    let expressed_channels = prepared.successor.observe_reached_contact_channel_states();
...
fn authored_contact_growth_appends_without_disturbing_a_living_body() {
    ...
    let pre_growth = ResidentCognitiveFormationState::decode(&before_bytes, 16_000_000).unwrap();
...
fn four_receptor_experience_selectively_emits_four_real_fractals() {
    ...
    for source in &recurrence_sources {
        ...
        integrated = ResidentCognitiveFormationState::decode(&interval_bytes, 16_000_000).unwrap();
...
fn one_transition_exports_one_composed_fractal_per_neuron() {
        ...
        if plastic != 0 {
            entries.push(
                PhysicalStateDeltaEntry::new(
                    ...
                    ExactPhysicalStateDelta::Rational(ExactRational::new(plastic, 3).unwrap()),
                )
                .unwrap(),
            );
        }
    ...
    assert_eq!(
        coalesced[0]
            .delta
            .exact_delta(crate::complete_neuron::PhysicalStateCoordinate::GateOpenPopulation,),
...
fn surviving_association_can_relearn_one_missing_cross_sensory_relation() {
    ...
    assert!(adds_unretained_cross_sensory_relation(&current, &retained, &[0], &topology,).unwrap());

    let retained = vec![RetainedOrganismMosaic::newly_admitted(
        already_cross_sensory,
    )];
    assert!(
        !adds_unretained_cross_sensory_relation(&current, &retained, &[0], &topology,).unwrap()
    );
}
...
fn same_members_with_distinct_retained_structure_admit_distinct_formation() {
    ...
    let changed = crate::complete_neuron::SparsePhysicalStateDelta::from_canonical_entries(vec![
        crate::complete_neuron::PhysicalStateDeltaEntry::new(
            crate::complete_neuron::PhysicalStateCoordinate::PlasticRestLength,
            crate::complete_neuron::ExactPhysicalStateDelta::Rational(
...
fn exact_optical_occurrence_physically_changes_the_resident_cell() {
    ...
    let restored = ResidentCognitiveFormationState::decode(&successor_bytes, 16_000_000).unwrap();
...
fn typed_vestibular_source_persists_one_specialized_neuron_then_emits_after_quiescence() {
    let canal_anatomy = CanalAnatomy::new(6, 13_200, PositiveRatio::new(25, 1).unwrap()).unwrap();
    ...
    let retired = ResidentCognitiveFormationState::default()
        ...
        .unwrap();
    let current =
        ResidentCognitiveFormationState::migrate_to_current_format(&retired, 16_000_000).unwrap();
    ...
    let predecessor_total_neurons =
        state.summary().complete_neuron_count + usize::try_from(predecessor_resting_count).unwrap();
    ...
    for source_tick in 1..=(DARK_TAIL_EPISODES * 4) {
        let reached =
            settle_reached_vestibular_bundle_tick(canal_anatomy, canal, 0, bundle_anatomy).unwrap();
...
fn continuing_contact_tail_cannot_begin_endogenous_reassembly() {
    ...
    let restored = ResidentCognitiveFormationState::decode(&encoded_at_rest, 16_000_000).unwrap();
    assert_eq!(restored, state);
    ...
    let original_contact = *retained.active_electrical_contacts.indices.first().unwrap();
...
fn sparse_retained_experience_codec_is_exact_and_canonical() {
    ...
    assert_eq!(&current[..MAGIC_V43.len()], MAGIC_V43);
    assert_eq!(
...
fn local_body_receptor_metabolism_enters_only_the_bounded_sparse_frontier() {
    ...
    for (source_tick, signed_step) in turn.trajectory.as_slice().iter().copied().enumerate() {
        ...
        heading = u32::try_from((i64::from(heading) + i64::from(signed_step)).rem_euclid(360_000))
            .unwrap();
...
fn body_terminal_integration_places_follow_the_existing_sensory_geography() {
    ...
    for (layer, topology_index) in served_pre_proprioceptive_roster {
        let last_existing =
            declared_neuron_territory(DeclaredNeuronPlace::new(layer, topology_index))
                .unwrap()
                .checked_sub(1)
                .unwrap();
        assert!(last_existing < u128::from(BODY_PROPRIOCEPTOR_LAYER6_TOPOLOGY_OFFSET));
    }
    ...
    let last = local_integration_place(DeclaredNeuronPlace::new(
        PhysicalSourceSense::Body.declared_layer().into(),
        u32::try_from(BODY_PROPRIOCEPTOR_TOPOLOGY_OFFSET + LEGACY_BODY_EFFECTOR_TERMINAL_COUNT - 1)
            .unwrap(),
    ))
    .unwrap();
    ...
    let first_tract_integration = local_integration_place(first_tract_proprioceptor).unwrap();
    assert_eq!(
    ...
    let last_root = local_integration_place(DeclaredNeuronPlace::new(
        PRE_PROPRIOCEPTIVE_BODY_SENSE_LAYER,
        u32::try_from(ROOT_YAW_PROPRIOCEPTOR_TOPOLOGY_OFFSET + ROOT_YAW_TERMINAL_COUNT - 1)
            .unwrap(),
    ))
    .unwrap();
...
fn recurrent_cell_output_is_the_only_formation_causal_cue() {
    ...
    let mut source = RetainedOrganismMosaic::newly_admitted(synthetic_admitted_mosaic(
        &[1, 2, 3],
        &[(1, 2), (2, 3)],
        1,
    ));
    ...
    .unwrap();
    let recurrent_bond = StablePhysicalBondReference::new(recurrent, member, 0).unwrap();
    ...
    let cues =
        recurrent_formation_causal_cues(&mosaics, &formation_index, &[predecessor], &[outward])
            .unwrap();
    ...
    .unwrap();
    assert!(
        recurrent_formation_causal_cues(&mosaics, &formation_index, &[predecessor], &[inward],)
            .unwrap()
            .is_empty()
    );
    assert!(
        recurrent_formation_causal_cues(&mosaics, &formation_index, &[], &[outward],)
            .unwrap()
            .is_empty()
    );
}
...
fn pending_original_continuation_requires_the_same_recent_l7_frontier() {
    ...
    fn original_through_association(
        ...
        let bonds = member_lineages
            .map(|member| StablePhysicalBondReference::new(member, association, 0).unwrap());
    ...
    assert!(
        pending_original_continues_through_association(&prior, &current, &topology, &recent,)
            .unwrap()
    );
    ...
    .unwrap());
    assert!(!pending_original_continues_through_association(
        &prior, &unrelated, &topology, &recent,
    )
    .unwrap());
}
...
fn varied_multisensory_occurrences_mount_only_exact_settled_assemblies() {
    ...
    let occupied = cohorts
        ...
        .collect::<Vec<_>>();
    let mut population =
        Some(DevelopmentalRestingPopulation::admit(16_000_000, 100_000, 100, &occupied).unwrap());
    ...
    let before_internal_receipts = mosaics
        ...
        .map(|retained| {
            sha256(
                &encode_retained_organism_mosaic(&cohorts, &fabric, retained, 16_000_000).unwrap(),
            )
        })
        .collect::<Vec<_>>();
    ...
    let mut formation_index = ResidentFormationIndex::build(&mosaics).unwrap();
    let (internal_receipt, internal_total, internal_count, _, internal_cues, external_frontiers, _) =
        settle_organism_mosaic_boundary(
            &cohorts,
            &topology_index,
            &[],
    ...
    for (index, retained) in mosaics.iter().enumerate() {
        ...
        let encoded =
            encode_retained_organism_mosaic(&cohorts, &fabric, retained, 16_000_000).unwrap();
        let cold =
            decode_retained_organism_mosaic(&cohorts, &fabric, &encoded, 16_000_000).unwrap();
    ...
    let mut formation_index = ResidentFormationIndex::build(&mosaics).unwrap();
    let (_, mixed_total, mixed_internal_count, _, mixed_internal_cues, external_frontiers, _) =
        settle_organism_mosaic_boundary(
            &cohorts,
            &topology_index,
            &[],
...
fn affective_growth_is_occurrence_local_and_requires_both_physical_transfers() {
    ...
    let first = vec![
        ...
        ActiveElectricalFrontierEntry::caused(
            bystander_association,
            bystander_integration,
            StablePhysicalBondReference::new(bystander_association, bystander_integration, 0)
                .unwrap(),
            1,
        )
        .unwrap(),
    ...
    let second = vec![
        ...
        ActiveElectricalFrontierEntry::caused(
            unrelated_integration,
            bystander_regulation,
            StablePhysicalBondReference::new(unrelated_integration, bystander_regulation, 0)
                .unwrap(),
            1,
        )
        .unwrap(),
    ...
    let transfers = first
        ...
        .collect::<Vec<_>>();
    let pairs = exact_occurrence_affective_pairs(&associations, &regulations, &transfers).unwrap();
    assert_eq!(
        pairs,
        vec![
    ...
    let first_occurrence_only = transfers
        ...
        .copied()
        .filter(|transfer| {
            transfer.sender != bystander_regulation && transfer.receiver != bystander_regulation
        })
        .collect::<Vec<_>>();
    assert_eq!(
        exact_occurrence_affective_pairs(&associations, &regulations, &first_occurrence_only,)
            .unwrap(),
...
fn v27_unlearned_affective_and_ordering_growth_is_retired_once() {
    ...
    let first_affective = cohorts
        ...
        .unwrap();
    let duplicate_affective =
        mount_next_intrinsic_in_layer(&mut cohorts, &mut population, &mut next_lineage, 10)
            .unwrap();
    let first_ordering =
        mount_next_intrinsic_in_layer(&mut cohorts, &mut population, &mut next_lineage, 11)
            .unwrap();
    let duplicate_ordering =
        mount_next_intrinsic_in_layer(&mut cohorts, &mut population, &mut next_lineage, 11)
            .unwrap();
    fabric = fabric
        .append_contacts(&[
    ...
    v27[MAGIC_V27.len()..MAGIC_V27.len() + 2].copy_from_slice(&VERSION_V27.to_le_bytes());
    assert!(matches!(
    ...
    let current =
        ResidentCognitiveFormationState::migrate_to_current_format(&v27, MAX_BYTES).unwrap();
    assert_eq!(&current[..MAGIC_V43.len()], MAGIC_V43);
    ...
    assert_eq!(
        ResidentCognitiveFormationState::migrate_to_current_format(&current, MAX_BYTES).unwrap(),
        current
    );
}
...
fn new_ordering_route_follows_one_exact_retained_body_motor() {
    ...
    let receptor_site = source
        ...
        .unwrap()
        .unwrap();
    let exact_terminal = BodyEffectorTerminal::new(axis, BodyEffectorDirection::TowardMinimum);
    ...
    fabric = fabric
        .without_contact_pairs(&BTreeSet::from([canonical_lineage_pair(ordering, motor)]))
        .unwrap();
    assert!(!fabric.contains_contact(ordering, motor));
    mount_reached_ordering_reach(
        &mut cohorts,
...
fn coincident_body_regulation_and_ordering_mount_one_reusable_motor_effector() {
    ...
    let (restored_anatomy, restored_state) = decode_reached_cohort_cell(&encoded_motor).unwrap();
...
fn live_reacted_load_prepares_only_the_motor_that_releases_the_joint_stop() {
    let axis = BodyAxis::LeftGripAperture;
    let loaded_terminal =
        BodyProprioceptorTerminal::new(axis, BodyEffectorDirection::TowardMaximum);
    let position_source =
        admit_complete_articulated_body_state_source(0, &ArticulatedBodyState::at_neutral())
            .unwrap();
    let anatomy = axis.anatomy();
    ...
    let paths = [
        MotorBodyAfferentPath {
            ...
            receptor_site: source_site(&load_source, EFFECTOR_REACTIVE_LOAD_FRACTION_QUANTITY),
        },
        MotorBodyAfferentPath {
            body_regulation_lineage: tonic_length_regulation,
            integration_lineage: [5_u8; 16],
            receptor_lineage: [6_u8; 16],
            receptor_site: source_site(&position_source, ANTAGONIST_PROPRIOCEPTOR_LENGTH_QUANTITY),
        },
    ];
...
fn only_incoming_ordering_or_body_regulation_transfer_prepares_motor() {
    ...
    .iter()
    .all(|transfer| transfer.sender != ordering));
    assert_eq!(
        exact_motor_preparation_transfers(
            motor,
            &settled,
            &[regulation],
            &BTreeSet::new(),
            Some(&BTreeSet::from([ordering])),
            Some(&BTreeSet::from([settled[3].bond])),
            layer_of,
        ),
        vec![settled[3]],
        "a vocal motor admits only the currently founded learned ordering",
    );
}
...
fn historical_load_correction_rewires_only_the_rejected_motor_contact() {
    ...
    let wrong_motor =
        mount_next_intrinsic_in_layer(&mut cohorts, &mut population, &mut next_lineage, 12)
            .unwrap();
    cohorts
        .iter_mut()
        ...
        .unwrap();
    let correct_terminal = BodyEffectorTerminal::new(axis, BodyEffectorDirection::TowardMinimum);
    ...
    let occupied_places = cohorts
        ...
        .collect::<Vec<_>>();
    let resting_population =
        DevelopmentalRestingPopulation::admit(MAX_BYTES, 100_000, next_lineage, &occupied_places)
            .unwrap();
    ...
    let current = corrected.encode(MAX_BYTES).unwrap();
    let restored = ResidentCognitiveFormationState::decode(&current, MAX_BYTES).unwrap();
...
fn v33_migration_removes_reintroduced_effector_pools_one_way() {
    ...
    let expected_motor = BodyEffectorTerminal::new(axis, BodyEffectorDirection::TowardMinimum);
    let motor = cohorts
        .iter()
        .flat_map(|cohort| {
            cohort
    ...
    let articulatory =
        mount_next_intrinsic_in_layer(&mut cohorts, &mut population, &mut next_lineage, 13)
            .unwrap();
    ...
    .unwrap();
    let conductance = ExactRational::integer(DEVELOPMENTAL_CONTACT_CONDUCTANCE_PICOSIEMENS);
    fabric = fabric
        .append_contacts(&[
        ...
        mosaic: AdmittedPhysicalMosaic::admitted_for_test(
            ...
            vec![
                bonds
                    .iter()
                    .copied()
                    .find(|bond| bond.endpoints() == canonical_lineage_pair(ordering_a, affective))
                    .unwrap(),
                bonds
                    .iter()
                    .copied()
                    .find(|bond| bond.endpoints() == canonical_lineage_pair(ordering_a, motor))
                    .unwrap(),
            ],
        ),
        recurrent_lineage: None,
    ...
    let migrated =
        ResidentCognitiveFormationState::migrate_to_current_format(&legacy, MAX_BYTES).unwrap();
    let restored = ResidentCognitiveFormationState::decode(&migrated, MAX_BYTES).unwrap();
    assert!(!restored
        .electrical_fabric
        .contains_contact(ordering_a, motor));
    assert!(!restored
        .electrical_fabric
        .contains_contact(ordering_b, motor));
    assert!(!restored
        .electrical_fabric
        .contains_contact(ordering_a, articulatory));
    assert!(restored
        .electrical_fabric
        .contains_contact(regulation, motor));
    assert!(restored
        .electrical_fabric
    ...
    assert_eq!(&migrated[..MAGIC_V43.len()], MAGIC_V43);
    assert_eq!(
        ResidentCognitiveFormationState::migrate_to_current_format(&migrated, MAX_BYTES).unwrap(),
        migrated
    );
}
...
fn motor_contact_requires_directed_causal_chain_not_coincidence() {
    ...
    let synchronous = directed_chain(&cohorts, &fabric, &[routed_ordering, affective, regulation]);
    mount_reached_motor_effector(
        &mut cohorts,
        &mut population,
        &mut next_lineage,
    ...
    let hop_prior =
        frontier_hop_with_frontier(&cohorts, &fabric, affective, routed_ordering, affective);
    mount_reached_motor_effector(
        &mut cohorts,
        &mut population,
        &mut next_lineage,
...
fn delayed_body_return_uses_the_existing_third_exact_frontier() {
    ...
    let older = frontier_hop_with_frontier(&cohorts, &fabric, ordering, association, association);
...
fn moved_root_terminal_mounts_only_its_paired_sensorimotor_reflex() {
    ...
    let mut fabric = ResidentElectricalFabric::default();
    let source =
        crate::root_yaw_joint_source_builder::admit_root_yaw_proprioceptive_source(0, 1_000)
            .unwrap();
    ...
    assert!(
        exact_reached_root_yaw_regulations(&cohorts, &topology, &[], &[regulation],)
            .unwrap()
            .is_empty()
    );
    ...
    assert!(
        exact_reached_root_yaw_regulations(&cohorts, &topology, &[receptor], &[],)
            .unwrap()
            .is_empty()
    );
    let continuations =
        exact_reached_root_yaw_regulations(&cohorts, &topology, &[receptor], &[regulation])
            .unwrap();
    ...
    let prior_ordering =
        frontier_hop_with_frontier(&cohorts, &fabric, ordering_support, ordering, ordering);
    mount_reached_motor_effector_with_root(
        &mut cohorts,
        &mut population,
        &mut next_lineage,
...
fn root_translation_feedback_cannot_repeat_the_action() {
    ...
    assert!(
        exact_root_translation_motor_preparation_transfers(motor, &[feedback], layer_of,)
            .is_empty()
    );
    assert_eq!(
        exact_root_translation_motor_preparation_transfers(motor, &[feedback, command], layer_of,),
        vec![command],
...
fn moved_root_translation_terminal_mounts_its_exact_feedback_anatomy() {
    ...
    let topology = ResidentTopologyIndex::build(&cohorts, &fabric).unwrap();
    let continuations =
        exact_reached_root_translation_regulations(&cohorts, &topology, &[receptor], &[regulation])
            .unwrap();
    assert_eq!(continuations.get(&regulation), Some(&vec![terminal]));
    mount_reached_motor_effector_with_root(
        &mut cohorts,
    ...
    let motors = cohorts
        ...
        .filter_map(|(mount, lineage)| {
            (mount.root_translation_effector_terminal() == Some(terminal)).then_some(*lineage)
        })
        .collect::<Vec<_>>();
    assert_eq!(motors.len(), 1);
    let (translation_mount, translation_neuron) = cohorts
        ...
        .unwrap();
    assert!(
            !terminal_retains_prepared_action_charge(
                translation_mount,
                translation_neuron,
                false,
            ),
...
fn contact_transfer_direction_uses_physical_not_canonical_endpoint_order() {
    ...
    assert_eq!(
        directed_physical_transfer(0, physical_left, physical_right, bond),
        None
    );
}
...
fn v41_mounts_one_new_dedicated_vocal_body_without_selecting_historical_cells() {
    ...
    let predecessor =
        ResidentCognitiveFormationState::decode_for_one_way_migration(&v40, MAX_BYTES).unwrap();
...
fn v34_replaces_broad_articulatory_pool_with_fixed_vocal_route_once() {
    ...
    let migrated =
        ResidentCognitiveFormationState::migrate_to_current_format(&v32, MAX_BYTES).unwrap();
    assert_eq!(&migrated[..MAGIC_V43.len()], MAGIC_V43);
    let restored = ResidentCognitiveFormationState::decode(&migrated, MAX_BYTES).unwrap();
    ...
    assert!(!restored
        .electrical_fabric
        .contains_contact(acoustic, articulatory));
    assert!(!restored
        .electrical_fabric
        .contains_contact(regulation, articulatory));
    assert!(!restored
        .electrical_fabric
        .contains_contact(motor, articulatory));
    assert!(restored
        .electrical_fabric
        .contains_contact(acoustic, regulation));
...
fn v32_retires_misprojected_root_yaw_paths_once() {
    ...
    let legacy_topology = u32::try_from(
        declared_neuron_territory(DeclaredNeuronPlace::from_source_site(&receptor_site)).unwrap()
            - 1,
    )
    .unwrap();
    ...
    let decoded = ResidentCognitiveFormationState::decode_for_one_way_migration(&v31, MAX_BYTES)
    ...
    let migrated =
        ResidentCognitiveFormationState::migrate_to_current_format(&v31, MAX_BYTES).unwrap();
    assert_eq!(&migrated[..MAGIC_V43.len()], MAGIC_V43);
    let restored = ResidentCognitiveFormationState::decode(&migrated, MAX_BYTES).unwrap();
    assert!(restored.electrical_fabric.contains_contact(receptor, {
        restored
            ...
            .find_map(|(mount, lineage)| {
                (mount.place()
                    == local_integration_place(DeclaredNeuronPlace::from_source_site(
                        &receptor_site,
                    ))
                    .unwrap())
                .then_some(*lineage)
...
fn historical_background_growth_migrates_once_and_cannot_restore() {
    ...
    let decoded =
        ResidentCognitiveFormationState::decode_for_one_way_migration(&v25, MAX_BYTES).unwrap();
    let decoded = decoded
        .retire_aliased_local_integrators()
        .unwrap()
        .unwrap_or(decoded);
    let corrected = decoded.retire_duplicate_motor_effectors().unwrap().unwrap();
    assert_eq!(
        corrected.electrical_fabric.contact_count(),
        contact_count - 2
    );

    let current =
        ResidentCognitiveFormationState::migrate_to_current_format(&v25, MAX_BYTES).unwrap();
    assert_eq!(&current[..MAGIC_V43.len()], MAGIC_V43);
    let restored = ResidentCognitiveFormationState::decode(&current, MAX_BYTES).unwrap();
    assert_eq!(
        restored
...
#[test]
fn exact_body_source_mounts_one_coordinated_vocal_preparation() {
    let mut cohorts = Vec::new();
    let mut population =
        Some(DevelopmentalRestingPopulation::admit(1_600_000_000, 100_000, 100, &[]).unwrap());
    let mut next_lineage = 1;
    ...
    for (index, terminal) in terminals.iter().copied().enumerate() {
        let (regulation, _, _) = mount_body_regulation_fixture(
            &mut cohorts,
            &mut population,
            &mut next_lineage,
        ...
        let association = mount_intrinsic_neuron_at_place(
            ...
            &mut population,
            &mut next_lineage,
            DeclaredNeuronPlace::new(7, u32::try_from(index).unwrap()),
        )
        .unwrap();
    ...
    mount_reached_affective_reach_indexed(
        ...
        &associations
            .iter()
            .copied()
            .zip(regulations.iter().copied())
            .collect::<Vec<_>>(),
    )
    .unwrap();
    let topology = ResidentTopologyIndex::build(&cohorts, &fabric).unwrap();
    let reached = ReachedAssociationsByOccurrence {
        lineages: associations
            .iter()
            .copied()
            .map(|association| vec![association])
            .collect(),
    };
    let moved = regulations
        .iter()
        .copied()
    ...
    let complete_recruitments = first_preparation
        .motors
        .iter()
        .map(|motor| MotorUnitRecruitment {
            neuron_lineage: motor.lineage,
            topology_index: 0,
            outward_elementary_carriers: 1,
            body_effector_terminal: terminal_for(motor.lineage),
            ...
            preparation_transfers: vec![MotorPreparationTransfer {
                transfer: DirectedPhysicalTransferObservation {
                    sender: first_preparation.ordering_lineage,
                    receiver: motor.lineage,
                    bond: motor.bond,
                    transferred_whole_carriers: 1,
                },
                sender_layer: 11,
        ...
        .collect::<Vec<_>>();
    let completed =
        exact_completed_vocal_preparation_discharge(&first_preparation, &complete_recruitments)
            .unwrap()
            .unwrap();
    assert_eq!(completed.0.len(), first_preparation.motors.len());
    assert_eq!(completed.1.len(), first_preparation.motors.len());
    assert!(exact_completed_vocal_preparation_discharge(
        &first_preparation,
        &complete_recruitments[..1],
    )
    .unwrap()
    .is_none());
    let mut duplicate_recruitments = complete_recruitments.clone();
    duplicate_recruitments.push(complete_recruitments[0].clone());
    assert!(exact_completed_vocal_preparation_discharge(
        &first_preparation,
    ...
    let association_contact = first[0].preparation.associations[0];
    let exact_predecessor = ActiveElectricalFrontierEntry::caused_with_provenance(
    ...
    assert_eq!(
        ...
        .unwrap(),
        Some(first[0].preparation.ordering_lineage),
        "causal reach advances the preparation even when symmetric current supplies the reached association",
    );
    let wrong_frontier = ActiveElectricalFrontierEntry::caused_with_provenance(
        association_contact.lineage,
        first[0].preparation.ordering_lineage,
        association_contact.lineage,
        association_contact.bond,
    ...
    let efferent = ActiveElectricalFrontierEntry::caused_with_frontier(
        association_contact.lineage,
        first[0].preparation.ordering_lineage,
        first[0].preparation.ordering_lineage,
        association_contact.bond,
    ...
    .unwrap();
    let wrong_bond = StablePhysicalBondReference::new(
        association_contact.lineage,
        first[0].preparation.ordering_lineage,
        association_contact.bond.parallel_ordinal() + 1,
    )
    .unwrap();
    let wrong_bond = ActiveElectricalFrontierEntry::caused_with_provenance(
        association_contact.lineage,
        first[0].preparation.ordering_lineage,
        first[0].preparation.ordering_lineage,
        wrong_bond,
    ...
    .unwrap();
    let unattributed = ActiveElectricalFrontierEntry::caused_with_frontier(
        association_contact.lineage,
        first[0].preparation.ordering_lineage,
        first[0].preparation.ordering_lineage,
        association_contact.bond,
    ...
    .unwrap();
    let topology = ResidentTopologyIndex::build(&cohorts, &fabric).unwrap();
    let continuations = vocal_cognitive_action_continuation_routes_from_source(
        &cohorts,
        &topology,
        first[0].preparation.ordering_lineage,
    )
    ...
    for continuation in &continuations {
        ...
        .unwrap();
        assert!(frontier_carries_vocal_action_continuation(
            &[arrival],
            continuation,
        ));
        let reverse_arrival = ActiveElectricalFrontierEntry::caused_with_frontier(
            continuation.destination_ordering_lineage,
            continuation.source_ordering_lineage,
            continuation.destination_ordering_lineage,
            continuation.continuation_bond,
...
fn returned_motor_synergy_grows_one_bounded_coaged_source_per_terminal() {
    ...
    let bond_for =
        |cohorts: &[ResidentReachedCohort], fabric: &ResidentElectricalFabric, left, right| {
        ...
        .filter_map(|(left, right)| {
            ...
            match (
                cohorts
                    ...
                    .find_map(|(mount, lineage)| {
                        (*lineage == left).then_some(mount.place().layer())
                    }),
                cohorts
                    .iter()
                    .flat_map(|cohort| {
                    ...
                    .find_map(|(mount, lineage)| {
                        (*lineage == right).then_some(mount.place().layer())
                    }),
            ) {
    ...
    let first_sibling = *first_sources
        .iter()
        .find(|source| **source != anchor)
        .unwrap();
...
fn motor_effector_exposes_exact_sparse_body_afferent_ancestry() {
    ...
    let flat_locations = cohorts
        ...
        .flat_map(|(cohort_index, cohort)| {
            cohort
                ...
                .iter()
                .enumerate()
                .map(move |(neuron_index, lineage)| (cohort_index, neuron_index, *lineage))
        })
        .collect::<Vec<_>>();
    ...
    let motor_flat = flat_locations
        ...
        .unwrap();
    let paths = exact_motor_body_afferent_paths(motor_flat, &flat_locations, &cohorts, &neighbours)
        .unwrap();
...
fn ambiguous_returned_vocal_consequence_cannot_author_motor_contact() {
    ...
    let (unrelated_sound_receptor, unrelated_acoustic) = mount_receptor_local_integration_fixture(
        &mut cohorts,
        &mut population,
        &mut next_lineage,
        &mut fabric,
    ...
    fabric = fabric
        ...
        .unwrap();
    let motor = mount_next_intrinsic_in_layer(&mut cohorts, &mut population, &mut next_lineage, 12)
        .unwrap();
    cohorts
        .iter_mut()
        .find(|cohort| cohort.anatomy.neuron_lineages().contains(&motor))
    ...
    fabric = fabric
        ...
        .unwrap();
    let contacts_before = fabric.contact_count();
    let mut current_consequence = directed_chain(&cohorts, &fabric, &[regulation, affective]);
    ...
    let preceding_ordering =
        frontier_hop_with_frontier(&cohorts, &fabric, ordering, association, association);
    let mut preceding_action =
        frontier_hop_with_frontier(&cohorts, &fabric, affective, association, affective);
    ...
    v37[MAGIC_V37.len()..MAGIC_V37.len() + std::mem::size_of::<u16>()]
        .copy_from_slice(&VERSION_V30.to_le_bytes());
    let migrated =
        ResidentCognitiveFormationState::migrate_to_current_format(&v37, 1_600_000_000).unwrap();
    assert_eq!(&migrated[..MAGIC_V43.len()], MAGIC_V43);
    ...
    assert_eq!(
        ResidentCognitiveFormationState::migrate_to_current_format(&migrated, 1_600_000_000,)
            .unwrap(),
        migrated
    );
    ...
    v38[MAGIC_V38.len()..MAGIC_V38.len() + std::mem::size_of::<u16>()]
        .copy_from_slice(&VERSION_V30.to_le_bytes());
    let direct =
        ResidentCognitiveFormationState::migrate_to_current_format(&v38, 1_600_000_000).unwrap();
    assert_eq!(&direct[..MAGIC_V43.len()], MAGIC_V43);
    let restored_direct = ResidentCognitiveFormationState::decode(&direct, 1_600_000_000).unwrap();
    ...
    assert_eq!(
        ResidentCognitiveFormationState::migrate_to_current_format(&direct, 1_600_000_000,)
            .unwrap(),
        direct
    );
}
...
fn non_simultaneous_body_and_sensory_activity_does_not_manufacture_effectors() {
    ...
    for interval in 0..256 {
        let prepared = state
            .prepare(&source, 16_000_000)
            .unwrap_or_else(|error| panic!("optical interval {interval} failed: {error:?}"));
        let emitted_lineages = prepared
            ...
            .map(|fractal| fractal.neuron_lineage)
            .collect::<Vec<_>>();
        motor_recruitments.extend(prepared.observation.motor_unit_recruitments.iter().cloned());
    ...
    let layer_eleven_lineages = state
        ...
        .filter_map(|(mount, lineage)| (mount.place().layer() == 11).then_some(*lineage))
        .collect::<Vec<_>>();
    ...
    let layer_of = |lineage: [u8; 16]| {
        state
            ...
            .find_map(|(mount, candidate)| (*candidate == lineage).then_some(mount.place().layer()))
    };
    ...
    let formation_layers = state
        ...
        .collect::<Vec<_>>();
    assert!(
        emitted_layers.contains(&0) && emitted_layers.contains(&1) && emitted_layers.contains(&7),
...
fn newly_mounted_nested_formations_keep_distinct_recurrent_lineages() {
    ...
    validate_recurrent_retention_lineage(&cohorts, &fabric, &narrow_members, mounted[0]).unwrap();
...
fn adjacent_exact_contact_transfers_are_the_only_ordered_path_evidence() {
    ...
    let predecessor = [ActiveElectricalFrontierEntry::caused(first, via, first_bond, 7).unwrap()];
    let current = [ActiveElectricalFrontierEntry::caused(via, last, second_bond, 5).unwrap()];
    let incidence = [(first, 0), (via, 0), (via, 1), (last, 1)];
    let paths = ordered_physical_paths_for_relation(&incidence, &[0, 1], &predecessor, &current);
    assert_eq!(paths.len(), 1);
    assert_eq!(
...
fn directed_transfer_frontier_preserves_direction_and_one_advancing_endpoint() {
    ...
    let transfer = ActiveElectricalFrontierEntry::caused(sender, receiver, bond, 7).unwrap();
    ...
    assert!(!frontier_crossing_advances(
        &incoming, receiver, bond, false,
    ));
    assert!(frontier_crossing_advances(&incoming, receiver, bond, true,));
    assert_eq!(
    ...
    assert_eq!(
        reverse_frontier.directed_transfer(),
        transfer.directed_transfer()
    );
    let mut encoded = Vec::new();
    ...
    let mut cursor = 0;
    assert_eq!(
        ActiveElectricalFrontierEntry::decode_v20(&encoded, &mut cursor, false, false, false),
        Err(FormationError::NoncanonicalState)
    );

    let body_owned_acoustic_efference = ActiveElectricalFrontierEntry::caused_with_frontier(
    ...
    let external_ingress = ActiveElectricalFrontierEntry::caused_with_provenance(
        sender, receiver, receiver, bond, 7, false, true,
    )
    .unwrap();
    let mut encoded = Vec::new();
    external_ingress.encode_v20(&mut encoded);
    let mut cursor = 0;
    assert_eq!(
        ActiveElectricalFrontierEntry::decode_v20(&encoded, &mut cursor, true, false, false),
        Err(FormationError::NoncanonicalState),
        "V43 cannot read V44 external-ingress provenance"
    );
    let mut cursor = 0;
    let decoded =
        ActiveElectricalFrontierEntry::decode_v20(&encoded, &mut cursor, true, false, true)
            .unwrap();
    assert_eq!(decoded, external_ingress);
    assert!(decoded.carries_external_ingress_cause());
    assert_eq!(cursor, encoded.len());

    let external_in_flight = ActiveElectricalFrontierEntry::causal_in_flight_with_provenance(
        sender, receiver, receiver, bond, false, true,
    )
    .unwrap();
    let mut encoded = Vec::new();
    ...
    let in_flight = ActiveElectricalFrontierEntry::in_flight(sender, receiver, bond).unwrap();
    assert!(in_flight.is_in_flight());
    assert_eq!(in_flight.frontier_lineage(), sender);
    assert_eq!(in_flight.directed_transfer(), None);
    assert!(!in_flight.carries_body_owned_acoustic_efference());
    let mut encoded = Vec::new();
    in_flight.encode_v20(&mut encoded);
    let mut cursor = 0;
    assert_eq!(
        ActiveElectricalFrontierEntry::decode_v20(&encoded, &mut cursor, true, false, false),
        Err(FormationError::NoncanonicalState),
        "pre-V43 codecs refuse zero-crossed-carrier causes"
    );
    let mut cursor = 0;
    assert_eq!(
        ActiveElectricalFrontierEntry::decode_v20(&encoded, &mut cursor, true, true, false)
            .unwrap(),
        in_flight
    );
    assert_eq!(cursor, encoded.len());

    let reverse_current_causal_frontier =
        ActiveElectricalFrontierEntry::causal_in_flight(sender, receiver, receiver, bond).unwrap();
    assert!(!reverse_current_causal_frontier.is_in_flight());
    assert!(reverse_current_causal_frontier.is_zero_carrier_frontier());
    assert_eq!(reverse_current_causal_frontier.frontier_lineage(), receiver);
    let mut encoded = Vec::new();
    reverse_current_causal_frontier.encode_v20(&mut encoded);
    let mut cursor = 0;
    assert_eq!(
        ActiveElectricalFrontierEntry::decode_v20(&encoded, &mut cursor, true, true, false)
            .unwrap(),
        reverse_current_causal_frontier,
    );
    assert_eq!(cursor, encoded.len());
...
fn body_consequence_preserves_both_directions_on_the_reached_vestibular_relation() {
    ...
    assert!(body_consequence_transfer_observation(&outward, &layers, &reached, false).is_empty());
    ...
    let inward = [ActiveElectricalFrontierEntry::caused(consequence, regulation, bond, 7).unwrap()];
...
fn two_recurring_ordered_paths_require_the_same_directed_physical_route() {
    ...
    let oldest =
        [
            ActiveElectricalFrontierEntry::caused(first, first_bond.endpoints().1, first_bond, 7)
                .unwrap(),
        ];
    let older = [ActiveElectricalFrontierEntry::caused(second, third, second_bond, 5).unwrap()];
    let preceding = [ActiveElectricalFrontierEntry::caused(first, second, first_bond, 9).unwrap()];
    let current = [ActiveElectricalFrontierEntry::caused(second, third, second_bond, 4).unwrap()];
...
fn v19_recipient_only_frontier_cannot_cross_current_topology_boundary() {
    ...
    let legacy = state
        ...
        .unwrap();
    let current =
        ResidentCognitiveFormationState::migrate_to_current_format(&legacy, 16_000_000).unwrap();
    assert_eq!(&current[..MAGIC_V43.len()], MAGIC_V43);
