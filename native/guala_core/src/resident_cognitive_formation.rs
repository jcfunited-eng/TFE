...
use crate::root_translation_terminal::{
    RootTranslationEffectorTerminal, RootTranslationProprioceptorTerminal,
    ROOT_TRANSLATION_PROPRIOCEPTOR_TOPOLOGY_OFFSET, ROOT_TRANSLATION_TERMINAL_COUNT,
};
...
/// V31 marks the one-way effector-pool decontamination boundary: a V31 body
/// has crossed the removal of every historically accumulated layer-11->12
/// and layer-11->13 contact, and its remaining effector contacts were
...
/// V34 restores the fixed anatomical bridge from the articulated body's ten
/// vocal antagonist motor terminals to one layer-13 respiratory/vocal
/// excitation cell. V33 correctly removed the learned coincidence pool, but
/// also removed this necessary innate route and then required a prior
/// utterance before it could be rebuilt. A V34 body can physically produce a
...
/// V37 removes only learned layer-11 contacts into fixed vocal layer-12
/// terminals that were admitted under V36's broad "some sound returned"
/// condition. Their acoustic-form provenance cannot be recovered. All fixed
/// vocal anatomy and unrelated learned motor routes remain. New vocal routes
/// require exact returned sound through the ordering route's own founding
/// sensory assembly, so the removed pool cannot regrow under the old law.
const MAGIC_V37: &[u8; 8] = b"GLCOG037";
/// V38 removes the vocal layer-11 -> layer-12 pool admitted when a general
/// whole-tract consequence was allowed to name every vocal motor that happened
/// to discharge in the preceding interval.  That rule collapsed a learned
/// articulatory gesture into cumulative fan-out: the restored production body
/// had all 26 antagonist vocal terminals attached to only two ordering cells.
/// A V38 body can learn a vocal motor only through that exact axis terminal's
/// own proprioceptive regulation path.  The fixed layer-12 -> layer-13 anatomy,
/// body, neurons, full DSF fields and unrelated motor learning are unchanged.
/// A retained formation whose original topology depended on one of the false
/// contacts is retired instead of being preserved as learned truth.  The byte
/// layout remains V30--V37.
const MAGIC_V38: &[u8; 8] = b"GLCOG038";
/// V39 retires the fixed layer-12 -> layer-13 convergence hub.  That hub made
/// every vocal motor an electrical input to one shared cell, so unrelated
/// motor state could suppress or excite respiration and an exact learned
/// motor discharge was not sufficient to reach the vocal body.  The declared
/// segmentation law instead gives layer 13 ordered language/action ingress.
/// A V39 body therefore retains exact layer-11 -> layer-12 tract control and
/// gives the same proved ordering lineage one sparse direct contact to the
/// resident layer-13 respiratory/articulatory effector.  The byte layout is
/// unchanged; the identity prevents the rejected hub from returning on cold
/// restore.
const MAGIC_V39: &[u8; 8] = b"GLCOG039";
/// V40 replaces V39's passive layer-11 -> layer-13 electrical identity bridge
...
/// regulator for a neutral motor strictly lowers electrostatic energy. A
/// generic "next layer-12 cell" has no such guarantee and produced the live
/// 37,942-pF -> 178-pF refused contact.
const ROOT_TRANSLATION_LAYER12_TOPOLOGY_OFFSET: u32 =
...
const CURRENT_FIXED_BYTES: usize =
    FIXED_BYTES + (7 * std::mem::size_of::<u64>()) + 1;
...
/// Identify the one causal boundary crossed by a settled contact.
///
/// Electrical maintenance may settle a contact with zero, one, or two causal
/// endpoints. Only exactly one causal endpoint can advance the organism's
/// cognitive frontier. In particular, a scheduler-selected contact with no
/// causal endpoint remains physics and can never manufacture action authority.
fn causal_frontier_crossing(
    left_flat: usize,
    right_flat: usize,
    left_is_seed: bool,
    right_is_seed: bool,
    ...
    if left_is_seed == right_is_seed {
        ...
        Some((left_flat, right_flat))
    } else {
        Some((right_flat, left_flat))
    }
}
...
/// One transient discharge through a retained root-translation effector.
/// Axis and direction are fixed body anatomy; the whole-carrier discharge is
/// the movement magnitude. This contains no destination or semantic command.
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct RootTranslationUnitRecruitment {
    pub(crate) neuron_lineage: [u8; 16],
    pub(crate) topology_index: u32,
    pub(crate) outward_elementary_carriers: u128,
    pub(crate) terminal: RootTranslationEffectorTerminal,
    pub(crate) preparation_transfers: Vec<DirectedPhysicalTransferObservation>,
}
...
pub(crate) struct ResidentCognitiveFormationState {
    ...
    /// act only through the exact motor-coupled settlement law.
...
fn validate_motor_effector_mounts(
    ...
    let mut root_translation_terminals = BTreeSet::<RootTranslationEffectorTerminal>::new();
    for mount in cohorts.iter().flat_map(|cohort| cohort.anatomy.mounts()) {
        ...
        match (
            ...
            mount.root_translation_effector_terminal(),
        ) {
            (Some(terminal), None, None) if body_terminals.insert(terminal) => {}
            (None, Some(terminal), None) if root_yaw_terminals.insert(terminal) => {}
            (None, None, Some(terminal)) if root_translation_terminals.insert(terminal) => {}
...
/// Validate the fixed vocal-body bridge independently of learned speech.
/// Every mounted vocal antagonist motor must converge on exactly one
/// layer-13 excitation cell. No non-vocal motor may use that boundary and no
/// motor may fan out across multiple layer-13 cells.
fn validate_fixed_vocal_articulatory_route(
    cohorts: &[ResidentReachedCohort],
    electrical_fabric: &ResidentElectricalFabric,
) -> Result<(), FormationError> {
    let mounted = cohorts
    ...
    let vocal_motors = mounted
        ...
        .collect::<BTreeSet<_>>();
    if vocal_motors.is_empty() {
        return Ok(());
    }

    let mut target = None;
    let mut routed_motors = BTreeSet::new();
    for (left, right) in electrical_fabric.contact_endpoints() {
        let left = electrical_fabric.lineages()[left];
        let right = electrical_fabric.lineages()[right];
        let (motor, articulatory) = match (
            mounted.get(&left).map(|mount| mount.place().layer()),
            mounted.get(&right).map(|mount| mount.place().layer()),
        ) {
            (Some(12), Some(13)) => (left, right),
            (Some(13), Some(12)) => (right, left),
            _ => continue,
        };
        if !vocal_motors.contains(&motor) {
            return Err(FormationError::NeuronLineageAuthorityChanged);
        }
        match target {
            Some(existing) if existing != articulatory => {
                return Err(FormationError::NeuronLineageAuthorityChanged)
            }
            None => target = Some(articulatory),
            Some(_) => {}
        }
        routed_motors.insert(motor);
    }
    if target.is_none() || routed_motors != vocal_motors {
        return Err(FormationError::NeuronLineageAuthorityAbsent);
    }
    Ok(())
}

/// Validate the current direct ordered-articulation anatomy.
///
/// Layer 12 remains the typed antagonist motor authority for jaw, glottis and
/// tract shape.  Layer 13 is a separate respiratory/articulatory effector and
/// may be reached only by an ordering lineage that also owns at least one
/// sparse learned route to a vocal layer-12 motor.  No layer-12 -> layer-13
/// bridge, sensory coincidence, observer label or generic body activity may
/// reach this boundary.
fn validate_v39_passive_vocal_articulatory_route(
    cohorts: &[ResidentReachedCohort],
    electrical_fabric: &ResidentElectricalFabric,
) -> Result<(), FormationError> {
    let mounted = cohorts
        ...
        .collect::<BTreeMap<_, _>>();
    let layer_of = |lineage: [u8; 16]| {
        mounted.get(&lineage).map(|mount| mount.place().layer())
    };
    let vocal_motors = mounted
        .iter()
        .filter_map(|(lineage, mount)| {
    ...
    let mut ordering_with_vocal_motor = BTreeSet::new();
    let mut ordering_to_articulatory = BTreeMap::<[u8; 16], [u8; 16]>::new();
    for (left, right) in electrical_fabric.contact_endpoints() {
        let left = electrical_fabric.lineages()[left];
        let right = electrical_fabric.lineages()[right];
        match (layer_of(left), layer_of(right)) {
            (Some(11), Some(12)) if vocal_motors.contains(&right) => {
                ordering_with_vocal_motor.insert(left);
            }
            (Some(12), Some(11)) if vocal_motors.contains(&left) => {
                ordering_with_vocal_motor.insert(right);
            }
            (Some(11), Some(13)) if articulatory.contains(&right) => {
                if ordering_to_articulatory.insert(left, right).is_some() {
                    return Err(FormationError::NeuronLineageAuthorityChanged);
                }
            }
            (Some(13), Some(11)) if articulatory.contains(&left) => {
                if ordering_to_articulatory.insert(right, left).is_some() {
                    return Err(FormationError::NeuronLineageAuthorityChanged);
                }
            }
            (Some(12), Some(13)) | (Some(13), Some(12)) => {
                return Err(FormationError::NeuronLineageAuthorityChanged)
            }
            (Some(13), _) | (_, Some(13)) => {
                return Err(FormationError::NeuronLineageAuthorityChanged)
    ...
    if ordering_to_articulatory.keys().copied().collect::<BTreeSet<_>>()
        != ordering_with_vocal_motor
    {
        return Err(FormationError::NeuronLineageAuthorityAbsent);
    }
    if ordering_to_articulatory
        .values()
        .copied()
        .collect::<BTreeSet<_>>()
        .len()
...
/// Validate the V40 motor-coupled vocal-body anatomy.
///
...
fn validate_motor_coupled_vocal_articulatory_route(
    ...
    let mounted = cohorts
        ...
        .map(|(mount, lineage)| (*lineage, mount))
        .collect::<BTreeMap<_, _>>();
    match vocal_articulatory_effector_lineage {
        Some(lineage)
            if mounted.get(&lineage).is_some_and(|mount| {
                mount.source_site().is_none() && mount.place().layer() == 13
            }) => {}
        // Absence is truthful: the organism has no mounted respiratory body
        // to recruit yet. Typed vocal motors may exist before embodiment, but
        // they cannot manufacture an effector or an utterance.
        None => {}
        _ => return Err(FormationError::NeuronLineageAuthorityAbsent),
    }
    for (left, right) in electrical_fabric.contact_endpoints() {
        let left = electrical_fabric.lineages()[left];
        let right = electrical_fabric.lineages()[right];
        if mounted
            .get(&left)
            .is_some_and(|mount| mount.place().layer() == 13)
            || mounted
                .get(&right)
                .is_some_and(|mount| mount.place().layer() == 13)
        {
...
impl ResidentCognitiveFormationState {
    ...
    /// One-way decontamination of the accumulated effector pools: remove
    /// EVERY layer-11->12 and layer-11->13 fabric contact. The historical
    /// pool was authored by same-interval coincidence and carries no
    /// trustworthy causal evidence, so no per-contact evidence test is
    /// attempted. Neurons, effector terminals, body anatomy, memories,
    /// reservoirs, and every unrelated contact are preserved exactly;
    /// retained frontier entries riding a removed contact are dropped with
    /// it. Runs once at the authenticated V31 migration boundary; a V31
    /// body never re-enters it, and the consecutive causal-frontier law is
    /// the only way a new effector contact can ever be authored again.
    fn retire_contaminated_effector_pools(
        &self,
    ) -> Result<Option<Self>, FormationError> {
        let mut layer_by_lineage = BTreeMap::<[u8; 16], u32>::new();
        for (mount, lineage) in self.cohorts.iter().flat_map(|cohort| {
    ...
    /// Remove only vocal routes learned under the retired V36 feedback law.
    /// That law proved a vocal discharge and some returned layer-1 activity,
    /// but did not prove the returned acoustic integration belonged to the
    /// ordering route's own founding sensory assembly. The missing provenance
    /// cannot be reconstructed after the fact, so every such learned contact
    /// is retired once. Fixed layer-12 -> layer-13 vocal anatomy, non-vocal
    /// motor learning, neurons, reservoirs, and unrelated formations remain.
    fn retire_unproved_vocal_learning_contacts(
        &self,
    ) -> Result<Option<Self>, FormationError> {
        let mounted = self
            .cohorts
            ...
            .map(|(mount, lineage)| (*lineage, mount))
            .collect::<BTreeMap<_, _>>();
        let is_ordering = |lineage: [u8; 16]| {
            mounted
                .get(&lineage)
                .is_some_and(|mount| mount.place().layer() == 11)
        };
        let is_vocal_motor = |lineage: [u8; 16]| {
            mounted.get(&lineage).is_some_and(|mount| {
                mount.place().layer() == 12
                    && mount
                        .body_effector_terminal()
                        .is_some_and(|terminal| terminal.axis().is_vocal_articulator())
        ...
        let retired = self
            ...
            .filter_map(|(left, right)| {
                ...
                ((is_ordering(left) && is_vocal_motor(right))
                    || (is_ordering(right) && is_vocal_motor(left)))
    ...
    /// Cross the V34 first-utterance boundary once. V33 has already removed
    /// every historical layer-13 coincidence contact. This step mounts only
    /// the fixed vocal-motor anatomy that a living body must possess before
    /// learned activity can cause its first pressure wave.
    fn into_fixed_vocal_articulatory_route(mut self) -> Result<Self, FormationError> {
        let mut cohorts = self.cohorts.into_vec();
        mount_fixed_vocal_articulatory_route(
            &mut cohorts,
            &mut self.resting_population,
            &mut self.next_lineage_ordinal,
            &mut self.electrical_fabric,
        )?;
    ...
    /// Cross the V40 boundary once: retain the unique V39 vocal-body target
    /// as explicit anatomy, then remove every electrical contact incident to
    /// layer 13. No neuron or physical state is copied or replaced. A later
    /// motor-coupled discharge addresses the retained body lineage directly.
    fn into_motor_coupled_vocal_articulatory_route(mut self) -> Result<Self, FormationError> {
        let mounted = self
            .cohorts
            .iter()
            .flat_map(|cohort| {
        ...
        let layer_thirteen = mounted
            ...
            .collect::<BTreeSet<_>>();
        let mut contacted_targets = BTreeSet::new();
        let mut retired = BTreeSet::new();
        for (left, right) in self.electrical_fabric.contact_endpoints() {
            let left = self.electrical_fabric.lineages()[left];
            let right = self.electrical_fabric.lineages()[right];
            if layer_thirteen.contains(&left) || layer_thirteen.contains(&right) {
                if layer_thirteen.contains(&left) {
                    contacted_targets.insert(left);
                }
                if layer_thirteen.contains(&right) {
                    contacted_targets.insert(right);
                }
                retired.insert(canonical_lineage_pair(left, right));
            }
        }
        if contacted_targets.len() > 1 {
            return Err(FormationError::NeuronLineageAuthorityChanged);
        }
        let target = match contacted_targets.iter().next().copied() {
            Some(lineage) => Some(lineage),
            // Migration preserves anatomy; it never invents a respiratory
            // body merely because a typed motor exists. A body with no prior
            // vocal effector remains truthfully unmounted.
            None => None,
        };
        if !retired.is_empty() {
            self = self
                .retire_fabric_contact_pairs(&retired)?
                .ok_or(FormationError::NeuronLineageAuthorityChanged)?;
        }
        self.vocal_articulatory_effector_lineage = target;
        self.topology_index = Arc::new(ResidentTopologyIndex::build(
            &self.cohorts,
            &self.electrical_fabric,
        )?);
    ...
    /// Replace the malformed generic layer-12 cells historically specialized
    /// as root-translation motors. Their arbitrary declared geometry was not
    /// checked against the paired body regulator; on the live body the
    /// 37,942-pF regulator was connected to a 178-pF motor, so even one
    /// carrier transfer would increase electrostatic energy and was lawfully
    /// refused forever. A translation motor is fixed body anatomy and belongs
    /// at the compact terminal-matched layer-12 place declared above.
    ///
    /// This correction is deliberately refuse-not-guess. It replaces only an
    /// electrically neutral singleton motor with exactly one virgin regulation
    /// contact. Any charged or multiply connected motor refuses migration.
    /// Evidence/frontier records that name the malformed cell are retired with
    /// it; they cannot be preserved as authority for anatomy that never could
    /// conduct. Unrelated formations and tissue remain byte-for-byte resident.
    /// The old lineage and claimed resting place leave together; the exact
    /// new place is claimed through the existing developmental population and
    /// the sole regulation contact is mounted normally. Re-running this on an
    /// already corrected body is the identity.
    fn into_compatible_root_translation_motors(mut self) -> Result<Self, FormationError> {
        #[derive(Clone, Copy)]
        struct Repair {
            old_lineage: [u8; 16],
        ...
        for (motor_flat, (cohort_index, neuron_index, motor_lineage)) in
            self.topology_index.flat_locations.iter().copied().enumerate()
        {
            let cohort = &self.cohorts[cohort_index];
            let mount = &cohort.anatomy.mounts()[neuron_index];
            let Some(terminal) = mount.root_translation_effector_terminal() else {
                continue;
            };
            ...
            if mount.place() == expected_place {
                continue;
            }
            if cohort.anatomy.neuron_count() != 1
                || cohort.anatomy.contact_count() != 0
                || neuron_index != 0
            {
                return Err(FormationError::NeuronLineageAuthorityChanged);
            }
            if cohort.state.neurons()[0].separated_elementary_charges() != 0 {
                return Err(FormationError::NeuronLineageAuthorityChanged);
            }
            if self
                .topology_index
                .flat_locations
                .iter()
                .any(|(other_cohort, other_neuron, _)| {
                    self.cohorts[*other_cohort].anatomy.mounts()[*other_neuron].place()
                        == expected_place
                })
            {
                return Err(FormationError::NeuronLineageAuthorityChanged);
        ...
        let retired_lineages = repairs
            .iter()
            .map(|repair| repair.old_lineage)
            .collect::<Vec<_>>();
        let retired = retired_lineages.iter().copied().collect::<BTreeSet<_>>();
        let references_retired = |entry: &ActiveElectricalFrontierEntry| {
            retired.contains(&entry.receiver())
                || entry.sender().as_ref().is_some_and(|lineage| retired.contains(lineage))
        };
        self.active_electrical_frontier = self
            .active_electrical_frontier
            .iter()
            .copied()
            .filter(|entry| !references_retired(entry))
            .collect::<Vec<_>>()
            .into_boxed_slice();
        self.preceding_active_electrical_frontier = self
            .preceding_active_electrical_frontier
            .iter()
            .copied()
            .filter(|entry| !references_retired(entry))
            .collect::<Vec<_>>()
            .into_boxed_slice();
        self.older_active_electrical_frontier = self
            .older_active_electrical_frontier
            .iter()
            .copied()
            .filter(|entry| !references_retired(entry))
            ...
            .filter(|retained| {
                !retained
                    ...
                    && retained
                        .recurrent_lineage
                        .as_ref()
                        .is_none_or(|lineage| !retired.contains(lineage))
                    && !retained
                        .mosaic
                        .original_bonds()
                        .iter()
                        ...
                        .any(|bond| {
                            let (left, right) = bond.endpoints();
                            retired.contains(&left) || retired.contains(&right)
                        })
            })
            .cloned()
            .collect::<Vec<_>>()
        ...
        for repair in repairs {
            if let Some(population) = self.resting_population.as_ref() {
                if population
                    .materialized_lineage_ordinal(repair.old_place)
                    .is_some()
                {
                    self.resting_population = Some(
                        population
                            .release_claimed_place(repair.old_place)
                            .map_err(
            ...
            let motor_place = DeclaredNeuronPlace::new(
                12,
                ROOT_TRANSLATION_LAYER12_TOPOLOGY_OFFSET
                    .checked_add(repair.terminal.ordinal())
                    .ok_or(FormationError::ArithmeticOverflow)?,
            );
            let motor_lineage = mount_intrinsic_neuron_at_place(
                &mut cohorts,
                &mut self.resting_population,
                &mut self.next_lineage_ordinal,
                motor_place,
    ...
    pub(crate) fn prepare_vestibular_transition(
        ...
        self.prepare_vestibular_transition_with_residency(ingress, max_encoded_bytes, &mut None)
    }

    pub(crate) fn prepare_vestibular_transition_with_residency(
        &self,
        ingress: &ResidentVestibularIngress,
        max_encoded_bytes: usize,
        residency: &mut Option<crate::causal_event_scheduler::CausalEventResidency>,
    ...
    fn prepare_typed_admitted_transition_from_owned(
        ...
        let Self {
            ...
            vocal_articulatory_effector_lineage: predecessor_vocal_articulatory_effector_lineage,
        ...
        let exact_moved_body_effectors_by_occurrence = admitted_sources
            .iter()
            .flat_map(|admitted| {
                let source = admitted.episode();
                source.joint_source_occurrences().iter().map(move |occurrence| {
                    let mut terminals = occurrence
                        .port_indices
                        .iter()
                        .map(|index| {
                            source
            ...
            .collect::<Result<Vec<_>, FormationError>>()?;
        if exact_moved_body_effectors_by_occurrence.len() != admitted_occurrence_count {
            return Err(FormationError::NoncanonicalState);
        }
        let mut exact_moved_body_effectors = exact_moved_body_effectors_by_occurrence
            .iter()
            .flatten()
            .copied()
            .collect::<Vec<_>>();
        exact_moved_body_effectors.sort_unstable();
        exact_moved_body_effectors.dedup();
        ...
        let settled_layer_six_lineages = internal_contact
            ...
            .collect::<BTreeSet<_>>();
        let exact_moved_body_regulations_by_occurrence =
        ...
        // Cross-sensory anatomy grows only for an occurrence whose exact body
        // evidence resolves to a moved regulation and whose layer-6 cells
        // actually settle through the causal electrical frontier. Coexisting
        // sensory context may join that body-owned occurrence, but a sensory-
        ...
        let root_translation_continuations = exact_reached_root_translation_regulations(
            &cohorts,
            &topology_index,
            &externally_energized_neuron_lineages,
            &reached_body_regulation_lineages,
        ...
        Ok(PreparedCognitiveFormationTransition {
            ...
            observation: CognitiveFormationObservation {
                ...
                root_translation_unit_recruitments:
                    internal_contact.root_translation_unit_recruitments,
    ...
    pub(crate) fn encode(&self, max_encoded_bytes: usize) -> Result<Vec<u8>, FormationError> {
        self.encode_current(max_encoded_bytes, false, true)
    ...
    pub(crate) fn seal_with_terminal_observation(
        ...
        let (encoded, terminal) = self.encode_current(max_encoded_bytes, true, true)?;
    ...
    > {
        ...
        match self.vocal_articulatory_effector_lineage {
            Some(lineage) => {
                output.push(1);
    ...
    fn decode_with_canonicality(
        ...
        let current_v38 = current_v39
            || (bytes.len() >= MAGIC_V38.len() && &bytes[..MAGIC_V38.len()] == MAGIC_V38);
        let current_v37 = current_v38
            || (bytes.len() >= MAGIC_V37.len() && &bytes[..MAGIC_V37.len()] == MAGIC_V37);
        let current_v36 = current_v37
        ...
        let vocal_articulatory_effector_lineage = if current_v40 {
            match *bytes.get(cursor).ok_or(FormationError::NoncanonicalState)? {
                0 => {
                    cursor += 1;
                    None
                }
        ...
        if format == CognitiveCodecFormat::V26 {
            ...
            if current_v41 {
                ...
                validate_v39_passive_vocal_articulatory_route(
                    &state.cohorts,
                    &state.electrical_fabric,
                )?;
            } else if current_v34 {
                validate_fixed_vocal_articulatory_route(
                    &state.cohorts,
                    &state.electrical_fabric,
                )?;
            }
    ...
    pub(crate) fn migrate_to_current_format(
        ...
        let current_v36 = current_v37 || bytes.get(..MAGIC_V36.len()) == Some(MAGIC_V36);
        ...
        // The root-translation terminal was introduced after the current
        // marker and therefore cannot rely on a later magic byte to repair
        // its malformed generic motor cell. The correction is structural and
        // idempotent: a compatible fixed motor is the identity; only an
        // untouched incompatible terminal is replaced. Every restore runs
        // this check, so an older CURRENT/predecessor cannot resurrect it.
        let state = state.into_compatible_root_translation_motors()?;
        let state = if current_v37 {
            state
        } else {
            match state.retire_unproved_vocal_learning_contacts()? {
                Some(corrected) => corrected,
                None => state,
            }
        };
        let state = if current_v38 {
            state
        } else {
            // V37's returned-sound gate still allowed a general whole-tract
            // consequence to connect one ordering cell to every vocal motor
            // that happened to fire.  Per-edge provenance is not resident, so
            // those learned vocal contacts are retired once.  The corrected
            // ordinary growth law below can recreate only exact-axis routes.
            match state.retire_unproved_vocal_learning_contacts()? {
                Some(corrected) => corrected,
                None => state,
            }
        };
        let state = if current_v40 {
            state
        } else {
...
fn local_integration_place(
    ...
    let root_translation_start =
        u32::try_from(ROOT_TRANSLATION_PROPRIOCEPTOR_TOPOLOGY_OFFSET)
            .map_err(|_| FormationError::ArithmeticOverflow)?;
    let root_translation_end = root_translation_start
        .checked_add(
            u32::try_from(ROOT_TRANSLATION_TERMINAL_COUNT)
                .map_err(|_| FormationError::ArithmeticOverflow)?,
        )
        .ok_or(FormationError::ArithmeticOverflow)?;
    if receptor_place.layer() == u32::from(PhysicalSourceSense::Body.declared_layer())
        && (root_translation_start..root_translation_end)
            .contains(&receptor_place.topology_index())
    {
        let terminal_ordinal = receptor_place
            .topology_index()
            .checked_sub(root_translation_start)
            .ok_or(FormationError::ArithmeticOverflow)?;
        return Ok(DeclaredNeuronPlace::new(
            6,
            ROOT_TRANSLATION_LAYER6_TOPOLOGY_OFFSET
                .checked_add(terminal_ordinal)
                .ok_or(FormationError::ArithmeticOverflow)?,
        ));
    }
...
/// carried at other sensory/body layers. A sensory-only sibling cannot found
/// an association, and a sibling occurrence on the same layer cannot be
/// collapsed into the body occurrence. At least three assembly integrations
/// must actually change through the causal frontier across at least two real
/// layers before growth is authorized. Labels, source order, and unrelated
/// reached cells have no authority. The sorted integration lineage set is the
/// assembly; a newly mounted resting cell is never seeded directly.
fn mount_reached_cross_sensory_association(
    ...
    let mut energized_integrations_by_occurrence =
        Vec::<Vec<([u8; 16], u32)>>::with_capacity(externally_energized_by_occurrence.len());
    for occurrence in externally_energized_by_occurrence {
        ...
        energized_integrations_by_occurrence.push(energized_integrations);
    }
    let candidate_assemblies_by_occurrence = energized_integrations_by_occurrence
        .iter()
        .zip(moved_body_regulations_by_occurrence)
        .map(|(local_integrations, moved_body_regulations)| {
            if local_integrations.is_empty() || moved_body_regulations.is_empty() {
                return None;
            }
            let local_layers = local_integrations
                .iter()
                .map(|(_, layer)| *layer)
                .collect::<BTreeSet<_>>();
            let mut assembly = local_integrations.clone();
            assembly.extend(
                energized_integrations_by_occurrence
                    .iter()
                    .flatten()
                    .copied()
                    .filter(|(_, layer)| !local_layers.contains(layer)),
            );
            assembly.sort_unstable();
            assembly.dedup();
            let settled = assembly
                .iter()
                .copied()
                .filter(|(lineage, _)| settled_layer_six_lineages.contains(lineage))
                .collect::<Vec<_>>();
            let settled_layer_count = settled
                .iter()
                .map(|(_, layer)| *layer)
                .collect::<BTreeSet<_>>()
                .len();
            (settled.len() >= 3 && settled_layer_count >= 2).then(|| {
                assembly
                    .into_iter()
                    .map(|(lineage, _)| lineage)
                    .collect::<Vec<_>>()
    ...
    for (receptor_lineage, receptor_place, effector_terminals) in reached_body_receptors {
        ...
        for terminal in effector_terminals {
            match terminal {
                ...
                DevelopedMotorTerminal::RootTranslation(terminal) => {
                    mount_fixed_root_translation_motor_terminal(
                        cohorts,
                        resting_population,
                        next_lineage_ordinal,
                        terminal,
...
fn mount_fixed_articulated_motor_terminal(
    ...
    if terminal.axis().is_vocal_articulator() {
        ensure_vocal_articulatory_effector(
            cohorts,
            resting_population,
            next_lineage_ordinal,
            electrical_fabric,
...
/// Return the one resident respiratory/articulatory effector, mounting it from
/// the compact resting anatomy only when none exists.  This function authors
/// no contact.  Learned direct ordering anatomy is mounted separately from an
/// exact returned vocal-axis route.
fn ensure_vocal_articulatory_effector(
    cohorts: &mut Vec<ResidentReachedCohort>,
    resting_population: &mut Option<DevelopmentalRestingPopulation>,
    next_lineage_ordinal: &mut u64,
    electrical_fabric: &ResidentElectricalFabric,
) -> Result<[u8; 16], FormationError> {
    let contacted = electrical_fabric
        .contact_endpoints()
        .flat_map(|(left, right)| {
            [
                electrical_fabric.lineages()[left],
    ...
    let mut candidates = cohorts
        ...
        .filter_map(|((mount, lineage), neuron)| {
            (mount.source_site().is_none()
                && mount.place().layer() == 13
                && (contacted.contains(lineage)
                    || neuron.separated_elementary_charges() == 0))
            .then_some((
                contacted.contains(lineage),
                mount.place().topology_index(),
                *lineage,
            ))
        })
        .collect::<Vec<_>>();
    candidates.sort_unstable_by_key(|(is_contacted, topology, _)| (!*is_contacted, *topology));
    if let Some((_, _, lineage)) = candidates.first() {
        Ok(*lineage)
    } else {
...
/// Mount only the organism's fixed respiratory/articulatory effector.
///
/// Vocal antagonist terminals are fixed layer-12 body anatomy, but their mere
/// existence is not authority to excite breath. The old constructor joined
/// every such motor to one layer-13 cell and recreated the rejected shared hub
/// for genesis and old-format migration. Current anatomy mounts the layer-13
/// effector without contacts. One sparse direct layer-11 route may be added
/// later only alongside an exact learned route to a typed vocal motor.
fn mount_fixed_vocal_articulatory_route(
    cohorts: &mut Vec<ResidentReachedCohort>,
    resting_population: &mut Option<DevelopmentalRestingPopulation>,
    next_lineage_ordinal: &mut u64,
    electrical_fabric: &mut ResidentElectricalFabric,
...
fn mount_fixed_root_translation_motor_terminal(
    ...
    let motor_lineage = match matching.as_slice() {
        ...
        [] => {
            ...
            motor_cohort
                .anatomy
                .specialize_root_translation_effector(lineage, terminal)
                .map_err(FormationError::PhysicalSettlementUnavailable)?;
            lineage
        }
        _ => return Err(FormationError::NeuronLineageAuthorityChanged),
...
/// Resolve the same-direction position regulation for every exact motor
/// terminal that the authenticated articulated-body consequence proves moved.
/// A successful unopposed movement has zero reacted load, so its quiet load
/// ending cannot be movement authority. Both position endings sense the
/// displacement; their fixed typed direction and the exact moved-effector set
/// select one local regulation. That regulation already owns its fixed L12
/// motor contact through ordinary body development.
fn exact_moved_body_regulations_by_occurrence(
    cohorts: &[ResidentReachedCohort],
    topology: &ResidentTopologyIndex,
    body_regulations: &[Vec<[u8; 16]>],
    moved_effectors: &[Vec<BodyEffectorTerminal>],
) -> Result<Vec<Vec<[u8; 16]>>, FormationError> {
    if body_regulations.len() != moved_effectors.len() {
        return Err(FormationError::NoncanonicalState);
    }
    ...
    let position_effector_for_regulation = |regulation: [u8; 16]| {
        ...
        let mut effectors = topology.neighbours_by_flat[integration_flat]
            ...
            .filter_map(|flat| {
                ...
                (source_site.physical_quantity() == ANTAGONIST_PROPRIOCEPTOR_LENGTH_QUANTITY)
    ...
    body_regulations
        .iter()
        .zip(moved_effectors)
        .map(|(regulations, moved)| {
            let moved = moved.iter().copied().collect::<BTreeSet<_>>();
            let mut exact_by_terminal = BTreeMap::new();
            for regulation in regulations.iter().copied() {
                let Some(terminal) = position_effector_for_regulation(regulation)? else {
                    continue;
                };
                if !moved.contains(&terminal) {
                    continue;
                }
                if exact_by_terminal.insert(terminal, regulation).is_some() {
                    return Err(FormationError::NeuronLineageAuthorityChanged);
                }
            }
            let mut exact = exact_by_terminal.into_values().collect::<Vec<_>>();
            exact.sort_unstable();
            Ok(exact)
        })
        .collect()
...
/// Return only association/body-regulation pairs carried by one exact admitted
/// physical occurrence. Both endpoints must transfer whole carriers in that
/// occurrence. Unrelated resident activity, static pose, retained history,
/// separate occurrences, and whole-frontier coincidence have no authority.
...
fn mount_reached_ordering_reach(
    ...
    let exact_motor_for_affective = |association: [u8; 16], affective: [u8; 16]| -> Result<Option<[u8; 16]>, FormationError> {
    ...
    for participants in active_routes {
        ...
        // first mounts its ordering cell and when the same resident ordering
        // cell is reached again after cold restore. Restricting this join to
...
enum DevelopedMotorTerminal {
    ...
    RootTranslation(RootTranslationEffectorTerminal),
}
...
/// Resolve the exact reached root-translation regulation for each directional
/// x/y proprioceptor whose physical value changed in this occurrence. This is
/// the translation analogue of root yaw and uses the same receptor -> local
/// integration -> body regulation anatomy without introducing a destination.
fn exact_reached_root_translation_regulations(
    cohorts: &[ResidentReachedCohort],
    topology_index: &ResidentTopologyIndex,
    externally_energized_lineages: &[[u8; 16]],
    reached_body_regulation_lineages: &[[u8; 16]],
) -> Result<BTreeMap<[u8; 16], Vec<RootTranslationEffectorTerminal>>, FormationError> {
    let mount_for = |lineage: [u8; 16]| {
        let flat = topology_index.flat_for_lineage(lineage)?;
        let (cohort_index, neuron_index, _) = topology_index.flat_locations[flat];
        cohorts
    ...
    let mut continuations =
        BTreeMap::<[u8; 16], Vec<RootTranslationEffectorTerminal>>::new();
    for receptor in externally_energized_lineages.iter().copied() {
        let receptor_mount = mount_for(receptor)?;
        let Some(terminal) = receptor_mount
            .source_site()
            .and_then(NeuronSourceSite::root_translation_proprioceptor_terminal)
            .map(RootTranslationProprioceptorTerminal::paired_effector)
        else {
            continue;
        };
        let integration_place = local_integration_place(receptor_mount.place())?;
        let regulation_place = body_regulation_place(receptor_mount.place(), integration_place)?;
        let Some(regulation) = regulation_by_place
            .get(&(regulation_place.layer(), regulation_place.topology_index()))
            .copied()
        else {
            continue;
        };
        ...
        continuations.entry(regulation).or_default().push(terminal);
    }
    for terminals in continuations.values_mut() {
        terminals.sort_unstable();
        terminals.dedup();
...
fn mount_reached_motor_effector(
    ...
    let moved_effectors = _moved_axes
        .iter()
        .copied()
        .flat_map(|axis| {
            [
...
fn mount_reached_motor_effector_with_reach_index(
    ...
    let lateral_at_growth_boundary = lateral_by_ordering.clone();
    ...
    for entry in predecessor_frontier
        ...
        .chain(older_predecessor_frontier)
    {
    ...
    for regulation in root_translation_continuations.keys().copied() {
        if !body_regulation.contains(&regulation) {
            body_regulation.push(regulation);
        }
    }
    ...
    for (candidate, mount) in &mounted {
        ...
        let terminal = match (
            ...
            (Some(terminal), None, None) => DevelopedMotorTerminal::Articulated(terminal),
            (None, Some(terminal), None) => DevelopedMotorTerminal::RootYaw(terminal),
            (None, None, Some(terminal)) => DevelopedMotorTerminal::RootTranslation(terminal),
    ...
    for regulation in body_regulation {
        ...
        terminals.sort_unstable();
        terminals.dedup();
        terminals.retain(|terminal| match terminal {
            DevelopedMotorTerminal::Articulated(terminal) => {
                moved_effectors.binary_search(terminal).is_ok()
            }
            DevelopedMotorTerminal::RootYaw(_) | DevelopedMotorTerminal::RootTranslation(_) => true,
        });
        let articulatory_consequence = neighbours_by_lineage
            ...
            .any(|source_site| {
                source_site.sense() == PhysicalSourceSense::Body
                    ...
                    && source_site.root_translation_proprioceptor_terminal().is_none()
                    && matches!(
        ...
        let proven_affective = regulation_neighbours
            ...
            .collect::<BTreeSet<_>>();
        let mut causal_path_ordering = ordering
            .iter()
            .copied()
            .filter(|lineage| {
                ...
                let indirect = |affective_frontier: &[ActiveElectricalFrontierEntry],
                                ordering_frontier: &[ActiveElectricalFrontierEntry]| {
                    affective_frontier.iter().any(|affective_entry| {
                        ...
                        ordering_frontier.iter().any(|ordering_entry| {
                ...
                direct
                    || indirect(predecessor_frontier, preceding_predecessor_frontier)
                    || indirect(preceding_predecessor_frontier, older_predecessor_frontier)
            })
            .collect::<Vec<_>>();
        causal_path_ordering.sort_unstable();
        causal_path_ordering.dedup();
        ...
        // authority.  Only one exact proprioceptive terminal below can name a
        // learned motor route; returned sound remains ordinary sensory evidence
                                ...
                                anchor.direction() == target.direction()
                                    ...
                                    && current_motor_recruitments.iter().any(|event| {
                                        event.neuron_lineage == *motor
                                            ...
                                            && event.learned_work_preparations.iter().any(
                                                |preparation| {
                                                    preparation.routes.iter().any(|route| {
                                                        route.ordering_lineage == *candidate
                                                    })
                                                },
                                            )
                                    })
        ...
        synergy_ordering.sort_unstable();
        synergy_ordering.dedup();
        // A co-moving learned anchor is the more specific cause of premotor
        // population growth.  It must not erase the independent ordering path
        // carried by the lived sensory/body episode.  When both are present,
        // ordinary local coactivity grows a sparse L11-to-L11 contact from the
        // current-experience ordering cell into the active premotor family.
        // That contact lets a later partial cue reach the already learned
        // population without adding another L11-to-L12 motor source.  The
        // ordinary path remains the direct motor bootstrap only when no
        // learned co-moving anchor exists yet.
        let using_synergy = !synergy_ordering.is_empty();
        let mut proven_ordering = if !using_synergy {
            causal_path_ordering.clone()
        } else {
            synergy_ordering.clone()
        };
        ...
        let synergy_population = if using_synergy {
            ...
            let family_sources = motors_at_growth_boundary
                .keys()
                .copied()
                .filter(|source| same_family(source))
                .collect::<Vec<_>>();
            let maximum_family_degree = family_sources
                .iter()
                .map(|source| {
                    lateral_at_growth_boundary
                        .get(source)
                        .into_iter()
                        .flatten()
                        .filter(|neighbour| same_family(neighbour))
                        .count()
                })
                .max()
                .ok_or(FormationError::NeuronLineageAuthorityAbsent)?;
            let family_hubs = family_sources
                .iter()
                .copied()
                .filter(|source| {
                    lateral_at_growth_boundary
                        ...
                        .filter(|neighbour| same_family(neighbour))
                        .count()
                        == maximum_family_degree
                })
                .collect::<Vec<_>>();
            let [family_hub] = family_hubs.as_slice() else {
                continue;
            };
            proven_ordering.clear();
            proven_ordering.push(*family_hub);
            Some((family, *family_hub))
        } else {
            None
        };
        ...
        let motor_lineage = match matching.as_slice() {
            ...
            [] if matches!(
                effector_terminal,
                DevelopedMotorTerminal::RootYaw(_) | DevelopedMotorTerminal::RootTranslation(_)
            ) => {
                let lineage = mount_next_intrinsic_in_layer(
                    cohorts,
                    resting_population,
            ...
            [] => match effector_terminal {
                ...
                DevelopedMotorTerminal::RootTranslation(_) => unreachable!(),
            },
            _ => return Err(FormationError::NeuronLineageAuthorityChanged),
        };
        let motor_cohort = cohorts
        ...
        match effector_terminal {
            ...
            DevelopedMotorTerminal::RootTranslation(terminal) => motor_cohort
                .anatomy
                .specialize_root_translation_effector(motor_lineage, terminal),
        }
        .map_err(FormationError::PhysicalSettlementUnavailable)?;
        ...
        if let Some((family, _)) = synergy_population {
            let same_family = |source: &[u8; 16]| {
                let mut founding = founding_by_ordering
                    .get(source)
                    .into_iter()
...
/// the motor. A retained layer-11 ordering cell is itself the resident cause:
/// requiring it to be relabelled as fresh external or frontier input in the
/// same interval would prevent an internally retained thought from acting.
/// Layer-8 reflex preparation remains tied to this occurrence's exact reached
/// regulation. Motor-to-neighbour flow is a consequence, never preparation.
...
fn exact_motor_preparation_transfers(
    ...
    fresh_seed_lineages: &BTreeSet<[u8; 16]>,
...
/// external same-interval label. Paired layer-8 regulation remains tied to a
/// current causal body seed.
...
fn settle_internal_contact_interval(
    ...
    // carrier as a motor carrier. A real transfer across the L11 cell's exact
    // founding L7 route releases exact work. Its downhill direction comes
    // from the two neurons' physical predecessor state. The source path and every
            ...
            ordering_motor_bridges.iter().copied()
        {
            ...
            let source_candidates = settled
                ...
                .filter_map(|(position, (transition, (left_flat, right_flat)))| {
                    ...
                    let founding_flat = if left_flat == ordering_flat {
                        right_flat
                    } else if right_flat == ordering_flat {
                        left_flat
                    } else {
                        return None;
                    };
                    if layer_of(flat_locations[founding_flat].2) != Some(7) {
                        return None;
                    }
                    let ordering_is_seed = is_causal_seed(ordering_flat);
                    let founding_is_seed = is_causal_seed(founding_flat);
                    if ordering_is_seed == founding_is_seed {
                        return None;
                    }
                    let causal_seed = if ordering_is_seed {
                        ordering_flat
                    } else {
                        founding_flat
                    };
    ...
    let cohort_results = cohorts
        ...
        .map(
            ...
            Option<(
                ...
                Vec<RootTranslationUnitRecruitment>,
        ...
        let root_yaw_unit_recruitments = selected_members
            ...
            .collect::<Vec<_>>();
        let root_translation_unit_recruitments = selected_members
            .iter()
            .filter_map(|(_, neuron_index)| {
                let mount = &cohort.anatomy.mounts()[*neuron_index];
                ...
                (mount.source_site().is_none() && mount.place().layer() == 12).then_some(
                    RootTranslationUnitRecruitment {
                        neuron_lineage: motor_lineage,
                        topology_index: mount.place().topology_index(),
                        outward_elementary_carriers,
                        terminal,
    ...
    for result in cohort_results {
        if let Some((
            ...
            cohort_root_translation_recruitments,
        ...
        )) = result?
        {
            ...
            root_translation_unit_recruitments.extend(cohort_root_translation_recruitments);
    ...
    // A learned vocal motor discharge co-recruits the one resident
    ...
    // how much of the layer-13 cell's own retained membrane-gradient work may
    ...
    // Contacts with both or neither endpoint seeded do not advance this
    // cognitive boundary. Scheduled carrier crossings remain exact physical
    // settlement and wake changed endpoints' incident contacts below, but the
    // maintenance schedule has no authority to manufacture cognition, action,
    // learning, or a new causal frontier.
        ...
        .zip(compact_edge_flat_endpoints.iter().copied())
    {
        ...
        if let Some((seed_flat, reached_flat)) = causal_frontier_crossing(
            left_flat,
            right_flat,
            left_is_seed,
            right_is_seed,
        ) {
            let frontier_lineage = flat_locations[reached_flat].2;
