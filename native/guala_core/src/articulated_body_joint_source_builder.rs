...
/// The caller separately admits decoded working memory; this limit bounds the
/// encoded expansion before sample-sized allocation. No motor receipt is made.
pub(crate) fn admit_passive_body_trajectory_source(
    ...
    let bound = headers.iter().try_fold(output.len() + 4, |total, header| {
        ...
        .ok_or(ArticulatedBodyJointSourceError::ArithmeticWidth)?;
    if bound > max_encoded_bytes {
        return Err(ArticulatedBodyJointSourceError::ResourceUnavailable);
    }
