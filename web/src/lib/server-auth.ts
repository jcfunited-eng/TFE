...
export async function getCurrentClerkUser(): Promise<UserSummary | null> {
  return getUserSummaryFromClerk();
}
...
export async function requireServerClerkAdminUser(
  currentPath: string,
): Promise<UserSummary> {
  const user = await getUserSummaryFromClerk();
  if (!user) {
    const safeNext = sanitizeNextPath(currentPath);
    redirect(`/sign-in?next=${encodeURIComponent(safeNext)}`);
  }
