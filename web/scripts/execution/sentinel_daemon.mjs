...
async function submitEntry(signal, executeFn, channelTag, budget, sameDayExitTickers) {
  ...
  if (String(channelTag ?? "").toUpperCase().includes("CH2")) {
    ...
    // cooling-off: a stock the law just sold is not re-entered on the same
    // class of signal for 14 calendar days (~10 sessions). Joseph 2026-09-02
    // set this for losing stop-outs (no averaging down into our own
    // rejects); Joseph 2026-09-15 restart: it covers every exit the law
    // makes — a reading that called the drive dying, the dead clock, the
    // brake, the ratchet — because the frozen-readings churn re-bought BDX,
    // VALE and CRVL the morning after the reading banked them, at higher
    // prices. A book reset ordered by Joseph (manual_reset_*) is not a
    // verdict on the stock and does not cool anything off.
    const cooled = await pool.query(
      `SELECT id, exit_reason FROM personal_trade_ledger
       WHERE UPPER(TRIM(ticker)) = $1 AND signal_class = 'CH2'
    ...
    if (cooled.rows.length > 0) {
      console.log(`[DAILY-ENTRY] ${ticker} excluded: cooling off after the law sold it (${cooled.rows[0].exit_reason ?? "exit"}, 14 days)`);
