...
const ARTIFACT_ID = "tuple_proximity_v1";
const ARTIFACT_DESCRIPTION = "30-nearest-neighbor WR in 7-dim per-stock-normalized tuple space";
const ARTIFACT_HASH = createHash("sha256").update(ARTIFACT_DESCRIPTION).digest("hex");
...
// ---------------------------------------------------------------------------
// Decision label is set by the tuple-proximity engine in
// sync_runtime_postgres_impl.mjs. This module provides provenance scaffolding.
// ---------------------------------------------------------------------------

export function deriveTypedFallbackLadderLevel({ decisionReasonCode }) {
  const reasonCode = String(decisionReasonCode ?? "").trim();
  if (reasonCode === "TUPLE_PROXIMITY_MISSING_FIELDS") {
...
export function computeDecisionTrace(
  ...
  const inputComplete = basis.missing_fields.length === 0;

  // Decision label is set by the tuple-proximity engine in
  // sync_runtime_postgres_impl.mjs. Default to Hold here;
  // the sync module overwrites with the actual decision.
  const decisionReasonCode = inputComplete
    ? "TUPLE_PROXIMITY_DEFERRED"
    : "TUPLE_PROXIMITY_MISSING_FIELDS";

  return {
    basis,
    anomalyFlags: { primitive_input_complete: inputComplete, anomaly_any: false },
    candidateKeyChain: [ARTIFACT_ID, decisionReasonCode],
    matchedKey: inputComplete ? ARTIFACT_ID : null,
    matchedIndex: inputComplete ? 0 : -1,
    matchLevel: inputComplete ? "L0_TUPLE_PROXIMITY" : "L4_INPUT_MISSING",
    fallbackLadderLevel: deriveTypedFallbackLadderLevel({ decisionReasonCode }),
    matchedExactBool: inputComplete,
    fallbackUsed: !inputComplete,
    fallbackReasonCode: inputComplete ? null : `missing_required_fields:${basis.missing_fields.join(",")}`,
    decision: "Hold",
    decisionReasonCode,
    coverageClass: inputComplete ? "exact" : "fallback",
  };
}
...
export function loadPolicyRuntimeArtifact(rootDir) {
  ...
  return {
    ...
    policy_artifact_hash_sha256: ARTIFACT_HASH,
    policy_source_mode: "tuple_proximity",
...
export function buildPersistedProvenanceRecord({
  ...
  const effectiveWriterBuildHash =
    String(writerBuildHash ?? "").trim() || sha256Hex(`${writerComponent}:${provenanceSchemaVersion}:${ARTIFACT_ID}`);
  ...
  const record = {
    ...
    policy_artifact_id: String(runtimeArtifact.policy_artifact_id ?? ARTIFACT_ID),
