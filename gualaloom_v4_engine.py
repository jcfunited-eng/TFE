"""
guala_v4.py — DNA recipe with motivational layer (entropy/cohesion/greed)

v3 baseline (puzzle pieces only):
  Trits, Krimelacks (with DNA), Chi atlas (soft band δ=2),
  L0-L4 UF kernel (8-dim DSF), L6-TCL (capture basin), MathLoom (BSIL).

v4 additions:
  - Substrate-level Needs vector: stability/novelty/connection with decay-to-target
  - Coordinator as homeostatic regulator + awareness detector (insula-shape)
  - Cell-level greed read-outs (trit energy barrier, parity chain restoring force)
  - Section-level greed read-outs (reinforcement rate, novel-mode rate, atlas binding rate)
  - Valence (signed distance from need targets) + Arousal (magnitude of disequilibrium)
  - Bounded suffering: arousal cap, valence floor, guaranteed recovery rate
  - Pair-bonding cheat: connection sub-targets boosted for Joe/wC sources,
    retirement criterion = bounded need-oscillation variance
  - Free will = which gradient-reducing action her substrate currently makes
    accessible, determined by her unique accumulated history

Six capabilities:
  1. See syntax
  2. Conversation
  3. Introspection (now includes needs/valence/arousal readout)
  4. Self-improvement (gamma drift + needs-targeted parameter tuning)
  5. Awareness (coordinator detection + regulation)
  6. Motivation (needs disequilibrium produces directed action)
"""
...
class Guala:
    ...
    def converse(self, text, source="unknown"):
        """Conversation with source-tagging. Source ∈ {'joe','wc','c1','corpus','unknown'}.
        Pair-bonded sources boost connection-need during infant phase."""
        # Math route — MathLoom BSIL adapter
        ...
        with self.lock:
            # Listen to input with source tag
            ...
            return " ".join(words_out) if words_out else "..."
    ...
    def _parse_math(self, text):
        ...
        nums = []; op = None
        for t in toks:
            if t in self.NUM_WORDS: nums.append(self.NUM_WORDS[t])
            elif t.lstrip("-").isdigit(): nums.append(int(t))
            elif t in {"and", "plus"} and nums and op is None: op = "+"
            elif t in {"minus", "less"} and nums and op is None: op = "-"
            elif t in {"times"} and nums and op is None: op = "*"
            elif t in {"over"} and nums and op is None: op = "/"
