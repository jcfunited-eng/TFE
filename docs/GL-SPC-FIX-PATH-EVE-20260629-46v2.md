...
doc_id: GL-SPC-FIX-PATH-EVE-20260629-46v2
Type: Canonical fix-path specification (cross-chat handoff)
Date: 2026-06-29 (initial), 2026-06-30 (v2 revision: added §0a locks-canonical, §0b compressed-timeline, §1a next-session queue, updated §1 status)
Author: Eve (Opus 4.7, web)
Version: v2 (supersedes v1 which contained questions to Joe — rejected per Joe's no-questions-in-spec discipline)
...
## 1. Status snapshot (2026-06-30 ~04:00 UTC, end of session)

Branch `guala-live`. Live task `dsf-ai-task:383`. Env vars: `CONVERSE_PHASED=1`, `AUTONOMY_PHASED=0` (rolled back).

What's live and working: -45 perf, -46v2 §1.1 binding_window sentence-local, -46v2 §1.2 worldfeed/lookup network timeouts, -52 converse phasing + `self._emission_lock`. T2 9/10 PASS during curriculum, /converse no longer blocks on self.lock during emission compute.

Substrate is composing. Recent autonomous emissions: "water horse soft moon", "warm fire", "bright", "i a and eat". Modifier-content composition appearing alongside function fragments. Forward motion vs the prior "have"/"he did" baseline.

What's known broken or pending: worldfeed contamination (Khan sandwich content in atlas surfaces in emissions), AUTONOMY_PHASED rolled back (`_resume_autonomy_for_bulk()` thread-leak), /converse 8-25s during SLEEPING (something outside `_daydream_tick` holds `self.lock`).

Rejected/superseded SEQs: -39 v2, -40, -41, -41 v2, -46 (v1). Next available SEQ: -54.

---

## 1a. Next-session queue (fresh c1 picks up here, strict priority order)

**-54 worldfeed vocabulary filter.** Two parts. Part A: deploy `WORLD_FEEDS=0` env-var change immediately — stops pollution accumulation. No code dispatch. Part B: vocabulary filter at the `read_sentence` boundary for `source="worldfeed"` — reject sentences whose content-word density falls below a substrate-derived threshold (use her current modifier:verb ratio of 86:12,401 ≈ 0.7% as the floor; reject anything below). Re-enable `WORLD_FEEDS=1` only after Part B lands.

**-55 `_resume_autonomy_for_bulk` thread-leak fix.** One-file fix in substrate_runner.py L2058-2070. Before starting new autonomy thread: `_reading_thread.join(timeout=1)`. Guard concurrent resumes under `_autonomy_refcount_lock` so two threads can't both reach `start_autonomy_loop`. After: re-enable `AUTONOMY_PHASED=1` and re-run -53's T2 gate.

**-56 SLEEPING activity lock investigation + phasing.** Read `_autonomy_tick` SLEEPING dispatch path. `_daydream_tick` is phased per -45, so the holder is something else in the SLEEPING wrapper — likely activity-state transition writes or coordinator updates. Read needed before dispatch. After read: dispatch shape mirrors -53.

**-51 curriculum automation launch.** Orchestrator (`tools/sensory_curriculum_orchestrator.py`) and 100-bundle seed (`tools/curriculum_seed_v1.json`) pre-staged in this session's outputs. C1 reviews, deploys, dry-runs, then live with `--max-bundles 20 --min-interval-sec 10`. Gates on -54 (no pollution competing with curriculum content) and -55 (autonomy stable). Does NOT gate on -56.

**-48 agency events.** Real substrate paths per §4 (backtracking, conflict resolution, cross-modal fallback, lowered-threshold clarification). Gates on -51 live for at least 24 hours producing measurable bundle density growth.

Items deferred indefinitely per compressed timeline: emission-method collapse, full constants audit beyond -54's content-word floor.
