# A-013 Native Root Translation — 2026-08-28

## Exact defect

Native cognition could actuate articulated joints and root yaw, but no native
terminal could translate the resident body in the persistent world. The Python
bridge consequently transported only yaw and time. A thought-caused native
discharge could therefore twitch or turn while the world root remained fixed.

## Delivered correction

- Four fixed native terminals represent negative/positive X and Y movement.
- Layer-12 developmental mounting binds each terminal to its exact regulation
  lineage; no observer, route metadata, target label, or scripted choice selects
  movement.
- Exact carrier discharge becomes signed integer millimetres at the physical
  world boundary.
- The world applies or physically refuses that displacement.
- Actual before/after displacement, never requested displacement, returns as a
  four-port proprioceptive source to the same resident organism.
- The terminal/source anatomy is cold-persistent and the two new Rust owners are
  explicitly included in the immutable production release manifest.

## Acceptance evidence before cutover

- Candidate native wheel built and installed; the new source symbol is present.
- Focused native root-translation, proprioception, and developmental-mount
  proofs pass.
- Python bridge proof passes for X-positive plus Y-negative movement and exact
  returned displacement.
- Production CURRENT tick 240705, state
  `b724219df0cb2669c84e1b3921f4cb9db53724a9bdcadd813358c260a8b0f884`,
  cold-restored unchanged under the candidate.
- On that copied live body, a one-millimetre X movement changed world position
  `(6703, 4500)` to `(6704, 4500)`, entered native sensory settlement, sealed at
  tick 240711, and cold-restored with identical successor state
  `94fa5579584c016e7d506c744bb7a71470b82bec7bcbffd00e2a07536149b09f`.
- Python cognition callback count remained zero.

## Truth boundary

This delivers the missing native locomotion route and same-organism consequence
return. It is not a claim that production has autonomously chosen and completed
