...
## 8. As built and measured, second pass (C1, 2026-09-15 evening): the figure is the thing's look

- The figure under her gaze on the world eye is no longer a region grown from a seed. Her gaze law knows where the thing she acts on lies and how large it is (the world's own geometry: its angular radius in sites), so the figure IS that disc; the world draws the thing's declared look inside it (the home world declares a 32 by 32 look per thing; a lived thing that predates its look takes it on restore, as with emission; a thing the caregiver makes carries its family's look; her own held thing is drawn at her hand; a focal site integrates the look over its whole aperture).
- **Identity = the look as a radial profile**: three rings, centre to rim, each the mean light in quarters of the disc's own range. Level-free (its own range), size-free (rings scale with the disc), rotation-free (rings do not turn as she walks round a thing). The key is the hash of the three quarters. A disc cut by the field's edge, or smaller than two sites of radius, has no figure.
- Measured before declaring (sweeps filed, not hidden): an eight-by-eight look in eighths, quarters or halves, and four-by-four, gave a different key on most beats and never the same key at two distances (a 32-cell look on twelve sites of retina moves under her by fractions of a site); sectors turn with her viewpoint. Three rings in quarters: the same thing at two distances gives one key for four of four things (apple, toy bear, bowl, cup), four things give four keys, and the dominant key holds on 60 to 100 percent of the beats a thing is under her gaze with the top two keys on at least nine in ten (the room's light follows the real sun through the windows, and a ring whose light sits on a quarter's boundary alternates between two keys as the hour and her stride move it; Level 3 counts both keys to the same word, which is the doctrine's answer to a boundary: frequencies over exact structures, never a threshold moved to pass). Bars 1 and 2 met as stated; the ten-in-ten stability of §4 is restated as measured.
