...
Full sensor-to-motor chain & \textbf{PROVEN} & Sharp $\rightarrow$ XADC $\rightarrow$ BSIL $\rightarrow$ SPPU $\rightarrow$ motor \\
Vision discrimination & \textbf{PROVEN (sim)} & 5 objects, 0.11--0.36 gaps, 4KB/object \\
Camera BSIL encoding & \textbf{PENDING} & PCAM 5C DVP modules written \\
Domain agnosticism & \textbf{PROVEN (sim)} & Financial, IR distance, camera pixels \\
...
Phase 2 (current) & Multi-sensor demo & 3$\times$Sharp + PCAM camera + motor \\
