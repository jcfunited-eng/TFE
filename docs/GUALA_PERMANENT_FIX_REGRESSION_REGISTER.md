...
| R-003 | **Open candidate 2026-08-29.** The eight persisted vocal-tract section areas were immutable parameters rather than body axes, so six airway sections could never be changed by native motor activity. | Duplicating the areas beside the body axes; adding phonemes, word targets, stored waveforms or Python speech authority; moving any of the original 37 axes, 74 terminal addresses, or six root-motion addresses; assigning new motor neurons arbitrary fixed places that may collide with developed resident anatomy. | Keep the body checkpoint exactly 195 bytes; decode V1's eight areas into eight appended axes; retain every old address; append 16 new receptor/load endings after root motion; claim layer-12 motor cells through the existing dormant-anatomy law; require a tract-only drive to change exact proprioception and the resulting physical pressure; then cold-restore the same live identity and prove one-way V2 persistence. |
