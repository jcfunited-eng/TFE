...
| F-007 | Ordinary cognition is not sealed on every interval; the current cadence seals every fourth interval. | Calling the unused snapshot API a mounted custodian, claiming checkpoint work is off-lock, or restoring per-interval whole-body publication. | Inspect the production call graph, not comments: until a bounded custodian is actually called outside `_transition_lock`, record checkpoint sealing/stage/publish as synchronous lock-held work. | **Reconciled Partial 2026-08-29.** Three intervals defer persistence, but the fourth seals and publishes synchronously under `_transition_lock`; no background custodian thread is mounted. |
...
| R-002 | **Open — the same defect has recurred 33 times; reproduced on production tasks 1317 through 1321.** Recovery is physical and native thought recurs, but every observed interval still recruits an action, so no action-free rest interval can occur. Task 1321 proved that a downstream motor predicate alone is insufficient: 127 motors still recruited. | Commit `9228fa81` promoted an unseeded scheduled carrier crossing into `ActiveElectricalFrontierEntry`. That gave the physical maintenance schedule cognitive/action authority and resurrected C-021 attempt 2: background balancing current becomes motor preparation and keeps the organism continuously acting. | Scheduled crossings must remain physical settlement only; changed endpoints continue through the independent incident-contact wake law, but an unseeded scheduled crossing must never enter the cognitive frontier. Ordinary body, root-yaw, and root-translation preparation must additionally require the exact causal frontier. Preserve the typed layer-8 reacted-load reflex and root-yaw's separately retained prepared terminal. Prove unseeded scheduled settlement creates no action authority, seeded action remains possible, and live production shows recovery followed by action-free internal recurrence and a later physical action with returned consequence. |
