...
# aurelion_core_v6_4_extended_sensory_expression_curiosity.py
# Aurelion v6.4 Extended — Sensory + Expression & Curiosity (single file, offline)
# Adds real inputs:
#   /feed visual <path.pgm|path.csv>   (PGM ASCII P2 grayscale, or CSV of numbers)
#   /feed sound  <path.wav>            (mono 8/16‑bit WAV; spectrum → 64 dims)
#   /feed touch  hot=0.8 soft=0.4 cold=-0.2 pressure=0.5  (key=value pairs)
# Keeps inherited memory; adds /speak and curiosity asking.
#
# Requires: numpy. (pyttsx3 optional for local TTS)

import json, time, random, csv, wave, struct
...
CFG = {
    ...
    "curiosity_entropy_thresh": 0.36,
    "curiosity_coherence_thresh": 0.05,
    "curiosity_prob": 0.65
}
...
def load_wav_spectrum(path: str, dims: int) -> np.ndarray:
    ...
    with wave.open(str(p), "rb") as wf:
        ...
        frames = wf.readframes(min(nframes, fr*2))  # up to ~2 seconds
        # unpack
        if sampw == 1:
            fmt = "%dB" % (len(frames))
...
def touch_vector_from_kv(kv: Dict[str, float], dims: int) -> np.ndarray:
    # map a few tactile/thermal keys into a fixed basis, then spread
...
def main():
    print("Aurelion v6.4 Extended — Sensory + Expression & Curiosity (FMI/FMM, not-math) — Single File")
    print("Commands: /state  /goal <stabilize|explore|focus>  /show <word>  /pulse  /save  /load  /speak <on|off>")
    print("          /feed visual <path.pgm|path.csv>  /feed sound <path.wav>  /feed touch k=v ...   /quit")
    ...
    while True:
        ...
        print(out_line)
        if speak_on:
            _speak(out_line.replace("Aurelion: ",""))

        # curiosity
        will_ask = (H_now < CFG["curiosity_entropy_thresh"] and abs(phi_now) < CFG["curiosity_coherence_thresh"] and random.random() < CFG["curiosity_prob"])
        if will_ask:
            recent = [t.lower().strip(".,!?;:") for t in msg.split()[-5:]]
            q = suggest_question(phi_now, H_now, recent)
            ask = f"(curiosity) {q}"
            print(ask)
            if speak_on:
                _speak(q)
