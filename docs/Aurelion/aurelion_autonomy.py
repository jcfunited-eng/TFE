...
"""
aurelion_autonomy.py — Aurelion autonomous runner with GUI, live plots, and adaptive learning
---------------------------------------------------------------------------------------------
Run:
    python aurelion_autonomy.py
"""
import os, sys, json, time, random, threading, datetime
...
def load_prior_insights():
    ...
    try:
        ...
        var_E = float(last["energy"].var() + 1e-9)
        alpha_nudge = 0.02 if mean_phi < 0.5 else -0.02
        thr_nudge = 0.02 if var_E > 0.05 else -0.02
        return {"alpha_nudge": alpha_nudge, "thr_nudge": thr_nudge}
    except Exception:
        return {}

class AutonomyEngine:
    def __init__(self, adapter: BaseAdapter, interval_sec: float = 1.0,
                 score_lambda: float = 0.5, eval_window: int = 30, switch_every: int = 30):
        ...
        self.source_stats: Dict[str, Dict[str, List[float]]] = {
            a.name: {"phi": [], "E": []} for a in ADAPTERS
        }
    ...
    def score_source(self, name: str) -> float:
        buf = self.source_stats[name]
        phi_arr = np.array(buf["phi"][-self.eval_window:]) if buf["phi"] else np.array([0.0])
        E_arr = np.array(buf["E"][-self.eval_window:]) if buf["E"] else np.array([1.0])
        return float(np.nanmean(phi_arr) - self.score_lambda * np.nanvar(E_arr))

    def best_source(self) -> str:
        names = [a.name for a in ADAPTERS]
        scores = {n: self.score_source(n) for n in names}
    ...
    def step_once(self):
        ...
        self.meta.tune(self.node, info_before["telemetry"], step=self.t)
        ...
        self.source_stats[name]["phi"].append(phi_after)
        self.source_stats[name]["E"].append(E_after)

        switched_to = None
        if self.auto_select and (self.t % self.switch_every == 0):
            best = self.best_source()
            if best != self.adapter.name:
                self.adapter = ADAPTER_BY_NAME[best]
                switched_to = best
        ...
        rec = {
            ...
            "comment": f"auto-switch→{switched_to}" if switched_to else ""
        }
        ...
        try:
            ...
            print("[Warning] Log file locked, skipping this write cycle.")
...
class AurelionGUI(tk.Tk):
    def __init__(self, engine: AutonomyEngine):
        super().__init__()
        self.title("Aurelion — Autonomy Console (Adaptive)")
        ...
        self.auto_btn = ttk.Button(control_frame, text="Auto‑Select: ON", command=self.toggle_auto)
        ...
        self.text_box.insert("end", "Aurelion (adaptive) is coming online…\n")
        self.text_box.config(state="disabled")

        self.fig, (self.ax_phi, self.ax_E) = plt.subplots(2, 1, figsize=(8.8, 6.0), dpi=100)
        self.fig.tight_layout(pad=2.0)
        self.ax_phi.set_title("Coherence φ over time")
        self.ax_phi.set_ylim(0, 1)
        self.ax_phi.set_xlim(0, 250)
        self.ax_phi.grid(True, alpha=0.25)

        self.ax_E.set_title("Energy E over time")
        self.ax_E.set_ylim(0, 2.0)
        self.ax_E.set_xlim(0, 250)
    ...
    def toggle_auto(self):
        with self.engine.lock:
            self.engine.auto_select = not self.engine.auto_select
            self.auto_btn.config(text=f"Auto‑Select: {'ON' if self.engine.auto_select else 'OFF'}")
            self._append_text(f"[System] Auto‑select {'enabled' if self.engine.auto_select else 'disabled'}.\n")

    def on_change_source(self, name):
        with self.engine.lock:
            self.engine.adapter = ADAPTER_BY_NAME.get(name, self.engine.adapter)
            self.engine.auto_select = False
            self.auto_btn.config(text="Auto‑Select: OFF")
            self._append_text(f"[System] Manual switch to {self.engine.adapter.name}. Auto‑select disabled.\n")
        ...
        while not self.stop_event.is_set():
            with self.engine.lock:
                if self.engine.running:
                    ...
                    scores = {n: f"{self.engine.score_source(n):.2f}" for n in ADAPTER_BY_NAME.keys()}
                    score_str = " | ".join([f"{k}:{v}" for k, v in scores.items()])
                    self.status_var.set(
                        f"Source={rec['source']}  φ={rec['phi_after']:.2f}  E={rec['energy']:.2f}  "
                        f"Goal={rec['goal']}  α={rec['alpha']:.2f}  τ={rec['threshold']:.2f}  "
                        f"[Scores] {score_str}"
                    )
...
def main():
    engine = AutonomyEngine(adapter=FinanceAdapter(), interval_sec=1.0,
                            score_lambda=0.5, eval_window=30, switch_every=30)
