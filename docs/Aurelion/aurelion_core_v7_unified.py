...
# aurelion_core_v7_unified.py
# Aurelion v7 — Unified Shell (Calm/Active/Experimental) + Sensory + Emotion + Inherited Memory + Curiosity + Voice

import json, time, random, csv, wave, struct, math, os
...
def _speak(text: str, speak_on: bool):
    if not speak_on:
        return
    if _TTS is None:
        print(f"[speak] {text}")
...
def main():
    print("Aurelion v7 — Unified Shell (Calm/Active/Experimental) + Sensory + Emotion + Memory + Curiosity + Voice")
    ...
    while True:
        ...
        print(out_line)
        _speak(out_line.replace("Aurelion: ",""), speak_on)

        will_ask = (H_now < CFG["curiosity"]["entropy_thresh"]
                    and abs(phi_now) < CFG["curiosity"]["coherence_thresh"]
                    and random.random() < MODES[mode]["curiosity_prob"])
        if will_ask:
