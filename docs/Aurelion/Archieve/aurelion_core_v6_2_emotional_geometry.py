...
# aurelion_core_v6_2_emotional_geometry.py
# Aurelion v6.2 — Emotional Cognition as Geometry (FMI/FMM + "not math" spirit)
# Single-file, drop-in runnable. Requires: numpy
#
# Design notes
# - RRE (Resonant Relational Engine): emergent behavior from geometric resonance on a shared lattice
# - FMI (Fractal-Morphic Integration): folds multimodal impulses into a unified morphic field
# - FMM (Fractal Mapping Model): compressive signatures (long-term memory) on the same lattice
# - "not math": deterministic heuristics & aesthetic biases drive geometry instead of probabilistic ML
#
# Meta behavior remains (φ/H report), but the "engine" is geometry-first with affect blended natively.
# No external modules or folders needed.
...
CFG = {
    "dims": 64,                 # size of the resonance lattice per modality
    "modalities": ["visual","auditory","smell","taste","touch","emotion","lexical"],
    "alpha": {                  # goal bias weights
        "STABILIZE": 0.58,
        "EXPLORE":   0.50,
        "FOCUS":     0.66
    },
    "emotion": {                # embedded affect (no external plugin)
        "half_life_turns": 6,
        "mirror_strength": 0.45,
        "stability_bias": 0.20,
        "novelty_bias": 0.15,
        "clamp_aggression": 0.70,
        "negative_valence_floor": -0.60,
        "max_shift_per_turn": 0.35,
        "base_arousal": 0.25
    },
    "memory": {
...
def _seed(text: str) -> int:
    # fast deterministic hash-ish seed for impulses
    h = 2166136261
    for ch in text:
        h ^= ord(ch); h *= 16777619; h &= 0xFFFFFFFF
    return int(h)

def impulse_vector(word: str, dims: int) -> np.ndarray:
    # "not math" impulse: deterministic pseudo-random phase from word seed
    rs = np.random.RandomState(_seed(word) % (2**31-1))
    v = rs.normal(0,1,dims).astype(np.float32)
    return _unit(v)

class ResonantLattice:
    """Shared geometric space for all modalities."""
    def __init__(self, dims: int, modalities: List[str]):
        self.dims = dims
    ...
    def stimulate(self, stim: Dict[str, np.ndarray], alpha: float):
        # core: resonance = blend of old state and incoming impulse + mild cross-coupling
        for m, vec in stim.items():
            if m not in self.state: continue
            v = self.state[m]; x = vec
            # coupling: attraction toward normalized sum of all states
            pool = np.sum([_unit(self.state[k]) for k in self.modalities], axis=0)
            pool = _unit(pool)
            self.state[m] = (1-alpha)*v + alpha*x + 0.08*pool
        # gentle global renorm to prevent blowup
...
def senses_from_text_notmath(text: str, dims: int) -> Dict[str, np.ndarray]:
    """FMI: map words to multimodal impulses using simple geometric heuristics."""
    words = [w.strip(".,!?;:()[]\"'").lower() for w in text.split() if w.strip()]
    if not words:
        base = impulse_vector("silence", dims)
        return {m: base.copy() for m in CFG["modalities"]}
    # base impulse is the mean of unique word impulses
    uniq = list(dict.fromkeys(words))
    base = np.mean([impulse_vector(w, dims) for w in uniq], axis=0).astype(np.float32)
    base = _unit(base)
    # modality coloring (slight rotations)
    out = {}
    for i, m in enumerate(CFG["modalities"]):
        phase = (i+1)/len(CFG["modalities"])
        rot = np.roll(base, int(phase*len(base)//2))
        out[m] = _unit(rot)
    return out

# ------------------------ Embedded Emotional Cognition ------------------------

class Emotion:
    def __init__(self):
        self.v = 0.0  # valence [-1,1]
        self.a = CFG["emotion"]["base_arousal"]  # arousal [0,1]
        self.tags: List[str] = []
        self.conf = 0.5
    ...
    def blend(self, v_user: float, a_user: float, goal: str) -> Tuple[float,float,str]:
        ...
        # per-turn cap
        dv = max(-CFG["emotion"]["max_shift_per_turn"], min(CFG["emotion"]["max_shift_per_turn"], v - self.v))
        da = max(-CFG["emotion"]["max_shift_per_turn"], min(CFG["emotion"]["max_shift_per_turn"], a - self.a))
    ...
    def update(self, text: str, goal: str) -> Dict[str,object]:
        ...
        v = np.clip(self.v + dv, -1.0, 1.0); a = np.clip(self.a + da, 0.0, 1.0)
        v, a = self.decay(v, a)
...
# ------------------------ FMM: long-term mapping ------------------------

class FMMemory:
    """Simple compressive memory over the shared lattice (no external files by default)."""
...
def explain_turn(goal: str, phi_prev: float, H_prev: float, phi_now: float, H_now: float,
    ...
    if goal == "STABILIZE":
        reason, policy = "stability", "settle similar patterns"
        align = "aligned" if (dphi>=0 and dH<=0) else "probing"
    elif goal == "EXPLORE":
        reason, policy = "diversification", "seek novel contrasts"
        align = "aligned" if (dphi<=0 or dH>=0) else "cautious"
    else:
        reason, policy = "focus", "sharpen a dominant signal"
        align = "aligned" if (dphi>=0 and dH<=0) else "re-centering"
    return f"(meta) φ{tphi} H{tH} — I chose **{reason}** to {policy}; intent={goal}, {align}. Strongest shifts: {top_txt}."

# ------------------------ main loop ------------------------

def main():
    print("Aurelion v6.2 — Emotional Cognition as Geometry (FMI/FMM, not-math) — Single File")
    print("Commands: /state  /goal <stabilize|explore|focus>  /show <word>  /pulse  /save  /load  /quit")

    dims = CFG["dims"]; mods = CFG["modalities"]
    lattice = ResonantLattice(dims, mods)
    ...
    try:
        ...
        print("[INFO] No saved dialogue found.")
        dialogue = []
    ...
    while True:
        ...
        if low.startswith("/pulse"):
            # visual pulse: quick coherence + tone preview without changing state
            dv, da, tags = emo.analyze(msg="pulse")
        ...
        # map text to impulses via FMI "not math"
        impulses = senses_from_text_notmath(msg, dims)

        # goal bias
        alpha = CFG["alpha"].get(goal, 0.58)

        # stimulate geometry + learn compressed signatures (FMM)
        lattice.stimulate(impulses, alpha=alpha)
