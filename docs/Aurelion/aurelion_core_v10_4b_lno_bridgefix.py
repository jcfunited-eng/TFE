...
def mk_seed_from_residue(residue: dict):
    # v10.5+ compatibility: use tapestries if motifs missing
    motif_token = residue.get("motifs", [{}])[0].get("token", "")
    motif_domain = residue.get("motifs", [{}])[0].get("domain", "")
    if not motif_token and "tapestries" in residue:
        motif_token = residue["tapestries"][0] if residue["tapestries"] else "unknown"
        motif_domain = motif_token

    g = [
        {"id": "n1", "label": motif_token, "type": "concept", "attrs": {"domain": motif_domain}},
        {"id": "n2", "label": "entropy", "type": "metric", "attrs": {"value": residue.get("entropy_curve", [0])[-1] if residue.get("entropy_curve") else 0}},
        {"id": "n3", "label": "energy", "type": "metric", "attrs": {"value": residue.get("energy_curve", [0])[-1] if residue.get("energy_curve") else 0}}
    ]
