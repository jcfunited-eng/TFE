# 155 DEEP SPORT SCIENCE — performance and its limits
Fitness and movement covers what a body does when demands are made on
it. This file is the floor above: what decides a performance, what sets
its ceiling, how skill is actually built, and which of the confident
things said around elite sport survive being looked at. Physiology,
skill learning, psychology and the honest state of the evidence. Where a
question is genuinely unsettled it says UNSURE.

ESSENCE: how hard you can go and how long you can hold it are one curve, not
  two facts. It has two parts — an intensity you could sustain almost
...
CANNOT: no burn explained by lactate — the acidity has other sources, and
  ...
  single predictors of endurance performance there is.
THREAD: medicine (a marker treated as the disease), weather (a falling
  barometer being a sign and not a cause), management (a metric that tracked
  something real and then got targeted).
ASKED-AS: lactic acid burn muscles lactate threshold soreness myth waste product marker testing blood
...
ESSENCE: power is force multiplied by the speed it is produced at, so it can
  be raised from either end — heavier and slower, or lighter and faster.
  Athletes who only lift heavy stay slow; athletes who only move fast stay
  weak. The peak sits somewhere in the middle.
ROOT: physics / power is work over time, so it depends on how much force and
  on how quickly; fitness and movement / a body answers exactly the question
  it was asked.
...
CANNOT: no reading learning from performance inside a session, which closes
...
THREAD: farming (a planting calendar written back from harvest), building
  ...
  forgetting).
ASKED-AS: periodisation training blocks phases plan season peak evidence models base build specific

ESSENCE: thirst is a good enough guide for almost everybody. Drinking to a
  schedule was taught for decades and has killed athletes by diluting their
...
ESSENCE: crossing time zones does not only make athletes tired, it puts the
  body at the wrong point in its own daily cycle — and strength, temperature
...
ASKED-AS: injury prevention screening movement tests predict risk factors reduce hamstring acl programme evidence

ESSENCE: whether an athlete goes back is a medical question surrounded by
  people with other interests — the player, the coach, the club, the season.
...
ASKED-AS: return play cleared injury pressure coach player decide criteria concussion protocol rules binding

ESSENCE: picking future champions from children barely works. The tests
  measure current performance, and current performance at twelve is mostly
  maturity plus practice so far. Most identified talents do not make it, and
  most who make it were never identified.
ROOT: evidence / a prediction is only as good as the stability of what it
...
ESSENCE: in almost every youth sport, children born just after the cut-off
  date are hugely over-represented in the top teams. They are up to a year
  older, so they are bigger, get selected, get better coaching, and the head
  start becomes real. The bias manufactures the talent it claims to find.
...
ASKED-AS: relative age effect birthday cut off youth sport selection older kids bigger academy september

ESSENCE: the finding was that top violinists had accumulated more deliberate
  practice than good ones — an average, in one domain, with a wide spread
  around it. It was never that ten thousand hours produces expertise, and
  practice explains only part of the gap between performers.
...
ESSENCE: performance rises with excitement and then falls. Too flat and you
  react late; too wound up and fine control goes first. Where the peak sits
  depends on the task — a lifter can be nearly enraged, a golfer putting
  cannot, and one athlete needs both levels in a day.
ROOT: the mind / arousal narrows attention and raises muscle tension, which
...
ASKED-AS: tactical periodisation football training methods game based conditioning coaching sessions week structure specificity

ESSENCE: small optimisations only matter once the large ones are complete. A
  cyclist with perfect nutrition timing and poor sleep has bought a fraction
...
CANNOT: no predicting a race from the ceiling alone among trained people,
  which makes it a poor selection tool at the top. And no raising it
...
ESSENCE: muscle fibres run along a range from slow and enduring to fast and
  quickly tired, and the proportions a person has are largely inherited.
  Training shifts the middle of that range and does not convert one end into
  the other, which is why sprinters are born as well as made.
ROOT: heredity / a trait with a strong genetic component and a modest
  trainable range; the body / a fibre's properties set by machinery that
  changes slowly and only so far.
CANNOT: no turning a distance runner into a sprinter by training, so no
...
ASKED-AS: weight cutting boxing wrestling making weight dehydration sauna weigh in rules dangerous class

ESSENCE: for hard efforts past about ninety minutes the limit is often
...
CANNOT: no advantage from raw reaction speed, so no testing sporting talent
  ...
  carry over to another one.
THREAD: driving (an experienced driver reading a road earlier), medicine (a
  diagnosis formed before the tests arrive), chess (a position recognised
...
CANNOT: no specificity from appearance, so no gym exercise justified by
  looking like the sport. And no skipping the general either: the useful gym
...
ASKED-AS: injury mental fear reinjury confidence identity athlete depression rehab returning psychological long term

ESSENCE: a brain injury with no visible wound, whose symptoms are unreliable
  and whose sufferer is the least able to judge them. Every rule that works
  removes the decision from the moment — if in doubt, sit out, then a staged
  return. The size of the long-term risk is UNSURE.
ROOT: living machinery / an organ that reports its own state using the very
...
CANNOT: no comparing performances across an equipment change, so no clean
  ...
  which is why governing bodies rule on the technology instead.
THREAD: money (returns that were really a rising market), farming (yields
  credited to skill during a wet decade), computing (a benchmark broken by a
  change of compiler).
ASKED-AS: records shoes swimsuits technology banned equipment times improved comparing eras rules track surface
...
CANNOT: no single peak age for athletes, so no age-based selection rule that
  ...
  why judgement cannot compensate.
THREAD: work (professions peaking at different ages for the same reason),
  music (a violinist's hands against a conductor's ear), chess (calculation
  against pattern knowledge).
