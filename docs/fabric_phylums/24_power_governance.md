# 24 POWER AND GOVERNANCE — how rule actually works
Described as found, no side taken. These patterns recur across
every century tried; softer than physics, harder than fashion.

ESSENCE: no ruler rules alone — every order is executed by chains
  of other hands, so power is really the standing expectation
...
ASKED-AS: king boss dictator coup mutiny strike ignored followers government enforce

ESSENCE: legitimacy is the shared belief that this power is
  rightful — the belief, not the force, does the daily work;
  force is what gets spent when belief runs short.
...
THREAD: money (paper money IS this — valuable because believed),
  ...
  belief).
ASKED-AS: rightful consent trust accepted tyrant revolution brainwash respect topple mandate

ESSENCE: whoever controls what is known controls much of what can
  be done — seeing (surveillance), hiding (secrecy), telling (the
...
ASKED-AS: spying propaganda news media cover up classified cameras rumor whistleblower gossip

ESSENCE: power drifts toward concentration — office gathers the
  means to hold office; dispersal (elections, courts, term
  limits, rival branches) is engineering against the current, and
...
ASKED-AS: checks balances monopoly incumbents rigged entrenched corruption oversight rich elites accumulate

ESSENCE: succession is the perennial crisis — the moment between
  rulers is the moment belief has no object, and much of
  history's bloodshed crowds into it.
...
THREAD: money (family firms wobble at handover — UNSURE on the
  ...
  in verse).
ASKED-AS: heir throne next in line vacuum transition civil war dynasty replace

ESSENCE: procedure is a machine for removing persons from
  decisions — when the rule, not the ruler, decides, outcomes
  become predictable to strangers, and strangers then build on
  the prediction.
...
THREAD: law + agreements (the whole field is this machine), money
  ...
  famous failures).
ASKED-AS: bureaucracy red tape paperwork process forms impartial consistent loophole exceptions fairness

ESSENCE: power poisons its own information — each rung reports
  upward what the rung above wants to hear, so the top rules a
  fiction that thickens the longer and harder the rule.
ROOT: people together / rung-pain — the messenger has a ladder to
...
THREAD: money (the boss's dashboard tells the same lies at
  ...
  doctors).
ASKED-AS: yes men flattery out of touch bubble echo chamber sugarcoat feedback surrounded blind

ESSENCE: rule endures by keeping rivals divided — the many who
  could combine against the few must never combine; the mirror
  tool of the many is combining anyway.
...
THREAD: war (defeat in detail — beat the armies one at a time),
  ...
  scale).
ASKED-AS: conquer unions strikes solidarity protest movement together factions infighting outnumbered

ESSENCE: every government runs on extraction — tax, levy, or
  monopoly — and survives by returning enough (order, roads,
...
THREAD: farming (the first states rose where grain could be
  ...
  receipt, written down).
ASKED-AS: tribute services deal protection evasion rebellion tolls budget public spending
