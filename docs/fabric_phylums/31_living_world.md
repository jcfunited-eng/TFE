# 31 THE LIVING WORLD — physics that pays its own bills
Everything alive is doing one thing: buying order with borrowed
energy, a step ahead of disorder, for a while.

ESSENCE: the cell is the unit of life — nothing alive is less than
...
ASKED-AS: bacteria germs microscope smallest organism membrane amoeba tiny biology bodies

ESSENCE: living is paid-for order — a body keeps its shape only by
  spending energy against disorder every second; stop paying and it
...
ASKED-AS: metabolism calories eating food starvation rot corpse hunger breathing constant

ESSENCE: evolution is differential survival compounding — offspring
...
ASKED-AS: darwin natural selection fittest adaptation species giraffe random breeding designed

ESSENCE: everything eats something and feeds something — the chains
...
THREAD: money + trade (supply chains are food webs with invoices),
  ...
  lines).
ASKED-AS: predator prey ecosystem wolves deer grass ripple balance nature bees

ESSENCE: a niche is a way of making a living, and two species
  cannot hold the same one in the same place for long — one
...
ASKED-AS: competition habitat territory crowded specialists overlap market job animals squeezed

ESSENCE: cooperation is a winning strategy, not a kindness —
  flowers and bees, gut and body, fungus and root: partners spread
...
ASKED-AS: clownfish anemone lichen pollination mutual helping together benefit exchange birds

ESSENCE: extinction is forever — when the last one dies, that way
  of being alive closes, and evolution never reruns it. (over 90%
...
ASKED-AS: dinosaurs endangered mammoth cloning conservation rare vanished irreplaceable revive save

ESSENCE: size sets the rules — double a creature's length and its
...
ASKED-AS: enormous ants movie monsters elephant spider godzilla shrink huge proportions

ESSENCE: all life is one family — every known creature runs the
  same four-letter code with nearly the same dictionary, one tree
...
THREAD: language (the Romance tongues betray one Latin mother the
  ...
  ancestors).
ASKED-AS: dna relatives chimp banana percent cousins origin common kinship apes
