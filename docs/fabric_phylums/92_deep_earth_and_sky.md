# 92 DEEP EARTH AND SKY — the planet and what is past it
One rock, a thin skin of air and water, a furnace inside and one
overhead. Everything here is those few things arranged by
distance, speed, and time — and the sky is the same story, larger.

ESSENCE: the planet files its violence along seams — plot every
  quake and every volcano on a blank globe and the plate edges
  draw themselves; a plate's middle is quiet by construction, and
  always was.
...
ESSENCE: a rock's name is its history, not its ingredients — the
  same few minerals become granite by cooling slowly, sandstone
  by settling, gneiss by being squeezed hot without ever melting.
...
ESSENCE: the fossil record is not merely thin, it leans — shells
  in shallow seas and lowland mud survive, while soft bodies,
  uplands, and forest floors are erased, because the places that
...
ESSENCE: the ice ages are paced by a whisper and delivered by a
  shout — small wobbles in the earth's tilt and orbit shift
  northern summer sunlight only slightly, and ice's whiteness plus
  the ocean's grip on carbon multiply that nudge into a mile of
  ice.
...
ESSENCE: carbon runs two loops at once — a fast one through
  plants, animals, and air that turns over in years, and a slow
  one through rock that takes hundreds of thousands, as rain
...
ESSENCE: most fresh liquid water is underground, filling the gaps
  between grains like water in a sponge, not running in caverns;
  it creeps a metre a day or a metre a year, so a well lifts rain
...
ESSENCE: a river's real cargo is dirt — it carries the land
  downhill in suspension and drops it where it slows, so a river
  is a conveyor shortening mountains at one end and building
...
ESSENCE: moving water sorts by weight, so grain size is a fossil
  of speed — boulders drop where the flow is fierce, sand where it
  eases, clay only in near-still water; a bed of rock reports how
...
ESSENCE: beaches are rivers of sand running sideways — waves
  striking a shore at an angle walk each grain along the coast in
  small zigzags, so a beach is not a place but a flow, forever
...
ESSENCE: the great deserts ring the planet in two bands near 30°
  north and south, because air that rose and rained at the equator
  must come down somewhere, and descending air warms and drinks
  instead of raining — Sahara, Arabia, Kalahari, Australia,
...
ESSENCE: between those belts the winds are steady enough to plan a
  life on — the trades blow the same way year after year, and the
  age of sail therefore ran on fixed loops: out on one belt, home
  on another, never the direct line.
...
ASKED-AS: trade winds sailing ships route columbus doldrums prevailing westerlies voyage return

ESSENCE: soil is rock ground fine and mixed with the dead — and
  the best of it is usually imported: the world's breadbaskets sit
  on river silt, wind-blown glacier dust, or volcanic ash
  delivered from somewhere else. Soil made in place from the rock
...
ESSENCE: limestone country is dissolved, not ground — rain picks
  up a trace of acid from the air and soil, and that weak acid
  eats carbonate rock away from the inside, which is why caves,
...
ASKED-AS: cave stalactite sinkhole limestone cavern underground river hard water karst

ESSENCE: volcanoes come in two tempers and stickiness decides —
  runny lava lets its gas bubble out and builds wide gentle
  shields that pour for years; stiff, silica-rich lava traps its
  gas until pressure wins, and builds steep cones that explode.
...
THREAD: cooking (thick porridge spits, thin soup simmers),
  ...
  catastrophic).
ASKED-AS: hawaii lava flow mount saint helens vesuvius eruption explosive cone shield ash

ESSENCE: mountains float — light crustal rock sits in the heavy
  mantle like an iceberg, so a high range must carry a deep root,
  and as erosion shaves the top the whole block rises to keep its
  balance.
...
ESSENCE: a tsunami is not a big wave but a whole column of water
  shifted at once — the sea floor jumps, the entire depth moves
  with it, and the pulse crosses an ocean at jetliner speed barely
  a metre high, only rearing up when the shallows force it to.
...
ESSENCE: earthquakes are also the only lamp we have for the deep
  earth — every quake sends two kinds of shaking through the
  planet, and one kind cannot cross liquid, so the shadow it casts
  on the far side proved the outer core is molten.
...
ESSENCE: the air is layered by where its temperature turns around
  ...
  ozone layer above; that warm lid stops rising air dead, which is
  why all weather is trapped below it and airliners cruise just
...
ESSENCE: the ozone shield would be three millimetres thick if
  gathered at sea-level pressure, and it works by being destroyed
  — split by the very ultraviolet it absorbs, then rebuilt,
  continuously. It is a running process, not a stored wall.
...
THREAD: the living world (life stayed in the water until this
  ...
  fails quietly).
ASKED-AS: ozone hole sunscreen uv rays skin cancer aerosol spray antarctica sunburn shield

ESSENCE: sunlight arrives short and crosses the air almost
  untouched; the warmed ground beams it back long, and certain
  gases absorb long and re-radiate it in all directions, half of
  it downward — so the surface must run hotter to shed the same
  amount to space.
...
THREAD: clothing (a blanket adds no heat; it makes losing heat
  ...
  the same fuel).
ASKED-AS: global warming greenhouse gas methane blanket temperature rising physics heat trapped

ESSENCE: the ocean hauls heat, and it also hauls time — the deep
  return leg takes roughly a thousand years, so water surfacing
  today sank when the surface world was different. The sea is the
...
THREAD: money (long-dated debts falling due from decisions nobody
  ...
  generation).
ASKED-AS: deep ocean old water currents conveyor slow warming lag absorb heat delay

ESSENCE: the Pacific runs a see-saw — normally the wind piles warm
  water in the west; every few years the wind slackens, the warm
  pool slides east, and the world's rain map moves with it. It
...
ESSENCE: the world's water is fixed and ancient, so the cycle is
  about waiting, not amount — a molecule spends about nine days in
  the air, weeks in a river, thousands of years in the deep sea or
  locked in ice. The same water has been through everything that
  ever drank.
...
ESSENCE: sea level is not one number — it rises when ice on LAND
  melts, rises again simply because warm water takes more room,
  and meanwhile the land itself is still lifting or sinking where
...
THREAD: measurement (a number with no place attached says little),
  ...
  target).
ASKED-AS: sea level rising melting ice caps flooding coastal cities glaciers land sinking

ESSENCE: the planet's magnetism is no buried bar magnet but a
  running dynamo — molten iron circulating in the outer core makes
  it continuously, which is why it drifts, weakens, and has
  flipped its poles hundreds of times, the record frozen into
...
ESSENCE: the aurora is the shield being hit — the sun throws out
  charged particles continuously and sometimes in gusts, the
  magnetic field deflects most of them, and the leak is funnelled
  down the field lines to the poles, where it lights the high air
  like a neon tube.
...
ESSENCE: seasons come from tilt, not distance — the earth leans
  23.5° and keeps that lean pointed the same way all year, so each
  half takes its turn tipping sunward, catching light at a steeper
  angle for more hours. The earth is actually NEAREST the sun in
  January.
...
THREAD: farming (the whole planting calendar is this lean),
  ...
  of the same hill).
ASKED-AS: seasons summer winter hot tilt closer sun southern hemisphere opposite solstice

ESSENCE: phases are geometry, not shadow — half the moon is always
  lit, and we see more or less of that lit half depending where it
  stands in its orbit; earth's shadow has nothing to do with it.
  That shadow works only at eclipses.
...
ESSENCE: the moon is leaving and the day is lengthening — the
  tides drag on a spinning earth and slow it, and the momentum
  lost has to go somewhere, so it goes into the moon's orbit,
...
ESSENCE: the sun runs on mass, not fuel — it converts about four
  million tonnes of itself into energy every second, which is why
  it can burn for billions of years where any chemical fire of the
...
ESSENCE: the solar system is mostly nothing — shrink the sun to a
  basketball and the earth is a peppercorn 26 metres off, Jupiter
  a marble at 135, and the next star a second basketball 6,800
  kilometres away. Every diagram ever drawn lies about the
...
ASKED-AS: solar system model distance planets far apart scale asteroid belt dodge empty

ESSENCE: a star is a standoff — gravity crushing inward, fusion's
  heat pushing outward — held for billions of years only while the
  fuel lasts; when it runs out the standoff breaks, and the star's
  mass alone decides whether it puffs off its skin quietly or
  collapses and detonates.
...
ESSENCE: a black hole is not a vacuum cleaner — it is mass packed
  so small that close in, nothing escapes; swap the sun for a
  black hole of the same mass and the earth would keep its orbit
...
ESSENCE: gravity assembles at every scale — dust into stars, stars
  into galaxies of hundreds of billions, galaxies into clusters —
  and our sun is one ordinary star far out on one arm of one
...
ESSENCE: every distant galaxy is receding, and the farther one is
  the faster it goes — not because they fly apart through space,
  but because the space between them is growing, like raisins
  spreading in rising bread. There is no centre; every raisin sees
  the same view.
...
ESSENCE: distance IS the past — looking further out is looking
  further back, so the deep sky is layered like rock: near
  galaxies as they are now, far ones as infants, and beyond a
  certain distance nothing at all, because the light has not had
  time to arrive.
...
THREAD: keeping knowledge (an archive of pure past, shelved by
  ...
  history).
ASKED-AS: telescope looking back time early universe deep field observable edge light years

ESSENCE: there is a wall at the far end of that view — for its
  first few hundred thousand years the universe was a glowing fog
  no light could cross, and the moment it cleared is still
  arriving, as a faint even hum of microwaves from every direction
  at once.
...
ASKED-AS: cosmic background radiation oldest light static big bang proof microwave glow universe

ESSENCE: the darkness of the night sky is itself evidence — in a
  universe infinitely old and infinitely wide, every line of sight
  would end on a star and the whole sky would blaze; that it does
  not means the universe had a beginning and a finite reach.
...
THREAD: evidence (the strongest tests are the ones already
  ...
  a whole theory).
ASKED-AS: why night dark sky stars everywhere infinite universe olbers paradox blackness

ESSENCE: comets and asteroids are the leftovers that never joined
  a planet, not the wreckage of one that broke — rock and metal in
  the warm inner belt, ice further out where ice could survive. A
  comet's tail is grown, not trailed: it points away from the sun
...
ASKED-AS: comet tail asteroid belt meteor shooting star halley impact dinosaurs rocks

ESSENCE: the earth's age is read off meteorites, not off the earth
  — this planet recycles its own crust, so its oldest surviving
  rock is far younger than the planet; the leftovers that never
  got reworked keep the original clock.
...
ASKED-AS: how old earth age dating meteorite rocks oldest billion years measured proof

ESSENCE: the sky over the ocean is punctuated — five times the
  fossil record thins to almost nothing worldwide in one narrow
  layer, and one of those layers carries a metal common in
  asteroids and rare in earth's crust, spread over the whole
  planet at once.
...
ASKED-AS: mass extinction dinosaurs asteroid impact crater iridium layer wiped out five

ESSENCE: the air you breathe is a waste product — the early
  atmosphere had almost no free oxygen, and the tiny green things
  that learned to eat sunlight spent hundreds of millions of years
  exhaling it, rusting the world's iron before any of it could
  stay in the sky.
...
ESSENCE: distance from the sun and size explain most of the other
  planets — near in, only rock and metal could condense, so the
  inner worlds are small and stony; far out ice survived, so those
  worlds grew massive enough to keep hydrogen. Then size decides
...
ASKED-AS: mars venus jupiter atmosphere thin gas giants rocky planets moon no air

ESSENCE: the moon is a chip off this planet — its rock matches
  earth's mantle too closely for a captured stranger, and the
  reading that fits is a collision early on that threw a ring of
  debris into orbit; the moon then steadied the tilt that gives us
  regular seasons.
...
ESSENCE: other stars' planets are found without being seen — a
  planet crossing its star dims it by a hair on a schedule, and a
  planet's pull rocks its star a few metres a second; thousands
...
ESSENCE: a world is habitable only where several unrelated
  conditions hold at once — liquid water needs a temperature band
  AND enough air pressure to keep it from boiling away; keeping
  the air needs enough gravity and a magnetic shield; and complex
  life needs the whole arrangement to stay steady for billions of
  years.
