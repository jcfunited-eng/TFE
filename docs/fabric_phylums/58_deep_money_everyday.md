# 58 DEEP COLUMN — MONEY IN DAILY LIFE
Middle floors for the wallet: checkout, casino, premium, tip jar
— chance+risk and the mind's bookkeeping, wearing prices.

ESSENCE: prices end in .99 because the eye weighs the left digit
  heaviest — $2.99 files as "two and change" though it is a penny
...
ESSENCE: the house wins because every game pays a notch under
  true odds — roulette pays 35-to-1 on a 37-to-1 shot — and
  plays millions of rounds to your dozens; luck rules an
...
ESSENCE: insurance is the casino reversed — a small sure loss
  (premiums exceed expected claims) buying off the rare loss
  that would end you; rational where ruin is on the table —
...
ESSENCE: compound interest grows the pile by a fraction of the
  pile, so growth itself grows — divide 72 by the yearly percent
  for doubling time (7% doubles in ~10 years; a 20% card doubles
  debt in under 4).
...
ESSENCE: sales manufacture urgency because a vanishing chance is
  booked as a loss, and losses press about twice as hard as
  gains — "ends Sunday" turns "do I want this?" into "can I
  stand to lose this?"
...
ESSENCE: cash hurts and cards don't — bills handed over are a
  felt, counted loss; a tap is a promise about a later self —
  people pay measurably more on plastic (one famous auction:
...
ESSENCE: the middleman deletes connections — a hundred growers
  and a hundred shops need ten thousand links, or two hundred
  through one wholesaler — paid from the deleted links, plus
...
ESSENCE: bulk is cheaper because every sale drags costs that
  ignore size — wrapper, label, shelf slot, scan, the truck's
  stop — a big package spreads the flat toll over more product
...
ESSENCE: tipping persists because everyone is locked in by
  everyone else — the wage is set expecting it (tipped minimums
  sit far below standard): the worker needs it, the owner prices
...
ESSENCE: a bubble is obvious only afterward because while it
  inflates the best evidence — price — keeps voting yes: every
  rise pays believers and punishes doubters; the early doubter
...
THREAD: story (hindsight rewrites every disaster as foretold),
  ...
  quoted).
ASKED-AS: bubble crash stocks housing crypto tulips everyone knew obvious hindsight burst mania
