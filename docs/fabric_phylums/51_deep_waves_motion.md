# 51 DEEP COLUMN — WAVES, CORDS, AND SPINNING THINGS
Middle floors between physics' roots and the whip, the swing, the
top. Still essences, never data; every entry carries its cannot-twin.

ESSENCE: a wave is a traveling shape, not traveling stuff — the hump
  runs the rope's whole length while every fiber only bobs in place;
...
ASKED-AS: ripple pond rope slinky ocean swell cork bobbing stadium wave moves travels

ESSENCE: a wave keeps its cargo of energy, so sent down a thinning
  cord it rides ever less material and must run faster and taller —
  a whip tapers so steeply the tip ends by outrunning sound (~340
...
ESSENCE: the snapped towel is the whip's law in cloth — the flick's
  wave runs through heavy wet fabric and ends at one light loose
  corner where the energy piles up, and the corner arrives fast
...
ESSENCE: every swinging thing owns one rhythm, and pushes timed to
...
ASKED-AS: swing pushing playground pump higher timing rhythm rock stuck car glass hum

ESSENCE: resonance is why soldiers break step on a bridge —
  hundreds of boots landing on the bridge's own beat are timed
...
ASKED-AS: soldiers marching bridge break step wobble sway collapse footbridge army stomp

ESSENCE: a spinning thing defends the direction of its axis —
  tipped, it answers with a slow sideways circling instead of a fall
  — so a top stands while it spins and a thrown football holds its
...
THREAD: shooting (rifled barrels spin bullets for the football's
  ...
  calendar).
ASKED-AS: top spinning gyroscope football spiral bullet frisbee bicycle wheel tip fall wobble

ESSENCE: the amount of spin is kept and shape sets its pace — mass
  held wide turns slow, the same mass pulled close must turn fast —
  so the skater folding arms in jumps from lazy to blurring
...
ASKED-AS: ice skater spin faster arms pulled tuck diver flip office chair twirl

ESSENCE: a pendulum's beat is set by its length and almost nothing
  else — not the bob's weight, and for small swings not even the
  width — a string about a meter long beats close to one second each
...
THREAD: music (the metronome — sliding its weight IS changing its
  ...
  pull).
ASKED-AS: grandfather clock pendulum tick swing metronome keep time slow fast bob adjust

ESSENCE: shake a held rope at one of its own fitting rhythms and
  the wave meets its reflection coming back — the two add into loops
  that stand in place with dead-still points between; only rhythms
...
THREAD: music (a guitar string's note IS its standing loop — fret it
  ...
  them).
ASKED-AS: jump rope shaking loops slinky guitar string vibrate node still spots microwave
