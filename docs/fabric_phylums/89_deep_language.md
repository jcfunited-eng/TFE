# 89 DEEP LANGUAGE — how words do their work
Sound alone carries nothing. What carries is the machinery around
it: the marks, the order, the turns, and the enormous amount both
sides leave unsaid because it is already held.

ESSENCE: no noise resembles what it names, and nothing about a tree
  suggests the sound "tree". So a word cannot be worked out, only
  handed over — a language is a chain of hand-offs, and where the
...
ESSENCE: nobody teaches a child grammar and it arrives anyway — on
  broken input, with almost no corrections, by about five. The proof
  that rules were built and not copies stored is the errors: goed,
...
ESSENCE: grammar issues no orders. It rules arrangements out, and
  everything not ruled out is available — which is why a few dozen
...
ESSENCE: who did what to whom has to be marked somewhere. English
  marks it by position and pays by losing the freedom to move words;
  Latin marks it on the word endings and can scatter them down the
...
ESSENCE: "the birds sing" says plural twice. The verb adds no news —
  it repeats what the doer already told you, so that with eleven
  words sitting between them the listener can still find the join,
...
ESSENCE: when is one dial and shape is another. "I read it" and "I
  was reading it" name the same afternoon and differ in where you
  stand — outside the event looking at a finished lump, or inside it
...
ESSENCE: "it is raining", "it might be raining" and "it must be
  raining" report one sky and three positions of the speaker toward
  it. The modal word changes no weather; it changes what you may
...
ESSENCE: "not" is a knife and nothing on the page says where it was
  aimed. "I did not take it because I was hungry" denies either the
  taking or the reason, and those are opposite confessions. A voice
...
ESSENCE: a question is a statement with a hole cut in it and a mark
  saying who fills it. The mark's shape names the size of the hole —
  yes or no, or a person, or a place — and the device works by
...
ESSENCE: a sentence can carry another sentence as cargo — "he said
  the bridge is out" — and the cargo's truth does not travel up. The
  outer verb decides: knew grants it, claimed withholds it, dreamt
...
ESSENCE: a thing grows shorter the longer it stays live: a woman,
  the woman, she, and finally nothing at all. The shrinking is a
  readout of how present it is in the listener's head, so a full name
...
ESSENCE: "a dog" puts something new on the table; "the dog" claims
  you can already find exactly one. So "the war" needs no explaining
  between two people from one country and baffles a third — and an
  advert opening with "the problem" has installed a problem you
...
ESSENCE: a sentence is a ramp — start on ground the reader already
  stands on, end on the new thing. Which is why the passive is not a
...
ESSENCE: every utterance names a drawer and then puts something in
  it. "As for the car, it needs tyres" says the drawer out loud;
  English usually leaves it to position and expects you to keep using
...
ESSENCE: "I didn't say she stole it" is seven different claims,
  picked by which word is leaned on. Each pick quietly asserts the
  alternatives it rejects — "I bought the RED one" says there were
  others. Position does the same job: the end of a sentence is the
  loud end.
...
ESSENCE: "roughly", "I think", "about a mile" mark a claim as loose,
  so that being wrong is not a lie. They buy accuracy with force —
  and past a certain dose they read as having nothing, since a
...
ESSENCE: politeness is not warmth, it is the handling of imposition.
  "Could you possibly" runs long because the ask is big or the gap
  in rank is wide, and every extra word buys the other person a way
...
ESSENCE: "some of them came" is true when all of them came, and
  nobody hears it that way — because you would have said all. Most
  of what is understood is worked out from what the speaker chose
  not to say. A reference praising only the handwriting sinks the
  candidate.
...
ESSENCE: "when did you stop?" asks about a date and smuggles the
  stopping. The smuggled part is not what is being asked, so
  answering at all accepts it. The tell is that it survives denial:
  "I did not stop last year" still hands over that there was a
  stopping.
...
ESSENCE: listeners assume you are telling the truth, saying enough
  and no more, staying on the point, and speaking plainly. Break one
  where it can be seen and the break itself becomes the message — an
...
ESSENCE: nobody waits for silence. A turn is built from units whose
  ends can be seen coming, and the next speaker aims at that end,
...
ESSENCE: speech is full of wrong starts and nearly all of them are
  mended by the speaker, mid-turn, before anyone else moves. The
  system strongly prefers you fix your own — which is why correction
...
ESSENCE: "so", "anyway", "well", "actually", "but" carry no facts and
  do the routing. They say what the next piece is doing to the last
  one — resuming, opposing, closing, conceding. Cut them all out and
...
ESSENCE: writing is not speech typed out. It loses the face, the
  voice, the shared room, and above all the checking — you cannot see
  the frown and rephrase. It pays that back with explicitness,
  structure and revision, and gains what speech never has: it can be
...
ESSENCE: the marks buy back a fraction of the voice, and a different
  fraction each century — they began as instructions for reading
  aloud and hardened into logic. What they never recover is stress
  and irony, which is why plain notes start fights and why people
...
ESSENCE: a speaker keeps several varieties and moves between them by
  audience — and the move itself is the message: stepping toward
  someone's way of speaking says closeness, stepping away says
  distance. The variety that counts as correct is the one spoken by
...
ESSENCE: one word standing in for a paragraph is real speed among
  people who hold the paragraph. It is the same word that locks
  everyone else out, and the speed and the lock are one property — so
...
ESSENCE: a translator cannot carry the sense, the sound, the level,
  the associations and the length all at once. Something is set down
  at the border every time, so every translation is an argument about
...
ESSENCE: the listener does not wait. Every meaning of a word opens
  for an instant and all but one is dropped within a heartbeat, well
  below the floor. The proof that it commits early is the sentence
...
ESSENCE: form and sense are separate machines. "Colourless green
  ideas sleep furiously" is faultlessly built and points at nothing.
  That is the whole business of fluent nonsense: a well-made sentence
...
ESSENCE: there are no spaces in speech. The sound is one continuous
  ribbon and the word boundaries are put in by the listener out of
  what he already holds — which is why a language you do not speak
...
ESSENCE: a sentence carries no meaning across. It is a set of
  instructions for building one at the far end out of the listener's
  own materials — so the same words build different things in
...
ESSENCE: words are assembled from smaller reusable pieces — un,
  friend, li, ness — so a tongue mints what it needs on the spot,
...
ESSENCE: a few hundred words do most of the talking, and the rare
  ones carry most of what is actually said. "The" is everywhere and
...
ESSENCE: some sentences do not report, they do. "I promise", "I
  resign", "I name this ship" — the saying is the act, and there is
  nothing in it to check for truth. What they need instead is
...
ESSENCE: some words aim from wherever the speaker stands — here,
  now, I, you, tomorrow, behind you. Move the speaker and the same
  word points somewhere else, which is why a note reading "back in
...
ESSENCE: a name keeps pointing at its bearer even when everything
  attached to it turns out wrong. Learn that the man never wrote the
  plays and the name still lands on the same man — because a name is
  a tag tied on at the start, not a description that must keep
  fitting.
...
ESSENCE: two people settle on their own names for things within a
  few turns — "the wobbly one", "that thing from Tuesday" — and from
...
ESSENCE: the verb decides what the sentence must contain. "Give"
  demands a giver, a thing and a receiver; leave one out and a hole
  is felt by someone who could never name the rule. The verb is not
...
ESSENCE: grammar offers a legal way to describe an event with nobody
  in it. "Mistakes were made", "the account was closed" — the thing
  happened, the doer is gone, and no rule was broken in removing
  him.
...
ESSENCE: yes comes fast and bare; no comes slow, padded and
  explained. So the delay is the answer before the words arrive —
  "well... I mean... it could work" has already refused, and both
  sides know it has.
...
ESSENCE: "soon", "a fair share", "a reasonable time" have no edges,
  and that is what they are for. Two parties who could never agree
  on a number can agree on the word, sign, and go — with the
...
ESSENCE: renaming a thing moves the feeling and leaves the thing.
  The new word is clean only while it is new; it takes the colour of
  what it names and within a generation needs replacing — lavatory,
...
ESSENCE: strong words are spent by use. Force is measured against
  what was expected, so a word used for everything stops meaning
  anything — awful, terrific, literally, amazing — and the tongue
...
ESSENCE: every community that has ever existed has speech; only some
  have writing. A child takes speech for nothing, by being near it,
  and takes writing only from years of instruction — which tells you
...
ESSENCE: languages made with the hands carry grammar of their own —
  word order, agreement, tense — all of it done in space rather than
  in time, and all of it learned on the same timetable as speech.
...
ESSENCE: put adults with no shared tongue together and they build a
  makeshift — words with almost no grammar. Hand that makeshift to
  their children and it comes back a full language, with tenses and
...
ESSENCE: many tongues sort every noun into classes — masculine and
  feminine, or animate, or long-and-thin — and the sorting says
  nothing true about the things. A German girl is neuter. Each item
  must be learned as it comes, and the whole apparatus exists so the
...
ESSENCE: a mark can stand for a sound, a syllable, or a whole word,
  and that choice decides what a beginner faces. Thirty letters and
  years of spelling; or two thousand characters and no spelling at
...
ESSENCE: no language stays short of a word for long. It borrows one,
  joins two it already has, or stretches an old one over the new
...
ESSENCE: in many tongues you cannot open your mouth without saying
  where you stand — which "you" to use, whether to add the title,
...
ESSENCE: you can put another person's words in your own mouth whole
  and claim they are the exact ones. That claim is checkable and is
  usually false — people store the sense and deliver it as the
  words. And the delivery lets you act the speaker while disowning
  what you are acting.
