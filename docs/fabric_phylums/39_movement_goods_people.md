# 39 THE MOVEMENT OF GOODS AND PEOPLE — geography deals, trade plays
Why cities sit where they sit, why the shelf is stocked, and why
the parcel's last mile costs more than its ocean.

ESSENCE: everything pools where paths cross — cities sit at fords,
  harbors, passes, and junctions, because whoever stands at the
  crossing touches every load that passes. Geography deals the
...
ASKED-AS: town located port grew built map settlement founded location riverbank big old

ESSENCE: the last mile costs most — the ocean moves a shirt for
  pennies because ten thousand shirts share the ship; the van to
...
THREAD: computing (the cable into the house costs more per user
  ...
  free — reaching the one confused student is the cost).
ASKED-AS: mailing delivery pricey amazon package bargain overseas china postage courier fees truck

ESSENCE: the standard box beat the fast ship — agreeing on one
  container size did more for trade than any engine, because what
...
ASKED-AS: harbor crates ports cranes pallets global loads docks stacking uniform freight interchangeable

ESSENCE: networks beat point-to-point — ten cities need forty-five
  direct routes, or one hub and ten; route through centers and a
...
THREAD: the body (all blood routes through one hub called the
  ...
  cheap, not the courier).
ASKED-AS: layover flight airport straight spoke airline train subway stopover switching wiring bus

ESSENCE: congestion is everyone's sensible choice colliding — each
  driver picks the fast road, which is what makes it slow; the jam
...
ASKED-AS: traffic rush hour highway lanes commute bumper gridlock stuck adding more cars

ESSENCE: every trip is secretly a round trip — the truck that
...
ASKED-AS: airline flights pricing uber surge deadhead freight rates imbalance both directions moving

ESSENCE: a supply chain is a chain — the shelf empties when any
  one link snaps (mine, ship, port, part, driver), and the chain's
...
ASKED-AS: shortage stock bare stores pandemic toilet paper chips factory delays backorder bottleneck

ESSENCE: distance died for words but not for bodies or hours — a
  message crosses the world in a blink for nothing, while goods
  still ride ships for weeks and a meeting still needs two cities
  awake at once.
...
THREAD: money (markets follow the sun: Tokyo hands to London hands
  ...
  "now" at all).
ASKED-AS: video call instant email free timezone zoom overnight cargo teleport scheduling texting
