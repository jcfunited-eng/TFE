# 82 CONTEXT — what settles which meaning is meant
Words never carry meaning alone. The room, the last turn and who is
speaking do half the work — the half nobody notices doing it.

ESSENCE: a word arrives with all its meanings and the surroundings
  kill all but one — "charge" at a till, at a battery and at a
...
ESSENCE: a sentence on its own is a list of possible sentences —
  "she couldn't lift it" holds a suitcase, a mood and a ban inside
...
ESSENCE: who is speaking, to whom, where they stand and what has
  just happened carry as much as the words do — "get down" from a
...
ESSENCE: what both sides already hold is the road a new thing
  travels on — every explanation is laid on top of things left
...
ESSENCE: the last thing said is the loudest context there is, and
  it fades fast — an answer given three turns late lands on a
...
ESSENCE: the same content has one dress for a courtroom, another
  for a kitchen and another for a joke — and the wrong one is not
...
ESSENCE: a frame decides what is even worth noticing — inside a
  recipe "sharp" is the knife, inside a chord it is the note, and
...
ESSENCE: context can be handed over on purpose — "speaking as your
  doctor", "this is a guess", "before you answer" — and one such
...
ESSENCE: a missing setting can be fetched — one question brings it
...
ESSENCE: the usual failure is not confusion but smooth agreement
  inside two different frames — both sides nod, nothing sounds
...
ESSENCE: what is in the room does half the pointing — "pass me the
  glass" in a kitchen reaches one object because only one is
...
ESSENCE: culture is context you were handed and never noticed
  receiving — how close to stand, whether to say no plainly, what
