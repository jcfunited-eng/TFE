# 137 DEEP GEOMETRY AND TRIGONOMETRY — shape, angle and distance
Geometry is what happens when you agree on a few things about points and
lines and then refuse to grant anything else. Trigonometry is the part
of it that escaped into every field where something goes round, comes
back, or repeats.

ESSENCE: a geometric proof grants nothing to the picture. The drawing is a
  memory aid; the argument runs on stated properties alone, which is why a
...
ASKED-AS: geometry proof why not just measure diagram assume looks like given

ESSENCE: one of the old assumptions — that through a point beside a line
  there is exactly one parallel — turned out to be a choice rather than a
  necessity. Drop it and other complete, consistent geometries appear, and
...
ASKED-AS: parallel postulate curved space non euclidean triangle angles more than assumption

ESSENCE: a locus is a shape defined by a rule instead of drawn by a hand —
  every point that is a fixed distance from here, or equally far from two
  things. Nearly every named curve in geometry turns out to be a rule
...
ASKED-AS: locus set of points rule distance from describes curve path condition trace

ESSENCE: everywhere equally far from two points is one straight line, at
  right angles to the join and cutting it in half. That single fact is why
  a circle can be drawn through any three points that are not in a row —
...
ASKED-AS: perpendicular bisector equidistant two points circle through three centre why line

ESSENCE: everywhere equally far from two LINES is the bisector of their
  angle. Distance to a line means the perpendicular drop, and once that is
  agreed, the same locus reasoning that placed a circle's centre now
...
ESSENCE: congruent means identical in every measure; similar means
  identical in shape alone. The whole distinction is one number: to fix a
  triangle's size you must be given at least one length, and no collection
...
ASKED-AS: congruent similar difference same size shape angles enough need one length

ESSENCE: three sides fix a triangle; two sides with the angle BETWEEN them
  fix it; two angles with any side fix it. But two sides with an angle not
  between them can leave two different triangles standing, and that gap is
...
ASKED-AS: congruence rules sss sas asa why not ssa ambiguous two triangles possible

ESSENCE: any two sides of a triangle together beat the third. That is not
  a theorem about triangles so much as a statement that the straight route
  is the short one — and any three lengths failing it cannot be built at
...
ASKED-AS: triangle inequality two sides longer third possible lengths make triangle detour shortest

ESSENCE: join the midpoints of two sides of any triangle and the segment
  you draw is parallel to the third side and exactly half its length.
...
THREAD: building (a cross brace at mid-height), craft (halving a pattern
  ...
  frame).
ASKED-AS: midpoint theorem join midpoints parallel half length triangle segment midline proof

ESSENCE: a line crossing two parallels makes the same angle with both,
  because sliding it along carries one crossing exactly onto the other.
  Every named pair — alternate, corresponding, co-interior — is that one
...
THREAD: building (a rafter meeting parallel walls at matched angles),
  ...
  a carriageway).
ASKED-AS: parallel lines angles alternate corresponding equal why transversal proof z shape

ESSENCE: cut any polygon into triangles from one corner and the angle sum
  falls out: a shape with n sides holds n minus two triangles, so its
  corners total that many half-turns. No new law is needed — only the
...
ASKED-AS: polygon interior angle sum formula pentagon hexagon triangles cut up degrees why

ESSENCE: walk right round any polygon and you turn through exactly one
  full turn, whatever the number of sides. So the exterior angles always
  total one turn — a hundred-sided shape and a triangle agree — and the
...
ESSENCE: a triangle has several natural centres and they are usually
  different points. The medians meet at the balance point; the
  perpendicular bisectors meet at the centre of the circle through the
...
ASKED-AS: triangle centre centroid circumcentre incentre different which one balance point circle

ESSENCE: the balance point of a triangle sits two-thirds of the way down
  each median from the corner. Cut the shape from card and it will rest on
  a pin there — geometry and physics agreeing exactly, with no measurement
  involved.
...
ESSENCE: an angle drawn from the rim of a circle onto a fixed arc is the
  same wherever on the rim you stand, and always half the angle at the
  centre. The circle turns a spread of positions into one number.
...
ASKED-AS: inscribed angle circle theorem same arc half centre why equal proof points

ESSENCE: any angle drawn on a diameter is a right angle. Stand anywhere on
  a circle, look at the two ends of a diameter, and your line of sight
  makes a square corner — which turns the circle into a machine for
...
ASKED-AS: angle in a semicircle right ninety diameter thales circle theorem why always

ESSENCE: a tangent touches a circle exactly once and stands square to the
  radius at that point. Everything about tangents follows: the two
  tangents drawn from an outside point are equal in length, and the figure
  they make is a kite.
...
ASKED-AS: tangent circle touches once perpendicular radius two tangents equal length outside point

ESSENCE: an arc is a fraction of the way round and a sector is that same
  fraction of the inside. Both are answered by asking what share of the
...
RULE: to find an arc or sector — divide the angle by a full turn to get
  the share; multiply that share by the whole circumference for the arc,
  or by the whole area for the sector; if the angle is in radians, the
  share is already the angle divided by two pi and the arc is simply
  radius times angle.
ASKED-AS: arc length sector area fraction of circle angle share radius formula pizza

ESSENCE: a triangle's area is half its base times its height, and ANY side
  may be called the base as long as the height is measured square to that
...
ESSENCE: on a grid, the middle of a segment is the average of its ends,
  one coordinate at a time. That generalises: to find the point a third of
  the way along, take a weighted average — geometry answered by arithmetic
...
ASKED-AS: midpoint formula average coordinates point partway along divide segment ratio between

ESSENCE: slope is a rate wearing a picture: how much it climbs for each
  step across. It is the same quantity as speed, as interest, as gradient
  — which is why a straight line on any chart is a claim that something
...
ESSENCE: a line can be written three ways and each is convenient for a
  different job: from a slope and where it cuts the axis, from a slope and
  any point on it, or as a balanced statement in both coordinates that
...
ESSENCE: parallel lines share a slope. Perpendicular lines have slopes
  that multiply to minus one — because turning a direction a quarter turn
  swaps the across-step and the up-step and reverses one of their signs.
...
ASKED-AS: perpendicular slopes negative reciprocal multiply minus one parallel same why quarter

ESSENCE: the equation of a circle is the distance rule written out and
  never simplified: every point whose distance from the centre equals the
  radius. Recognising that is what turns a page of algebra back into a
...
ASKED-AS: circle equation centre radius complete square recognise x squared y squared

ESSENCE: slice a cone at different tilts and you get a circle, an ellipse,
  a parabola, then a hyperbola. Four famous curves that seem unrelated are
...
ASKED-AS: conic sections cone slice circle ellipse parabola hyperbola why related shapes

ESSENCE: each conic is also a rule about distance. A parabola is
  everywhere equally far from a point and a line. An ellipse is everywhere
  whose two distances to two points add to a fixed total. A hyperbola is
...
ASKED-AS: ellipse definition two foci string parabola directrix hyperbola difference distances draw

ESSENCE: a parabola sends every incoming parallel ray to one point, and
  everything from that point out as a parallel beam. One shape does both
  jobs, which is why it is the profile of every dish, headlamp and solar
  concentrator ever built.
...
ASKED-AS: parabola focus reflector dish headlight satellite why shape collects light point

ESSENCE: an ellipse has two focal points, and everything sent from one
  arrives at the other. Rooms shaped this way carry a whisper across
  themselves; orbits shaped this way put a planet nearer the sun at one
...
ASKED-AS: ellipse two foci whispering gallery orbit sun eccentricity squashed circle why

ESSENCE: one number runs the whole conic family — how far the shape
  departs from being a circle. At nothing it is a circle, below one an
  ellipse, at exactly one a parabola, above one a hyperbola. A single dial
...
ASKED-AS: eccentricity circle ellipse parabola hyperbola one number family comet escape orbit

ESSENCE: three coordinates locate a point in a room, and the same
  arithmetic carries over unchanged — distances add in squares, midpoints
  are still averages. What is genuinely new is that a flat piece of the
...
ASKED-AS: three dimensions coordinates x y z plane equation space distance point line

ESSENCE: anything that narrows to a point holds exactly a third of the box
  or cylinder around it. A cone is a third of its cylinder, a pyramid a
  third of its prism — the same fraction whatever the base, and it is not
  a coincidence.
...
ESSENCE: surface area is the shape flattened out — the net you would cut
  from card. That is why it is measured in different units from volume and
  why the two answer entirely different questions: one is what you must
...
THREAD: packaging (a box's cardboard against its contents), tailoring
  ...
  again).
ASKED-AS: surface area net unfold box wrapping paper volume difference sphere cannot flatten

ESSENCE: a cross section is what a slice reveals, and it is quite unlike a
  shadow. A shadow flattens the whole solid and loses the inside; a slice
  keeps the inside and loses everything else — and a full stack of slices
...
THREAD: medicine (scanners that slice rather than shadow), geology (a core
  ...
  rose).
ASKED-AS: cross section slice through solid shape shows inside scan shadow difference stack

ESSENCE: in a right triangle the three sides give six ratios, and two of
  them carry everything: the opposite over the hypotenuse, and the
  adjacent over the hypotenuse. Those two are sine and cosine, and every
...
THREAD: building (a roof pitch given as a ratio), navigation (resolving a
  ...
  directions).
ASKED-AS: sine cosine tangent ratios right triangle opposite adjacent hypotenuse sohcahtoa what

ESSENCE: the reason a table of sines could ever be written is that the
  ratio depends on the angle and on nothing else. A tiny triangle and an
  enormous one with the same angle give the identical number, so one table
  serves every triangle that will ever exist.
ROOT: geometry / similar shapes scale every length by one factor — the
...
ASKED-AS: why do sine tables work same angle any size triangle ratio similar

ESSENCE: put the triangle inside a circle of radius one and the two ratios
  become simply the coordinates of a point on the rim. That single move
  frees them from right triangles entirely, and lets angles past a quarter
...
ESSENCE: because that point sits on a circle of radius one, its two
  coordinates squared must add to one. Sine squared plus cosine squared is
  one — the oldest theorem in geometry, wearing trigonometry's clothes and
...
ASKED-AS: sine squared plus cosine squared equals one identity why pythagoras unit circle

ESSENCE: a radian is the angle that wraps one radius round the rim. It
  looks awkward and it is the natural measure, because with it the arc
  length is simply radius times angle — no conversion factor anywhere, and
...
ASKED-AS: radians why not degrees natural measure arc length conversion pi calculator mode

ESSENCE: in any triangle at all, each side divided by the sine of the
  angle facing it gives the same number. That is the rule for triangles
  with no right angle — it links pairs of a side and its opposite angle,
...
ESSENCE: the cosine rule is Pythagoras with a correction term for the
  corner not being square. Squeeze the angle below a right angle and the
  far side comes in; open it past and the far side goes out — and the
...
ASKED-AS: cosine rule law of cosines three sides included angle pythagoras correction when

ESSENCE: solving a triangle is choosing between two tools by what you were
  given, not by preference. A matched side and opposite angle points to
  the sine rule; three sides, or two sides with the angle between them,
...
RULE: to solve a triangle — list what is known; if it includes a side with
  its opposite angle, use the sine rule and check the ambiguous case by
  asking whether a second angle also fits; otherwise use the cosine rule
  to find a third side or an unknown angle; find the largest angle first
  where possible, since only it can be obtuse; finish with the angle sum
  and check that the biggest side faces the biggest angle.
ASKED-AS: solve a triangle which rule to use sine cosine given sides angles

ESSENCE: two sides and the angle between them give the area directly: half
  their product times the sine of that angle. It is the ordinary area
  formula with the sine supplying the height, and it needs no
...
ASKED-AS: area triangle half ab sin c two sides included angle formula why

ESSENCE: a periodic function is one that repeats: its value at any moment
  equals its value one period later, forever. Sine and cosine are the
  simplest such functions, and that is their real job — right triangles
...
ESSENCE: any simple wave is fixed by three dials. Amplitude is how far it
  swings, period is how long one repeat takes, and phase is where in the
...
THREAD: music (loudness, pitch and timing), physics (every oscillator
  ...
  and nothing more).
ASKED-AS: amplitude period phase shift wave graph describe height width start offset

ESSENCE: a trigonometric identity is one quantity written two ways. That
  sounds useless and is the opposite — the whole art is rewriting an
  expression into a form where the next step becomes possible, and the
...
ASKED-AS: trig identities what for why useful prove simplify rewrite same value different

ESSENCE: add two waves of the same frequency and you always get a single
  wave of that frequency — never a new one, only a change in size and
  starting point. That one fact is why the whole subject is manageable and
...
ASKED-AS: adding two waves same frequency single wave interference cancel louder phase combine

ESSENCE: anything that repeats can be built from sines and cosines, which
  is why trigonometry escaped geometry and colonised physics, music,
  medicine and engineering. It stopped being about triangles a long time
...
ASKED-AS: trigonometry why important waves sound signals repeating anything cycles fourier real uses

ESSENCE: a vector is an arrow: a size together with a direction, and no
  fixed home. Two arrows are the same vector if they are the same length
  and point the same way, wherever they are drawn — which is exactly what
  makes them addable.
...
ASKED-AS: vector arrow direction magnitude what is add scalar difference position moving

ESSENCE: vectors add tip to tail — walk the first, then the second from
  where you stopped, and the arrow from start to finish is the sum. The
  order makes no difference to where you end up, which is the same
...
THREAD: navigation (a ship's course plus the current's drift), physics
  ...
  giving track).
ASKED-AS: adding vectors tip to tail parallelogram resultant order swim current wind

ESSENCE: the dot product asks how much one arrow goes the other's way. It
  is the length of the shadow one casts along the other, times that
  other's length — and it is a plain number at the end, with the
  directions used up.
...
THREAD: physics (work done is force dotted with movement, so pushing
  ...
  to the source).
ASKED-AS: dot product what means projection shadow angle between work perpendicular zero number

ESSENCE: when the dot product is nothing, the arrows are square to each
  other — no agreement of direction at all. That turns a geometric
  question into one multiplication and a comparison, with no angles
...
ASKED-AS: dot product zero perpendicular right angle test vectors check square orthogonal

ESSENCE: the cross product of two arrows in space gives a third arrow,
  square to both, whose length is the area of the parallelogram they span.
  One operation carrying both an area and a direction, which is why it
...
ASKED-AS: cross product area perpendicular vector direction torque right hand rule order

ESSENCE: a matrix is bookkeeping for a transformation. Its columns are
  simply where the basic directions land, and everything else follows —
  because a transformation that respects addition is fully determined once
  those few arrows are placed.
...
ASKED-AS: matrix transformation columns where basis vectors go rotation scaling meaning what does

ESSENCE: the determinant tells you what a transformation does to area, and
  its sign tells you whether the shape got flipped. When it is nothing the
  transformation has flattened everything onto a line, and no return
  journey exists.
...
ASKED-AS: determinant area scale factor zero means flattened no inverse negative flip meaning

ESSENCE: doing one transformation then another is recorded by multiplying
  their matrices, and the order matters — rotate then stretch is a
  different result from stretch then rotate. The awkward multiplication
...
ASKED-AS: matrix multiplication order matters rotate then move different why weird rule

ESSENCE: transformations sort by what they leave alone. Rigid motions keep
  every distance; similarities keep every angle but not size; affine ones
  keep parallel lines parallel; projections keep only straightness. Each
...
ASKED-AS: transformations preserve distance angle parallel rigid similarity projection what stays same

ESSENCE: with only a straightedge and compasses, the lengths you can
  construct are exactly those reachable by adding, multiplying and taking
  square roots. Anything needing a cube root is out of reach — which is
...
ASKED-AS: trisect angle impossible ruler compass construction double cube proved why not
