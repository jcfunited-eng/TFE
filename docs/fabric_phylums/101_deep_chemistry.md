# 101 DEEP CHEMISTRY — what substances do
Middle floors under file 03: how fast a reaction goes, where it
stops, and how one substance is told from another. Still essences,
never data; every entry carries its cannot-twin.

ESSENCE: the table is a map with directions, not a list — rightward
  along a row atoms shrink and grip harder, downward a column they
...
ESSENCE: a column is a ranked family, not a set of twins — the same
  habit turned up or down by distance, since each step down holds
...
ESSENCE: an ion is an atom that has paid or been paid an electron
  and now carries charge; water pulls a salt crystal apart not by
  force but by crowding — lopsided water molecules swarm each ion
...
ESSENCE: acid and base trade one thing — a bare hydrogen — and
  strength is not how much you hold but how completely you let go:
  a strong acid hands over every proton at once, a weak one keeps
...
ESSENCE: pH counts zeros, not amounts — each step of one is a
  tenfold change, so tidy small numbers hide enormous distances:
  stomach at 2 and blood at 7.4 stand more than a hundred thousand
  times apart.
...
ESSENCE: a buffer is a pair kept side by side — a weak acid and its
  own leftover partner — so arriving acid is eaten by one and
...
ESSENCE: oxidation is losing electrons and reduction is gaining
  them, and they are one transaction with two names — nothing gives
  unless something takes, same instant, same amount. Oxygen is only
...
ESSENCE: burning, rusting and breathing drop the same fuel to the
  same floor; what differs is the staircase — fire falls in one
  step and throws the payment away as light, rust falls a grain at
  a time behind its own skin, the body walks down a ladder and
...
ESSENCE: a catalyst works by holding both partners in one exact
  grip, so it is shape-fussy and easily poisoned — one molecule
  that fits the seat and refuses to leave stops the whole vat, and
  a trace does it.
...
ESSENCE: a settled reaction is busy, not stopped — traffic runs at
  full speed both ways and only the count stands still; mark the
  atoms on one side and they turn up on the other while the numbers
  never move.
...
ESSENCE: push a settled reaction and it leans away from the push —
  which industry does not merely tolerate but exploits: steal the
  product as it forms and the balance keeps leaning your way, so a
...
ESSENCE: reactions happen where particles meet, so the rate rides
  on the crowd — and every reaction eats its own crowd as it runs,
  slowing itself; the last tenth always takes longer than the first
  nine.
...
ESSENCE: heat does not push every molecule over the threshold, it
  widens the spread — only the fast tail of the crowd is over the
  line, and a small warming moves a large share across it, so rate
...
ESSENCE: where two different states of matter react, only the
  boundary can work — so the surface is the entire capacity, and
  grinding a lump multiplies that boundary thousandfold: flour and
...
ESSENCE: every solvent has a ceiling for every guest at every
  temperature, and the ceiling is itself a truce — dissolving and
  undissolving running at matched speeds along the leftover grains
  at the bottom.
...
ESSENCE: past the ceiling the extra must leave, and it leaves in
  order — a crystal grows by accepting one right-shaped particle at
  a time onto a face and turning the wrong ones away, so slow
...
ESSENCE: a mixture keeps its parts' tempers and can be sorted by
  any difference they still have; a compound has a new temper and
  can be undone only by a reaction — sodium explodes in water,
  chlorine kills, and their compound sits on every table.
...
THREAD: cooking (a salad can be picked apart, a cake cannot),
  ...
  quality).
ASKED-AS: salt sodium chlorine mixture blend alloy separate filter sieve magnet salad cake

ESSENCE: boiling separates by eagerness to leave — the vapour above
  a mixture is always richer in the easier-boiling part and never
  pure, so one pass improves and no pass finishes; a tall column is
...
ESSENCE: let a mixture race along a standing material while a
  liquid pushes it, and each substance is held back by how much it
  prefers sitting to travelling — nothing is sorted by size or
...
ESSENCE: a polymer is one small unit repeated, and length alone —
  with no change of ingredient — decides what it is: short chains
  slide past each other as oil and wax, long chains tangle and grip
...
ESSENCE: plastic lasts not because its bonds are unbreakable but
  because nothing alive has a mouth for it — rot is a service
  performed by living things, and they eat only what their
  ancestors met.
...
ASKED-AS: plastic bottle bag landfill breaks down biodegradable ocean microplastic forever compost rot

ESSENCE: carbon makes four strong bonds and makes them to itself as
  gladly as to anything else, so it builds chains, rings and
  branches of unlimited length that still hold together at the
...
ESSENCE: a molecule's behaviour lives in the small clusters hung
  off it, not in its bulk — the carbon frame is scaffolding and the
  group is the business end; change the group and the substance
...
ESSENCE: two substances can hold the very same atoms in the very
  same numbers and be different things — the arrangement is not a
  detail of the substance, it is the substance; a formula names
...
ESSENCE: many molecules come in left- and right-handed versions,
  alike in weight, colour and melting point, differing only in how
  they meet other handed things — and a body is built entirely of
...
ESSENCE: soap's two-handed molecules do little alone — above a
  certain crowding they suddenly assemble into tiny spheres, oily
  hands inward around the dirt and watery hands outward, and the
...
ESSENCE: dissolving is bookkeeping, not affection — a solvent must
  break its own grip on itself and the guest's grip on itself, and
  pay for both out of the new grips it forms; water's own grip is
...
ESSENCE: a smell is not in the molecule — it is the report of which
  of a few hundred receptor shapes the molecule happened to fit,
  read as a pattern across all of them at once, the way a colour is
...
ESSENCE: chemistry happens by count and the bench works by weight,
  so the whole trade runs on an exchange rate — a fixed enormous
  number of particles, weighed out as grams — because atoms are far
...
ASKED-AS: grams weigh scale how much atoms count number recipe amount measure convert particles

ESSENCE: a reaction stops when the first partner runs out, and not
  a moment later — whatever else is in the pot is inventory, not
...
ASKED-AS: ran out short missing ingredient leftover extra waste enough limit stops yield

ESSENCE: breaking bonds costs and making them pays, so a reaction's
  heat is only the difference between two ledgers — which is why
  some real reactions run backwards on that count and get cold,
...
ASKED-AS: cold pack hand warmer heat pack instant ice bag reaction warm cools chemical gets

ESSENCE: strong bonds hold a molecule's frame and weak ones do its
  thinking — a grip that breaks at body temperature is the only
  kind a living thing can use for a decision, because a decision
...
ASKED-AS: stick lightly hold tight strong weak grip velcro glue temporary permanent letting go

ESSENCE: a gas has no shape and no size of its own — it takes the
  container and pushes on every wall of it, and the push, the room
  and the warmth are one tied triple: squeeze it and it heats,
  warm it in a sealed vessel and the push climbs with nowhere left
  to go.
...
ASKED-AS: pressure cooker aerosol can heat spray bottle tyre pump bang sealed jar bursting

ESSENCE: boiling is a contest with the air above — a liquid boils
  where its own outward push equals the pressure pressing down, so
  the boiling temperature belongs to the pair and not to the
  liquid alone.
...
ASKED-AS: mountain altitude boiling water cooking pasta longer pressure cooker denver bubbles hot lid

ESSENCE: anything dissolved gets in the way of freezing and clings
  against boiling, so salt water freezes below zero and boils above
  a hundred — and the effect counts particles, caring nothing for
  what they are.
...
ASKED-AS: salt ice road grit antifreeze car radiator sea freeze cold water boiling faster

ESSENCE: left alone, molecules spread from crowded to empty by
  chance alone — but the time it takes grows with the square of the
  distance, so spreading crosses a cell in an instant and a room in
  days.
...
ASKED-AS: smell spreads room perfume tea bag colour water mixes slowly stir fan marinade

ESSENCE: some reactions make their own successor — one reactive
  fragment attacks a neighbour and creates another fragment in the
  act, so a single starting event runs thousands of steps before it
  is stopped.
...
THREAD: crowds (a rumour makes rumourers), disease (each case makes
  ...
  one bad start).
ASKED-AS: rancid antioxidant vitamin preservative spreads spoils chain fire retardant stops once started

ESSENCE: fire stands on three legs — fuel, an electron-taker, and
  heat enough to keep paying the entry price — and cutting any one
  of the three puts it out, which is why there is no single right
  way to fight one.
...
ASKED-AS: fire extinguisher water oil pan lid smother blanket put out flames grease kitchen

ESSENCE: you cannot argue a metal out of wanting oxygen, so you
  offer a more willing victim — a skin of zinc, or a lump of it
  bolted to a hull, is eaten first and keeps the iron whole while
  any of it remains.
...
ASKED-AS: galvanised zinc rust boat hull anode roof nails coating protect iron sacrificial

ESSENCE: everything on the chemistry floor is electrons trading —
  and the nucleus, where the atom's identity and its radioactivity
  live, is untouched by all of it; no heat, reagent or catalyst
  reaches down there.
...
ASKED-AS: radiation nuclear waste dangerous chemicals neutralise clean up radioactive years decay safe treat

ESSENCE: a pure metal is soft because its layers of atoms slide
  over each other in neat planes — mix in atoms of the wrong size
  and the planes jam, so a small deliberate impurity is what makes
  a metal hard.
...
ASKED-AS: steel bronze brass alloy pure gold carat soft hard mix metal strong bends

ESSENCE: glass is a liquid caught standing — cooled faster than its
  molecules could arrange themselves, so it froze in a jumble and
...
ASKED-AS: glass window bottle obsidian hard candy shatter crack melt soften blow sharp

ESSENCE: between dissolved and settled there is a third state —
  particles too large to dissolve and too small to fall, held up by
  the jostling of the liquid itself: milk, fog, smoke, ink, paint,
  blood.
...
ASKED-AS: milk cloudy fog smoke paint ink muddy water clear see through settles cream

ESSENCE: a substance's colour is a readout of its bonding — it
  swallows exactly the light whose energy lifts one of its
  electrons, and the gap that must be crossed is set by how the
...
ASKED-AS: litmus test strip colour change red blue cabbage indicator dye paint why coloured
