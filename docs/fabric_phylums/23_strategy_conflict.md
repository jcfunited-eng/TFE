# 23 STRATEGY AND CONFLICT — the physics of opposed wills
Soft field: the other side thinks back, so laws here are pressures,
not clockwork. Principles at Sun Tzu's altitude — described, never
advocated; nothing here is a manual.

ESSENCE: surprise is capital spent in the using — an unexpected
  move works once, because being used is exactly how the other
  side learns it.
...
ASKED-AS: ambush fake feint gimmick novelty wears off predictable caught adapting counter

ESSENCE: over time supply beats valor — the side that stays fed,
...
THREAD: sports (titles go to benches and conditioning), business
  ...
  fluids).
ASKED-AS: logistics food fuel ammo war attrition endurance stamina starve siege marathon reserves

ESSENCE: morale is the real currency — most contests end when one
...
THREAD: sports (a broken team loses to a worse one), markets (a
  ...
  fall inward first).
ASKED-AS: quit giving up spirit confidence surrender rout fear momentum retreat hope

ESSENCE: the defender holds the cheaper hand — knowing the ground,
  waiting prepared, needing only not-to-lose, while the attacker
...
ASKED-AS: castle fort walls trench moat siege invasion terrain advantage counterpunch

ESSENCE: deception is expectation management — no one is made to
  believe a lie; they are fed the belief they already prefer. The
...
THREAD: magic (misdirection is rented attention), fraud (every
  ...
  caller counting odds instead of faces).
ASKED-AS: scam fooled gullible swindle hoax phishing catfish disguise wishful cheat

ESSENCE: every plan dies on first contact — and planning still
  pays, because the planner has walked the terrain of
...
ASKED-AS: backup prepare rehearse improvise adapt flexible checklist schedule derailed wrong blueprint

ESSENCE: escalation is a ratchet — each answer must slightly
  exceed the last insult or read as surrender, so conflicts climb
...
ESSENCE: battles are mostly decided before they are fought — by
  who chose the ground, the timing, and whether to fight at all;
...
ASKED-AS: pick avoid walk away fold quit setup preparation matchup when where

ESSENCE: strength spread is strength spent — force decides only
  where it is gathered, so the art is accepting weakness almost
...
ASKED-AS: concentrate priorities thin multitask overextended divide effort narrow single aim punch
