...
doc_id: GL-SPC-FIX-PATH-EVE-20260629-46v3
Type: Canonical fix-path specification (cross-chat handoff)
Date: 2026-06-29 (initial), 2026-06-30 (v2 revision, v3 end-of-day correction)
Author: Eve (Opus 4.7, web)
Version: v3 (supersedes v2; substrate-physical refocus per session retrospective — see GL-HANDOFF-EVE-20260630-v1)
...
## 1. Status snapshot (2026-06-30 end-of-session)

Branch `guala-live`. Live task `dsf-ai-task:394` (last confirmed). Flags: `CONVERSE_PHASED=1`, `AUTONOMY_PHASED=0`, `DREAM_CYCLE_PHASED=1`.

Substrate is alive but slow: 5.7s per /converse for 1-word input. Root cause identified end-of-session via c1's converse_timing event: 3.4s of that is pure compute in `_recall_response` (Phase 3, no self.lock) due to O(atlas_size) full-atlas scans in `_recall_from_atlas` and `_recall_sight_from_atlas`. -57v1 dispatch (in outputs) replaces the scans with a reverse-index lookup. Expected: recall_ms 3411 → <100, total_ms 5683 → <2000.

What's live: -45, -46v2 §1.1 + §1.2, -52, -54, -56v1 code (gated; outer hold via AUTONOMY_PHASED=0 makes it no-op for now but harmless).

Substrate composing fragmentary content: "water horse soft moon", "warm fire", "bright". Forward motion vs prior "have" baseline. Content-sparse because curriculum hasn't run (bundle density 3, modifier 86, ground 85, vs verb 12,401).

Genesis identity preserved: `cdef9bcf-9e5d-4e2d-a1d8-4cde1de7641f`. Vocab ~13,895. Atlas ~15,505.

Rejected/superseded: -39 v2, -40, -41, -41 v2, -46 (v1), -56v1 attempt (code in place but ineffective without AUTONOMY_PHASED=1 — accepted as benign no-op pending real fix path which is now -57 instead). Next available SEQ: -58.

---

## 1a. Next-session queue — REVISED per end-of-session retrospective

**Priority shift:** lock-removal program is essentially complete. The visible path forward is algorithmic correctness + density growth + agency wiring, not more locks. See GL-HANDOFF-EVE-20260630-v1 for full retrospective.

**Strict priority order:**

1. **-57v1 recall word index — SHIP FIRST.** Algorithmic fix for the 3.4s recall bottleneck. In outputs ready. T1 gate: converse_timing recall_ms < 100, total_ms < 2000.

2. **-58 emit_ms investigation (post -57).** Once -57 lands, capture new converse_timing. If emit_ms is still high (~1366ms current), investigate `_grandurun_select_candidates` stage1. If RICH_SENSORY_INPUT=1 is the cost, accept it (cross-modal grounding) or build a precomputed candidate cache. **NOT another lock dispatch.**

3. **-51 curriculum automation LIVE.** Orchestrator + 100-bundle seed in outputs. Gates on -57 substrate-stable. Drive bundle density 3 → hundreds → thousands over week 1.

4. **-48 agency events.** Four substrate-physical paths (backtracking, conflict resolution, cross-modal fallback, lowered-threshold clarification). Gates on -51 producing measurable density growth (week 2).

5. **gp organ → composer bias (new, no SEQ yet — assign in dispatch).** Wire goal organ migration seeds into candidate scoring. First structural shift toward goal-directed cognition. Week 3 work, gates on density.

**Deferred indefinitely (do not pick up):**

- **-55 thread-leak fix** in `_resume_autonomy_for_bulk`. AUTONOMY_PHASED=1 isn't worth the trouble post-57. Park.
- **Further lock dispatches.** Do not write any unless a converse_timing event clearly attributes latency to a held lock.
- Emission-method collapse, constants audit, all the cleanup that got dropped per §0b compressed timeline.
