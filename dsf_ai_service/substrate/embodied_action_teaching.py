...
EMBODIED_ACTION_TEACHING_CONSUMER_ID = "embodied-action-teaching"
...
class EmbodiedActionTeachingAuthority:
    ...
    def _open_custody(
        ...
        *,
        ...
        if not isinstance(authority, SettledExperienceCustodyAuthority):
            raise TypeError(
                "demonstration requires settled experience custody authority"
            )
        if not isinstance(capability, SettledExperienceConsumerCapability):
            raise TypeError(
                "demonstration requires a typed custody capability"
            )
        if capability.consumer_id != EMBODIED_ACTION_TEACHING_CONSUMER_ID:
            raise ValueError(
                "demonstration custody belongs to another consumer"
            )
        view = authority.open_child(capability)
        if (
            view.source_kind
            is not SettledExperienceSourceKind.PHYSICAL_EVIDENCE
            or view.physical_evidence_receipt is None
            or view.self_acoustic_receipt is not None
        ):
            raise ValueError(
                "demonstration requires one physical W1 occurrence"
            )
        return view
    ...
    def _receipt_for(
        ...
        *,
        ...
        pre_outcome: SettledExperienceConsumerView,
        post_outcome: SettledExperienceConsumerView,
    ...
    def demonstrate(
        ...
        *,
        ...
        pre_custody_authority: SettledExperienceCustodyAuthority,
        pre_custody_capability: SettledExperienceConsumerCapability,
        post_custody_authority: SettledExperienceCustodyAuthority,
        post_custody_capability: SettledExperienceConsumerCapability,
    ) -> GuidedActionBinding:
        ...
        if encode_command(decoded_command) != command_payload:
            raise ValueError("demonstration command is not canonical W1 bytes")
        pre_outcome = self._open_custody(
            pre_custody_authority,
            capability=pre_custody_capability,
        )
        post_outcome = self._open_custody(
            post_custody_authority,
            capability=post_custody_capability,
        )
        execution_receipt = post_outcome.world_execution
        if execution_receipt is None:
            raise ValueError(
                "demonstration post-custody is not an applied occurrence"
            )
        ...
        if (
            pre_outcome.world_execution is not None
            or pre_outcome.world_observation.authority_receipt_sha256
            != execution_receipt.before.authority_receipt_sha256
            or post_outcome.world_observation.authority_receipt_sha256
            != execution_receipt.after.authority_receipt_sha256
            or pre_outcome.source_occurrence_id
            == post_outcome.source_occurrence_id
        ):
            raise ValueError(
                "demonstration custodies name different world evidence"
            )
        ...
        with self._lock:
            ...
            cycle_snapshot = self._cycle.transaction_snapshot()
            try:
                ...
                if (
                    ...
                    raise RuntimeError(
                        "causal cycle did not retain demonstration authority"
                    )
                self._records = candidate
            except BaseException:
                self._cycle.restore_transaction(cycle_snapshot)
                raise
            return guided
