...
class ThermallyCoupledEmbodimentWorldAuthority(EmbodimentWorldAuthority):
    ...
    def admit_authored_departure(self, object_id: str) -> str:
        """Atomically bind one authored departure (the bin) to thermal custody."""

        with self._thermal_lock:
            self._require_no_physical_return_for_renovation()
            state_sha = super().admit_authored_departure(object_id)
            observation = super().observation_snapshot()
            self._thermal_world_revision = observation.revision
            self._thermal_world_observation_receipt_sha256 = (
                observation.authority_receipt_sha256
    ...
    def admit_authored_body_transport(self, body_id: str, target_pose) -> str:
        """Atomically bind one authored body transport to thermal custody."""

        with self._thermal_lock:
            self._require_no_physical_return_for_renovation()
            state_sha = super().admit_authored_body_transport(body_id, target_pose)
            observation = super().observation_snapshot()
            self._thermal_world_revision = observation.revision
            self._thermal_world_observation_receipt_sha256 = (
                observation.authority_receipt_sha256
