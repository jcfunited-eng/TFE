...
def _resolve_tie(bounds: tuple[int, int], interval: _Interval) -> int | None:
    """Settle an exact half-millimetre knife-edge tie away from zero."""
    if bounds[0] == bounds[1]:
        return bounds[0]
    if bounds[1] - bounds[0] == 1:
        tie = Fraction(bounds[0] * 2 + 1, 2)
        if interval[0] <= tie <= interval[1]:
            return bounds[1] if tie > 0 else bounds[0]
    return None
...
def rotate_lattice_offset(
    ...
    for term_count in _PROOF_TERM_COUNTS:
        ...
        if term_count == _PROOF_TERMS:
            x_res = _resolve_tie(x_bounds, rotated_x)
            y_res = _resolve_tie(y_bounds, rotated_y)
