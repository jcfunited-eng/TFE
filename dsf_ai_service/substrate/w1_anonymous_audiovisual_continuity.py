...
CONTINUITY_AUTHORITY_SCHEMA = (
    "guala.w1.anonymous_audiovisual_continuity.authority.v2"
)
CONTINUITY_AUTHORITY_DOMAIN = (
    b"guala.w1.anonymous_audiovisual_continuity.authority.v2\0"
)
LINEAGE_AUTHORITY_DOMAIN = (
    b"guala.w1.anonymous_audiovisual_lineage.authority.v1\0"
)
SNAPSHOT_SCHEMA = "guala.w1.anonymous_audiovisual_continuity.snapshot.v2"
SNAPSHOT_AUTHORITY_DOMAIN = (
    b"guala.w1.anonymous_audiovisual_continuity.snapshot.v2\0"
)
PHYSICAL_CONTINUITY_CUSTODY_DOMAIN = (
    b"guala.w1.anonymous_audiovisual_continuity.physical-custody.v1\0"
)
...
def _physical_custody_payload(
    ...
    return {
        ...
        "correspondence_object_identity": id(correspondence),
        "evidence_authority_receipt_sha256": (
            evidence_receipt.authority_receipt_sha256
        ),
        "evidence_object_identity": id(evidence_receipt),
    }


def physical_continuity_custody_hmac(
    physical_authority_key: bytes | str,
    correspondence: W1AnonymousAcousticVisualCorrespondence,
    evidence_receipt: W1PhysicalEvidenceReceipt,
) -> str:
    """Authenticate exact request-local objects already verified by physics."""
    key = _key(
    ...
    return hmac.new(
        key,
        PHYSICAL_CONTINUITY_CUSTODY_DOMAIN
        + _canonical(_physical_custody_payload(
            correspondence,
            evidence_receipt,
        )),
        hashlib.sha256,
...
def _verify_serialized_full_fields(correspondence: dict[str, object]) -> None:
    ...
    compact_text = (
        auditory.get("compact_full_field_base64")
        if isinstance(auditory, dict) else None
    )
    if not isinstance(compact_text, str) or not compact_text:
        raise ValueError("continuity two-ear field changed")
    try:
        compact = decode_compact_auditory_field(
            base64.b64decode(compact_text, validate=True)
        )
    except Exception as error:
        raise ValueError("continuity two-ear field changed") from error
    if (
        len(compact.components) != MAX_COMPACT_AUDITORY_COMPONENTS
        or compact.authority_receipt_sha256
        != auditory.get(
            "compact_full_field_authority_receipt_sha256"
        )
        or compact.structural_fingerprint
        != auditory.get("structural_fingerprint")
    ):
        raise ValueError("continuity two-ear field changed")
...
def _verify_correspondence_record(
    ...
    auditory_authority = record.pop("auditory_l5_authority", None)
    ...
    if (
        not isinstance(auditory_authority, dict)
        or record.get("auditory_l5_authority_receipt_sha256")
        != auditory_authority.get("authority_receipt_sha256")
    ):
        raise ValueError("continuity two-ear authority reference changed")
    persisted_record = {
        **record,
        "auditory_l5_authority": auditory_authority,
    }
    _verify_serialized_full_fields(persisted_record)
    forbidden_keys = {
        "body_id",
        "object_id",
        "port_id",
...
class W1AnonymousAudiovisualContinuityExperience:
    ...
    def authority_payload(self) -> dict[str, object]:
        return {
            "correspondence_authority_receipt_sha256": (
                self.correspondence.authority_receipt_sha256
            ),
    ...
    def _verify_authority(self, authority_key: bytes | str) -> None:
        """Verify local authority while nested objects remain in custody."""
        key = _key(authority_key, "continuity authority key")
        ...
        return {
            **self.authority_payload(),
            "correspondence_authority": (
                self.correspondence.authority_record()
            ),
...
class W1AnonymousAudiovisualContinuityOwner:
    ...
    def prepare(
        ...
        evidence_receipt: W1PhysicalEvidenceReceipt,
        *,
        physical_custody_hmac_sha256: str | None = None,
        ...
        if not isinstance(evidence_receipt, W1PhysicalEvidenceReceipt):
            raise TypeError("typed physical evidence receipt is required")
        if physical_custody_hmac_sha256 is None:
            correspondence.verify(self._physical_key)
            evidence_receipt.verify(self._physical_key)
        else:
            _sha256(
                physical_custody_hmac_sha256,
                "continuity physical custody HMAC",
            )
            expected_custody_hmac = physical_continuity_custody_hmac(
                self._physical_key,
                correspondence,
                evidence_receipt,
            )
            if not hmac.compare_digest(
                physical_custody_hmac_sha256,
                expected_custody_hmac,
            ):
                raise ValueError(
                    "continuity physical input custody changed"
                )
        if (
    ...
    def restore_encoded(self, encoded: bytes) -> None:
        ...
        if latest is not None:
            ...
            correspondence_authority = latest_record.pop(
                "correspondence_authority", None
            )
            ...
            if (
                ...
                != correspondence_record.get(
                    "authority_receipt_sha256"
                )
            ):
                raise ValueError(
                    "anonymous continuity correspondence reference changed"
                )
