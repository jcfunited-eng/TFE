...
AUDITORY_L5_SCHEMA = "guala.auditory_l5.full_field.v4"
AUDITORY_L5_COMPONENT_SCHEMA = "guala.auditory_l5.kernel_component.v2"
AUDITORY_L5_PAIR_SCHEMA = "guala.auditory_l5.cochlear_pair.v2"
AUDITORY_L5_AUTHORITY_SCHEMA = "guala.auditory_l5.authority.v4"
AUDITORY_L5_AUTHORITY_PROFILE = b"guala.auditory_l5.authority.profile.v4"
...
def _validate_component(component: AuditoryL5KernelComponent) -> None:
    ...
    for index, sample in enumerate(component.samples):
        ...
        relevance = float(sample.relevance)
        if not math.isfinite(relevance) or not 0.0 <= relevance <= 1.0:
            raise ReceiptError(
                "auditory L5 relevance left its physical interval"
            )
        if component.kind is AuditoryL5ComponentKind.PRESSURE:
            ...
            if sample.relevance != sample.signal ** 2:
                raise ReceiptError(
                    "auditory L5 relevance differs from squared pressure amplitude"
                )
            if sample.phase_turns != 0:
                raise ReceiptError("auditory L5 pressure component phase is not zero")
        else:
            _binary64(sample.phase_turns, "auditory L5 cumulative carrier phase")
...
def _validate_channel(channel: AuditoryL5CochlearChannel) -> None:
    ...
    for component, topology_index, substream_id, quantity, unit in expected:
        if (
            ...
            raise ReceiptError("auditory L5 kernel mount is not canonical v4")
        _validate_component(component)
    if _component_grid(pressure) != _component_grid(phase):
        raise ReceiptError("auditory L5 cochlear component grids differ")
    if tuple(
        value.relevance for value in pressure.samples
    ) != tuple(
        value.relevance for value in phase.samples
    ):
        raise ReceiptError(
            "auditory L5 cochlear pair relevance diverged"
        )
    if (
...
def _sample_payload(sample: AuditoryL5ComponentSample) -> dict[str, object]:
    return {
        ...
        "relevance": _fraction_text(sample.relevance),
...
def _transaction_source_samples(
    built: BuiltSixSenseFullField,
    sound: SensoryFullFieldBoundary,
    substream: SensorySubstreamFullField,
) -> tuple[AuditoryL5ComponentSample, ...]:
    digest = substream.profile.physical_derivation_receipt_sha256
    native = built.source_native_input(digest)
    if (
        native.sense is not PhysicalSense.SOUND
        or native.sensor_id != substream.profile.sensor_id
        or native.substream_id != substream.profile.substream_id
        or native.topology_index != substream.profile.topology_index
        or native.coordinates != substream.profile.coordinates
        or native.physical_quantity
        != substream.profile.physical_quantity
        or native.physical_unit != substream.profile.physical_unit
    ):
        raise ReceiptError(
            "auditory L5 native source belongs to another component"
        )
    return tuple(
        AuditoryL5ComponentSample(
            ...
            source_time=source_time,
            causal_offset=source_time - sound.source_time_start,
            signal=Fraction.from_float(float(signal)),
            relevance=relevance,
...
def _build_component(
    ...
    *,
    ...
    component = AuditoryL5KernelComponent(
        ...
        samples=(
            _transaction_source_samples(
                built,
                sound,
                substream,
            )
            if transaction_verified
            else _source_samples(
...
def _verify_fresh_constructed_experience(
    *,
    experience: AuditoryL5Experience,
    built: BuiltSixSenseFullField,
    channels: tuple[AuditoryL5CochlearChannel, ...],
    component_and_pair_payloads: tuple[bytes, ...],
    authority_payload: bytes,
) -> None:
    """Verify one newly built immutable L5 graph without rebuilding payloads.

    The caller has just derived every component from ``built`` and retains the
    exact payload bytes used to issue every receipt. The transaction authority
    binds the upstream objects by identity. This verifier therefore checks
    each newly issued object-to-payload receipt exactly once instead of
    serializing the same complete field again through the public verifier.
    """
    built.verify_construction(
        boundary=experience.upstream_boundary,
        receipt_registry=experience.upstream_receipt_registry,
    ...
    if (
        experience.channels is not channels
        or experience.assembly_id != built.boundary.assembly_id
        or experience.assembly_receipt_sha256
        != built.boundary.authority_receipt_sha256
        or experience.event_boundary not in ("ambient", "utterance")
        or experience.relation not in (
            "first_observation",
            "recurrence",
            "structural_change",
        )
        or experience.source_time_start
        != sound.source_time_start
        or experience.source_time_end
        != sound.source_time_end
        or experience.receipt_registry.profile_binding_sha256
        != receipt_sha256(AUDITORY_L5_AUTHORITY_PROFILE)
    ):
        raise ReceiptError(
            "fresh auditory L5 construction left its verified transaction"
        )
    if len(component_and_pair_payloads) != (
        AUDITORY_KERNEL_COMPONENT_COUNT + COCHLEAR_CHANNEL_COUNT
    ):
        raise ReceiptError(
            "fresh auditory L5 receipt topology changed"
        )
    payload_index = 0
    for channel in channels:
        for component in (
            ...
            payload = component_and_pair_payloads[payload_index]
            payload_index += 1
            if (
                component.authority_receipt_sha256
                != receipt_sha256(payload)
                or experience.receipt_registry.resolve(
                    component.authority_receipt_sha256,
                    "fresh auditory L5 component authority",
                )
                != payload
            ):
                raise ReceiptError(
                    "fresh auditory L5 component receipt changed"
                )
        pair_payload = component_and_pair_payloads[payload_index]
        payload_index += 1
        if (
            channel.pair_receipt_sha256
            != receipt_sha256(pair_payload)
            or experience.receipt_registry.resolve(
                channel.pair_receipt_sha256,
                "fresh auditory L5 pair authority",
            )
            != pair_payload
        ):
            raise ReceiptError(
                "fresh auditory L5 pair receipt changed"
            )
    if (
        payload_index != len(component_and_pair_payloads)
        or experience.authority_receipt_sha256
        != receipt_sha256(authority_payload)
        or experience.receipt_registry.resolve(
            experience.authority_receipt_sha256,
            "fresh auditory L5 authority",
        )
        != authority_payload
    ):
        raise ReceiptError(
            "fresh auditory L5 authority receipt changed"
        )


@dataclass(frozen=True, slots=True)
class _AuditoryL5TransactionState:
    latest: AuditoryL5Experience | None
    pending: tuple[tuple[str, AuditoryL5Experience], ...]
    recent_experience_ids: tuple[str, ...]
...
class AuditoryL5Owner:
    ...
    def transaction_state(self) -> _AuditoryL5TransactionState:
        """Capture exact in-process state for one outer causal transaction."""

        with self._lock:
            return _AuditoryL5TransactionState(
                latest=self._latest,
                pending=tuple(self._pending.items()),
                recent_experience_ids=tuple(self._recent_experience_ids),
                transitions=tuple(self._transitions.items()),
                settled=self._settled,
            )

    def rollback_transaction_state(
        self,
        state: _AuditoryL5TransactionState,
    ) -> None:
        """Restore only a state issued by this exact transaction boundary."""

        if not isinstance(state, _AuditoryL5TransactionState):
            raise TypeError("auditory L5 rollback state is not typed")
        with self._lock:
            self._latest = state.latest
            self._pending = OrderedDict(state.pending)
            self._recent_experience_ids = OrderedDict(
                (experience_id, None)
                for experience_id in state.recent_experience_ids
            )
            self._transitions = OrderedDict(state.transitions)
    ...
    def settle(
        ...
        *,
        ...
        with self._lock:
            ...
            if transaction_verified:
                _verify_fresh_constructed_experience(
    ...
    def verify_transaction_linkage(
        self,
        *,
        built: BuiltSixSenseFullField,
        experience: AuditoryL5Experience,
    ) -> None:
        """Authenticate the exact L5 object freshly published by this owner."""
        if not isinstance(built, BuiltSixSenseFullField):
            raise TypeError(
                "auditory L5 transaction requires a built full field"
            )
        if not isinstance(experience, AuditoryL5Experience):
            raise TypeError(
                "auditory L5 transaction requires a typed experience"
            )
        built.verify_construction(
            boundary=experience.upstream_boundary,
            receipt_registry=experience.upstream_receipt_registry,
        ...
        with self._lock:
            if (
                self._latest is not experience
                or experience.assembly_id != built.boundary.assembly_id
                or experience.assembly_receipt_sha256
                != built.boundary.authority_receipt_sha256
            ):
                raise ReceiptError(
                    "auditory L5 transaction left its publishing owner"
                )
