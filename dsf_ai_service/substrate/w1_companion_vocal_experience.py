...
EPISODE_INTENT_SCHEMA = "guala.w1.companion_vocal_episode_intent.v2"
EPISODE_INTENT_DOMAIN = b"guala.w1.companion_vocal_episode_intent.v2\0"
EPISODE_SCHEMA = "guala.w1.companion_vocal_episode.v2"
EPISODE_AUTHORITY_SCHEMA = "guala.w1.companion_vocal_episode.authority.v2"
EPISODE_AUTHORITY_DOMAIN = b"guala.w1.companion_vocal_episode.authority.v2\0"
MAX_COMPANION_VOCAL_EPISODE_SAMPLES = 8 * 16_000
MAX_COMPANION_VOCAL_EPISODE_AUTHORITY_BYTES = 8 * 1024 * 1024
_EPISODE_COMMIT_UNDO_AUTHORITY = object()
_EPISODE_FINALIZATION_UNDO_AUTHORITY = object()
...
class CompanionVocalEpisodeIntentReceipt:
    companion_port_id: str
    world_observation_receipt_sha256: str
    causal_parent_receipt_sha256: str | None
    pcm_sha256: str
    ...
    def payload(self) -> dict[str, object]:
        return {
            ...
            "causal_parent_receipt_sha256": (
                self.causal_parent_receipt_sha256
            ),
            "companion_port_id": self.companion_port_id,
            "pcm_sha256": self.pcm_sha256,
    ...
    def verify(self, authority_key: bytes | str) -> None:
        ...
        _sha256(self.world_observation_receipt_sha256, "world observation")
        if self.causal_parent_receipt_sha256 is not None:
            _sha256(
                self.causal_parent_receipt_sha256,
                "companion episode causal parent",
            )
        ...
        if (
            ...
            or self.total_sample_count % OBSERVATION_HOP_SAMPLES
            or isinstance(self.block_count, bool)
            or not isinstance(self.block_count, int)
            or self.block_count != expected_blocks
            or (
                self.causal_parent_receipt_sha256 is not None
                and self.block_count != 1
            )
        ):
...
class W1CompanionVocalEpisodeBlock:
    ...
    anonymous_av_continuity_receipt_sha256: str | None
    ...
    def verify(self) -> None:
        ...
        if self.anonymous_av_continuity_receipt_sha256 is not None:
            _sha256(
                self.anonymous_av_continuity_receipt_sha256,
                "companion episode audiovisual continuity",
            )
        if bool(self.anonymous_av_correspondence) != bool(
            self.anonymous_av_continuity_receipt_sha256
        ):
            raise ValueError(
                "companion episode source relation lost continuity"
            )
        if (
            ...
            or (
                ...
                and (
                    ...
                    .causal_settlement_receipt_sha256
                    != self.causal_settlement_receipt_sha256
                    or self.anonymous_av_correspondence.auditory_l5
                    != self.binaural_l5
                )
            )
...
class PreparedCompanionVocalPredictionBlock:
    execution_receipt: ActionExecutionReceipt
    physical_mount: W1PhysicalEvidenceMount = field(repr=False)

    @property
    def causal_settlement(self) -> CausalExperienceSettlement:
        settlement = self.physical_mount.causal_settlement
        if settlement is None:
            raise ValueError(
                "companion prediction block lost its causal settlement"
            )
        return settlement

    @property
    def evidence_receipt(self) -> W1PhysicalEvidenceReceipt:
        receipt = self.physical_mount.evidence_receipt
        if receipt is None:
            raise ValueError(
                "companion prediction block lost its physical evidence"
            )
        return receipt

    @property
    def anonymous_av_continuity(
        self,
    ) -> W1AnonymousAudiovisualContinuityExperience | None:
        return self.physical_mount.anonymous_av_continuity
...
class _EpisodeFinalizationState:
    phase: str = "publishing"


@dataclass(frozen=True, slots=True)
class CompanionVocalEpisodeFinalizationUndo:
    commit_undo: CompanionVocalEpisodeCommitUndo
    _state: _EpisodeFinalizationState = field(
        repr=False,
        compare=False,
    )
    _owner_authority: object = field(repr=False, compare=False)
...
class W1CompanionVocalExperienceAuthority:
    ...
    def __init__(
        ...
        *,
        ...
        self._episode_finalization_undo: (
            CompanionVocalEpisodeFinalizationUndo | None
        ) = None
        self._episode_undo_owner = object()
    ...
    def _sample_count(pcm_s16le: bytes) -> int:
        ...
        if count % OBSERVATION_HOP_SAMPLES:
            raise ValueError(
                "companion pressure does not close an exact auditory hop"
            )
        return count
    ...
    def _episode_blocks(pcm_s16le: bytes) -> tuple[bytes, ...]:
        ...
        if sample_count % OBSERVATION_HOP_SAMPLES:
            raise ValueError(
                "companion episode does not close an exact auditory hop"
            )
        block_count = (
            sample_count + MAX_EMITTED_PCM_SAMPLES - 1
        ) // MAX_EMITTED_PCM_SAMPLES
        full_blocks = block_count - 1
        counts = (
            (MAX_EMITTED_PCM_SAMPLES,) * full_blocks
            + (
                sample_count
                - full_blocks * MAX_EMITTED_PCM_SAMPLES,
            )
        )
        if any(
            value < MIN_EMITTED_PCM_SAMPLES
            or value > MAX_EMITTED_PCM_SAMPLES
            or value % OBSERVATION_HOP_SAMPLES
            for value in counts
        ):
    ...
    def _issue_episode_intent(
        ...
        *,
        ...
        result = CompanionVocalEpisodeIntentReceipt(
            ...
            causal_parent_receipt_sha256=(
                provisional.causal_parent_receipt_sha256
            ),
    ...
    def prepare_episode(
        ...
        *,
        ...
        if (
            causal_parent_receipt_sha256 is not None
            and len(pcm_blocks) != 1
        ):
            raise ValueError(
                "companion episode causal parent requires exactly one "
                "physical block"
            )
        with self._lock:
            if (
                ...
                or self._episode_finalization_undo is not None
            ):
            ...
            try:
                ...
                for sequence, pcm_block in enumerate(pcm_blocks):
                    ...
                    if sequence == 0 and before != world_before:
                        raise RuntimeError(
                            "companion vocal episode starting world changed "
                            "after its signed intent"
                        )
                    ...
                    if (
                        ...
                        or (
                            sequence == 0
                            and execution.before.authority_receipt_sha256
                    ...
                    if (
                        ...
                        raise RuntimeError(
                            ...
                            f"{mounted.state.value}:{mounted.reason}"
                        )
                    ...
                    episode_blocks.append(W1CompanionVocalEpisodeBlock(
                        ...
                        anonymous_av_continuity_receipt_sha256=(
                            ...
                            if mounted.anonymous_av_continuity is not None
                            else None
                        ),
                        anonymous_av_correspondence=(
    ...
    def _require_prepared_episode(
        ...
        if (
            len(current.prediction_blocks) != len(current.episode.blocks)
            or current.episode.intent_authority_receipt_sha256
            != current.intent_receipt.authority_receipt_sha256
            or current.episode.world_before_receipt_sha256
            != current.intent_receipt.world_observation_receipt_sha256
        ):
        ...
        for block, prediction in zip(
            ...
            self._physical.verify_mount(prediction.physical_mount)
            if (
                block.world_execution_receipt_sha256
                != prediction.execution_receipt.authority_receipt_sha256
                or prediction.execution_receipt.causal_intent_receipt_sha256
                != current.intent_receipt.authority_receipt_sha256
                or (
                    prior_after is None
                    and prediction.execution_receipt.before
                    .authority_receipt_sha256
                ...
                or (
                    ...
                    and prediction.execution_receipt.before != prior_after
                )
                ...
                or block.binaural_l5
                is not prediction.physical_mount.binaural_auditory_l5
                or block.anonymous_av_continuity_receipt_sha256
                != (
                    prediction.anonymous_av_continuity
                    .authority_receipt_sha256
                    if prediction.anonymous_av_continuity is not None
                    else None
                )
            ):
                raise ValueError(
            ...
            prior_after = prediction.execution_receipt.after
        return current

    def verify_episode_intent(
        self,
        intent: CompanionVocalEpisodeIntentReceipt,
    ) -> None:
        """Verify one episode intent without disclosing the authority key."""

        if not isinstance(intent, CompanionVocalEpisodeIntentReceipt):
            raise TypeError("typed companion vocal episode intent is required")
        intent.verify(self._key)

    def episode_intent_record(
        self,
        intent: CompanionVocalEpisodeIntentReceipt,
    ) -> dict[str, object]:
        """Return one verified signed intent record without raw pressure."""

        self.verify_episode_intent(intent)
        return {
            **intent.payload(),
    ...
    def episode_intent_from_record(
        self,
        record: Mapping[str, object],
    ) -> CompanionVocalEpisodeIntentReceipt:
        """Reconstruct and verify one exact signed intent record."""

        if not isinstance(record, Mapping):
            raise TypeError(
                "companion vocal episode intent record must be a mapping"
            )
        expected = {
        ...
        if (
            set(record) != expected
            or record.get("schema") != EPISODE_INTENT_SCHEMA
            or record.get("sample_rate_hz") != 16_000
        ):
            raise ValueError(
                "companion vocal episode intent record shape changed"
            )
        intent = CompanionVocalEpisodeIntentReceipt(
            companion_port_id=record["companion_port_id"],
            world_observation_receipt_sha256=(
                record["world_observation_receipt_sha256"]
            ),
            causal_parent_receipt_sha256=(
                record["causal_parent_receipt_sha256"]
            ),
            pcm_sha256=record["pcm_sha256"],
            total_sample_count=record["total_sample_count"],
    ...
    def commit_episode(
        ...
        with self._lock:
            ...
            return CompanionVocalEpisodeCommitUndo(
                ...
                _owner_authority=self._episode_undo_owner,
                _construction_authority=(
                    _EPISODE_COMMIT_UNDO_AUTHORITY
                ),
            )
    ...
    def rollback_committed_episode(
        ...
        with self._lock:
            if (
                ...
                is not _EPISODE_COMMIT_UNDO_AUTHORITY
                or undo._owner_authority is not self._episode_undo_owner
    ...
    def commit_episode_finalization(
        self,
        undo: CompanionVocalEpisodeCommitUndo,
    ) -> CompanionVocalEpisodeFinalizationUndo:
        """Remove the transaction latch while retaining publish rollback."""
        ...
        with self._lock:
            if (
                ...
                or self._prepared_episode is not None
                or self._episode_finalization_undo is not None
                or self._latest_episode != undo.prepared.episode
            ):
                raise ValueError(
                    "companion episode finalization custody changed"
                )
            result = CompanionVocalEpisodeFinalizationUndo(
                commit_undo=undo,
                _state=_EpisodeFinalizationState(),
                _owner_authority=self._episode_undo_owner,
                _construction_authority=(
                    _EPISODE_FINALIZATION_UNDO_AUTHORITY
                ),
            )
            self._latest_episode = None
            self._episode_finalization_undo = result
            return result

    def _verify_episode_finalization_undo(
        self,
        undo: CompanionVocalEpisodeFinalizationUndo,
    ) -> None:
        if (
            not isinstance(
                undo,
            ...
            or undo._construction_authority
            is not _EPISODE_FINALIZATION_UNDO_AUTHORITY
            or undo._owner_authority is not self._episode_undo_owner
            or self._episode_finalization_undo is not undo
            or undo._state.phase != "publishing"
            or self._latest_episode is not None
        ):
            raise ValueError(
                "companion episode publication rollback changed"
            )

    def rollback_episode_finalization(
        self,
        undo: CompanionVocalEpisodeFinalizationUndo,
    ) -> None:
        """Undo the authentic committed episode after publication failure."""

        with self._lock:
            self._verify_episode_finalization_undo(undo)
            commit = undo.commit_undo
            self._physical.rollback_committed_atomic_episode(
                commit.physical_undo
            )
            self._world.restore_encoded(
                commit.prepared.world_snapshot
            )
            self._latest_episode = commit.prior_latest_episode
            undo._state.phase = "rolled_back"
            self._episode_finalization_undo = None

    def finalize_episode_finalization(
        self,
        undo: CompanionVocalEpisodeFinalizationUndo,
    ) -> None:
        """Release all episode rollback custody after publication succeeds."""
    ...
    def status(self) -> dict[str, object]:
        with self._lock:
            ...
            if self._episode_finalization_undo is not None:
                retained += sum(
                    (
                        len(block.physical_mount.binaural_pcm.left_pcm_s16le)
                        + len(
                            block.physical_mount.binaural_pcm
                            .right_pcm_s16le
                        )
                    )
                    for block in (
                        self._episode_finalization_undo
                        .commit_undo.prepared.prediction_blocks
                    )
                    if block.physical_mount.binaural_pcm is not None
                )
            return {
                "companion_port_id": self._companion_port_id,
                "has_latest_episode": self._latest_episode is not None,
                "episode_publication_pending": int(
                    self._episode_finalization_undo is not None
                ),
