...
Sight, optional sound, body, touch, smell, and taste enter the canonical
native six-sense builder from the same authenticated world edge, so frozen L0--L4
...
from dsf_ai_service.substrate.w1_binaural_acoustic_physics import (
    ...
    distance_squared_mm as _distance_squared,
    propagation_delay_samples as _delay_samples,
    render_ear_pressure as _render_ear,
    signed_pcm16_samples as _signed_pcm_samples,
)
...
CORRESPONDENCE_AUTHORITY_SCHEMA = (
    "guala.w1.anonymous_av_correspondence.authority.v2"
)
CORRESPONDENCE_AUTHORITY_DOMAIN = (
    b"guala.w1.anonymous_av_correspondence.authority.v2\0"
)
...
_TOUCH_AXES = ("contact", "contact-radius", "normal-load")
_PHYSICAL_BODY_RECEPTOR_COUNT = len(_BODY_AXES)
_PHYSICAL_TOUCH_RECEPTOR_COUNT = 6
_PHYSICAL_SMELL_RECEPTOR_COUNT = 8
_PHYSICAL_TASTE_RECEPTOR_COUNT = 5
...
_MAX_W1_NON_SOUND_NATIVE_SAMPLES = 2 * (
    MAX_NATIVE_SIGHT_SUBSTREAMS
    + _PHYSICAL_BODY_RECEPTOR_COUNT
    + _PHYSICAL_TOUCH_RECEPTOR_COUNT
    + _PHYSICAL_SMELL_RECEPTOR_COUNT
    + _PHYSICAL_TASTE_RECEPTOR_COUNT
)
...
def _visual_inputs(
    ...
    *,
    ...
    inputs = physical_receptor_substreams(
        before,
        after,
    ...
    return (
        inputs,
        _digest({
            "schema": "guala.w1.retinal_light_series.v1",
            "substreams": [
                {
                    "coordinates": [
                        [axis.axis_id, axis.coordinate_id]
                        for axis in item.coordinates
                    ],
                    "substream_id": item.substream_id,
                    "values": [
                        _fraction_text(Fraction.from_float(value))
                        for value in item.normalized_signal
                    ],
                }
                for item in inputs
            ],
...
def _somatic_inputs(
    ...
    *,
    ...
    physical = physical_receptor_substreams(
        before,
        after,
        causal_transition=not tonic_reference,
        source_time_start=source_time_start,
        source_time_end=source_time_end,
    )
    ...
    observed = {
        **(
            ...
            if PhysicalSense.BODY in physical
            else {}
        ),
        **material,
    }
    return observed, _digest({
        "schema": "guala.w1.physical_somatic_series.v2",
        "substreams": [
            {
                "coordinates": [
                    [axis.axis_id, axis.coordinate_id]
            ...
            for item in observed.get(sense, ())
        ],
    })
...
class W1AnonymousAcousticVisualCorrespondence:
    ...
    def authority_record(self) -> dict[str, object]:
        return {
            **self.authority_payload(),
            "auditory_l5_authority": (
                self.auditory_l5.persistence_record()
            ),
    ...
    def verify(self, authority_key: bytes | str) -> None:
        ...
        self._verify_authority(key)

    def _verify_authority(self, authority_key: bytes | str) -> None:
        """Verify this correspondence's own authenticated linkage.

        The physical authority uses this only while it retains exact
        request-local custody of the immutable, already-verified nested
        auditory and causal objects.  Public verification remains deep.
        """
        key = _key(authority_key)
...
class W1PhysicalEvidenceReceipt:
    ...
    def verify(self, authority_key: bytes | str) -> None:
        ...
        if (
            self.anonymous_av_correspondence_receipt_sha256 is not None
            and not self.acoustic_emission_receipt_sha256s
        ):
            raise ValueError(
                "W1 audiovisual correspondence lacks acoustic evidence"
            )
        _sha256(
            self.causal_settlement_receipt_sha256,
...
class W1PhysicalEvidenceMount:
    ...
    binaural_receptor_settlement: (
        W1BinauralReceptorSettlement | None
    ) = None
    ...
    native_full_field: BuiltSixSenseFullField | None = None
    ...
    def persistence_record(self) -> dict[str, object]:
        ...
        return {
            ...
            "binaural_receptor_settlement": (
                self.binaural_receptor_settlement.authority_record()
    ...
    def _verify_linkage(
        ...
        *,
        verified_causal_capability: (
            VerifiedCausalSettlementCapability | None
        ) = None,
    ) -> None:
        """Verify the authenticated links between already-verified objects.

        This is used only while the owning authority retains the exact frozen
        objects in its capacity-one prepared transaction.  Public verification
        still performs every nested deep verification before these links.
        """
        if not isinstance(self.state, W1EvidenceState):
            raise ValueError("W1 evidence mount state changed")
        if self.evidence_receipt is None:
            if self.native_full_field is not None:
                raise ValueError(
                    "unsettled W1 evidence retained a native full field"
                )
            ...
            if self.binaural_receptor_settlement is not None:
                raise ValueError(
                    "unsettled W1 evidence retained auditory receptors"
                )
        ...
        if self.native_full_field is None:
            raise ValueError("settled W1 evidence lacks its native full field")
        self.native_full_field.verify_construction()
        if (
            self.native_full_field.boundary.authority_receipt_sha256
            != self.causal_settlement.assembly_receipt_sha256
        ):
            raise ValueError(
                "W1 native full field differs from its causal settlement"
            )
        if verified_causal_capability is None:
            self.causal_settlement.verify()
        else:
            verified_causal_capability.verify_linkage(
        ...
        if acoustic:
            ...
            if self.binaural_receptor_settlement is None:
                raise ValueError(
                    "settled W1 acoustic receptor evidence is incomplete"
                )
            ...
            if (
                ...
                or self.binaural_receptor_settlement.upstream_w1_l5
                != self.binaural_auditory_l5
                or self.binaural_receptor_settlement
                .upstream_causal_settlement_receipt_sha256
                != self.causal_settlement.authority_receipt_sha256
            ):
                raise ValueError("W1 auditory L5 differs from physical evidence")
            if (
                self.anonymous_av_correspondence is None
                or self.anonymous_av_continuity is None
            ):
                if (
                    self.anonymous_av_correspondence is not None
                    or self.anonymous_av_continuity is not None
                    or self.evidence_receipt
                    .anonymous_av_correspondence_receipt_sha256 is not None
                ):
                    raise ValueError(
                        "W1 unresolved audiovisual source fabricated a relation"
                    )
            elif (
                self.anonymous_av_correspondence.authority_receipt_sha256
                != self.evidence_receipt
                .anonymous_av_correspondence_receipt_sha256
                ...
                raise ValueError(
                    "W1 audiovisual source relation differs from evidence"
                )
        elif self.binaural_pcm is not None or dict(
            self.evidence_receipt.binaural_commitment
        ) or self.binaural_auditory_l5 is not None or (
            self.binaural_receptor_settlement is not None
        ) or (
            self.anonymous_av_correspondence is not None
            or self.anonymous_av_continuity is not None
        ):
    ...
    def verify(self, authority_key: bytes | str) -> None:
        """Deep-verify a mount received outside prepared owner custody."""
        if self.evidence_receipt is not None:
            acoustic = (
                self.evidence_receipt
            ...
            if acoustic:
                if self.binaural_pcm is None:
                    raise ValueError(
                        "settled W1 acoustic evidence is incomplete"
                    )
                if self.binaural_auditory_l5 is None:
                    raise ValueError(
                        "settled W1 acoustic L5 evidence is incomplete"
                    )
                if self.binaural_receptor_settlement is None:
                    raise ValueError(
                        "settled W1 acoustic receptor evidence is incomplete"
        ...
        self._verify_linkage(authority_key)
...
class _PreparedAudiovisualMount:
    ...
    verified_causal_capability: VerifiedCausalSettlementCapability
...
class W1AudiovisualPhysicalEvidenceAuthority:
    ...
    def _mount_anonymous_physical_boundary(
        ...
        *,
        ...
        if not isinstance(source_time_start, Fraction):
            raise TypeError(
                "W1 physical boundary source-time start must be exact"
            )
        if reserve and commit:
            raise ValueError("W1 physical boundary cannot reserve and commit")
        if execution_receipt is None:
            source_time_end = source_time_start + Fraction(1)
            execution_receipt_sha256 = None
        else:
            self._world.verify_execution_receipt(execution_receipt)
            if (
                execution_receipt.before != before
                or execution_receipt.after != after
            ):
                raise ValueError(
                    "W1 material boundary differs from its world edge"
                )
        ...
        states = {
            sense: (
                ...
                if (
                    sense is PhysicalSense.SIGHT
                    or sense in somatic_inputs
                )
    ...
    def mount_authenticated_observation(
        ...
        *,
        ...
        if not isinstance(source_time_start, Fraction):
            raise TypeError(
                "W1 current observation source time must be exact"
            )
    ...
    def _seal_anonymous_av_correspondence(
        self,
        *,
        candidates: tuple[W1AnonymousVisualCandidate, ...],
        matched_ordinal: int,
        ...
        """Seal one relation only after physics leaves exactly one source."""

        structural_payload = {
            "auditory_l5": auditory_l5.structural_payload(),
            "body": _exact_sense_payload(
        ...
        provisional = W1AnonymousAcousticVisualCorrespondence(
            ...
            somatic_series_sha256=somatic_series_sha256,
            physical_evidence_core_sha256=(
                physical_evidence_core_sha256
            ),
            causal_settlement_receipt_sha256=(
        ...
        authority_payload = provisional.authority_payload()
        signature = hmac.new(
            self._key,
            CORRESPONDENCE_AUTHORITY_DOMAIN
        ...
        result = W1AnonymousAcousticVisualCorrespondence(
            correspondence_id=provisional.correspondence_id,
            structural_fingerprint=provisional.structural_fingerprint,
            candidates=provisional.candidates,
            matched_ordinal=provisional.matched_ordinal,
            observed_acoustic_path=provisional.observed_acoustic_path,
            sight_interpretation=provisional.sight_interpretation,
            body_interpretation=provisional.body_interpretation,
            auditory_l5=provisional.auditory_l5,
            visual_series_sha256=provisional.visual_series_sha256,
            somatic_series_sha256=provisional.somatic_series_sha256,
            physical_evidence_core_sha256=(
                provisional.physical_evidence_core_sha256
            ),
            causal_settlement_receipt_sha256=(
        ...
        result._verify_authority(self._key)
        return result
        ...
        with self._lock:
            ...
            states = {
                sense: (
                    SenseBoundaryState.OBSERVED
                    if (
                        sense in (PhysicalSense.SIGHT, PhysicalSense.SOUND)
                        or sense in somatic_inputs
                    )
                    else SenseBoundaryState.SENSOR_UNAVAILABLE
                )
            ...
            verified_causal_capability = (
                self._causal_owner.prepared_transaction_capability(
                    settlement
                )
            )
            ...
            try:
                binaural_auditory_l5 = (
                    self._binaural_auditory_l5_owner.prepare(
                        settlement,
                        verified_capability=(
                            verified_causal_capability
                        ),
            ...
            state = W1EvidenceState.OBSERVED
            reason = (
                "anonymous_multisensory_evidence_observed"
                if len(correspondence_matches) == 1
                else (
                    "anonymous_multisensory_evidence_observed_source_unknown"
                    if not correspondence_matches
                    else (
                        "anonymous_multisensory_evidence_observed_"
                        "source_ambiguous"
                    )
                )
            )
            ...
            correspondence = (
                self._seal_anonymous_av_correspondence(
            ...
            if correspondence is not None:
                try:
                    from dsf_ai_service.substrate.w1_anonymous_audiovisual_continuity import (
                        physical_continuity_custody_hmac,
                    )
                    continuity = (
                        self._anonymous_av_continuity_owner.prepare(
                            correspondence,
                            receipt,
                            physical_custody_hmac_sha256=(
                                physical_continuity_custody_hmac(
                                    self._key,
                                    correspondence,
                                    receipt,
    ...
    def _prepared_for(
        ...
        if (
            prepared is None
            or prepared.mount is not mount
        ...
        prepared.verified_causal_capability.verify_linkage(
            mount.causal_settlement
        )
        mount._verify_linkage(
            self._key,
            verified_causal_capability=(
                prepared.verified_causal_capability
            ),
        )
        return prepared
    ...
    def _commit_prepared_multisensory_mount(
        ...
        *,
        ...
        try:
            ...
            if continuity_undo is not None:
            ...
            raise
        try:
            self._causal_owner.commit_prepared(
                mount.causal_settlement,
                verified_capability=(
                    prepared.verified_causal_capability
                ),
