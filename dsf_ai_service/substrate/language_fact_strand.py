...
@dataclass(frozen=True)
class L6Direction:
    """One direction of non-null comparison and canonical L6 exhaustion."""
...
def canonical_l6_direction(
    *,
    ...
    if dimensions < 0 or matching_non_null < 0 or matching_quiescent < 0:
        raise ValueError("L6 dimension counts cannot be negative")
    ...
    # Canonical L6 law: n_eff < n_start/e.  Since n_eff is an integer,
    # ceil(n_start/e) is the exact integer boundary; round() changes the
    # physics for values such as n_start=4 and must not be substituted.
    knee = math.ceil(dimensions / math.e)
    return L6Direction(
