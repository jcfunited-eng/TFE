    ...
    if states[PhysicalSense.TOUCH] is SenseBoundaryState.OBSERVED:
        ...
        observed[PhysicalSense.TOUCH] = tuple(
            _native_signal(
                ...
                sensor_id="tactile-surface-matrix",
                substream_id=f"tactile-{quantity}",
                topology_index=index,
                coordinates=(
                    NativeAxisCoordinate(
                        axis_ordinal=0,
                        fraction=Fraction(index, TOUCH_RECEPTOR_COUNT),
                    ),
                ),
                physical_quantity=quantity,
                physical_unit=unit,
                values=(touch_before[index], touch_after[index]),
    ...
    if states[PhysicalSense.SMELL] is SenseBoundaryState.OBSERVED:
        ...
        observed[PhysicalSense.SMELL] = tuple(
            _native_signal(
                ...
                sensor_id="olfactory-epithelium-matrix",
                substream_id=f"odorant-channel-{channel}",
                topology_index=channel,
                coordinates=(
                    NativeAxisCoordinate(
                        axis_ordinal=0,
                        fraction=Fraction(channel, ODORANT_CHANNELS),
                    ),
                ),
                physical_quantity=f"odorant-saturation-{channel}",
                physical_unit="saturation-ratio",
                values=(smell_before[channel], smell_after[channel]),
                source_time_start=source_time_start,
                source_time_end=source_time_end,
            )
    ...
    if states[PhysicalSense.TASTE] is SenseBoundaryState.OBSERVED:
        ...
        observed[PhysicalSense.TASTE] = tuple(
            _native_signal(
                ...
                sensor_id="gustatory-papilla-matrix",
                substream_id=f"tastant-channel-{channel}",
                topology_index=channel,
                coordinates=(
                    NativeAxisCoordinate(
                        axis_ordinal=0,
                        fraction=Fraction(channel, TASTANT_CHANNELS),
                    ),
                ),
                physical_quantity=f"tastant-saturation-{channel}",
                physical_unit="saturation-ratio",
                values=(taste_before[channel], taste_after[channel]),
                source_time_start=source_time_start,
                source_time_end=source_time_end,
            )
...
def build_coupled_six_sense_full_field(
    *,
    ...
    if sound_substreams:
        ...
        binaural_units = binaural_joint_units(left_sound, right_sound)
