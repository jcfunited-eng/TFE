...
class DeterministicVisualRegionContinuityAuthority:
    ...
    def snapshot_encoded(self) -> bytes:
        with self._lock:
            return self._snapshot_encoded_locked()

    def _snapshot_encoded_locked(self) -> bytes:
        payload = {
    ...
    def _restore_encoded(self, encoded: bytes, *, activate: bool) -> None:
        with self._lock:
            self._restore_encoded_locked(encoded, activate=activate)

    def _restore_encoded_locked(self, encoded: bytes, *, activate: bool) -> None:
        if not isinstance(encoded, bytes) or not encoded:
