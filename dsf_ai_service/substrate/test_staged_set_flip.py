...
def _physical_sound(g, ordinal):
    sample_count = 4160
    pcm = struct.pack(
        f"<{sample_count}h",
        *(
            12_000 if (index + ordinal) % 16 < 8 else -12_000
            for index in range(sample_count)
        ),
    )
    start_ns = 1_000_000_000 + ordinal * 260_000_000
    g.process_sound_frame(
        pcm_s16le_wav(pcm),
        source="staged-flip-physical-auditory-test",
        source_anchor_ns=start_ns,
        source_time_end_ns=start_ns + 260_000_000,
