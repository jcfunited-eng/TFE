...
class AuditoryVerifiedSettlementCapability:
    ...
    causal_settlement: CausalExperienceSettlement = field(
        compare=False,
        repr=False,
    )
    ...
    def verify_linkage(
        ...
        *,
        ...
        if (
            ...
            or causal_settlement is not self.causal_settlement
            or joint_settlement is not self.joint_settlement
        ):
            raise ValueError(
                "incremental auditory capability left its verified transaction"
            )
        if self.verified_causal_transaction is not None:
            self.verified_causal_transaction.verify_linkage(
                causal_settlement
            )
...
def _cells_from_owner(
    owner: AuditoryReciprocityOwner | None,
) -> tuple[_Cell, ...]:
    if owner is None:
        return ()
...
class AuditoryIncrementalTerminalOwner:
    ...
    def __init__(
        self,
        *,
        reciprocity_owner: AuditoryReciprocityOwner | None,
        ...
        if (
            reciprocity_owner is not None
            and not isinstance(reciprocity_owner, AuditoryReciprocityOwner)
        ):
        ...
        *,
        ...
        if verified_causal_transaction is not None:
            if not isinstance(
                ...
                raise TypeError(
                    "incremental auditory causal authority has wrong type"
                )
    ...
    def _recognize_experience(
        ...
        *,
        ...
        experience.verify()
        if self._reciprocity_owner is None:
            return AuditoryRecognitionState.UNKNOWN, None, 0
    ...
    def _full_gate(
        ...
        for channel_index, definition in enumerate(AUDITORY_CHANNELS):
            ...
            pressure_substream_id = f"{definition.name}_pressure"
            pressure_relevance = tuple(
                Fraction.from_float(
                    value.pressure[channel_index]
                ) ** 2
                for value in selected
            )
            common_coordinates = (
