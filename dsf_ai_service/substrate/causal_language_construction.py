...
def _settlement_witness(settlement: CausalExperienceSettlement) -> dict[str, object]:
    payload = causal_experience_settlement_receipt_payload(
        ...
        native_evidence_witness=settlement.native_evidence_witness,
...
class CausalLanguageConstructionAuthority:
    ...
    def settle_complete_constructions(self) -> tuple[LearningResult, ...]:
        ...
        or preferred.  The complete settlement call is one transaction: an
        exceptional failure cannot retain only an earlier component.
        """

        with self._lock:
            original_working = self._working
            original_constructions = self._constructions
            original_ambiguous_families = self._ambiguous_families
            try:
                ...
                if len(results) > self._working_capacity:
                    raise RuntimeError(
                        "construction settlement result boundary changed"
                    )
                return tuple(results)
            except Exception:
                self._working = original_working
                self._constructions = original_constructions
                self._ambiguous_families = original_ambiguous_families
                raise
