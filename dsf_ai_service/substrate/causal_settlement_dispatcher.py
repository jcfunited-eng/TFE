...
STATE_SCHEMA = "guala.causal_settlement_dispatcher.state.v2"
ENVELOPE_SCHEMA = "guala.causal_settlement_dispatcher.state.hmac.v2"
LEGACY_STATE_SCHEMA = "guala.causal_settlement_dispatcher.state.v1"
LEGACY_ENVELOPE_SCHEMA = "guala.causal_settlement_dispatcher.state.hmac.v1"
...
STATE_DOMAIN = b"guala-causal-settlement-dispatcher-state-v2\0"
LEGACY_STATE_DOMAIN = b"guala-causal-settlement-dispatcher-state-v1\0"
...
def _action_from_record(value: object) -> ActionCommand:
    ...
    action.verify(max_command_bytes=MAX_ACTION_PAYLOAD_BYTES)
    if action.as_record() != dict(value):
        raise ValueError("dispatcher action record is not canonical")
    return action


def _retired_speech_active_record(
    value: object,
    *,
    dispatcher_key: bytes,
) -> bool:
    """Validate and identify a legacy speech dispatch under state HMAC."""
    ...
    if not isinstance(action, Mapping) or action.get("kind") != "speech":
        return False
    active_fields = {
        "acknowledgement",
        "binding_id",
    ...
    scalars = action.get("unicode_scalars")
    if (
        set(value) != active_fields
        or value.get("schema") != ACTIVE_SCHEMA
        ...
        or not scalars
        or len(scalars) > MAX_SPEECH_SCALARS
        or any(
        ...
        raise ValueError("retired speech dispatcher state is malformed")
    request_payload = {
        key: item
    ...
    _verify_hmac(
        ...
        signature=request.get("authority_hmac_sha256"),
    )
    return True
...
class ExecutorRequest:
    ...
    def verify(self, authority_key: object) -> None:
        ...
        self.action.verify(max_command_bytes=MAX_ACTION_PAYLOAD_BYTES)
        _verify_hmac(
            key=key,
            domain=REQUEST_DOMAIN,
...
class CausalSettlementDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(embodiment_executor):
            raise TypeError("dispatcher embodiment executor must be callable")
        if any(
    ...
    def _dispatch(
        ...
        *,
        ...
        with self._lock:
            ...
            executor = self._embodiment_executor
        try:
    ...
    def accept_executor_acknowledgement(
        ...
        with self._lock:
            ...
            expected_id = self._embodiment_executor_id
            key = self._embodiment_executor_key
            acknowledgement.verify(key)
            if (
    ...
    def _migrate_legacy_active(
        self, value: object
    ) -> _ActiveDispatch | None:
        if _retired_speech_active_record(
            value, dispatcher_key=self._key
        ):
            return None
        if not isinstance(value, Mapping):
            raise ValueError("legacy active dispatcher state changed")
        request_value = value.get("request")
        action_value = (
            request_value.get("action")
        ...
        if (
            not isinstance(action_value, Mapping)
            or set(action_value) != action_fields
            or action_value.get("schema")
            != "guala.causal_action_cycle.action.v1"
            or action_value.get("kind") != "embodiment_port"
            or action_value.get("unicode_scalars") != []
        ):
            raise ValueError("legacy dispatcher embodiment action changed")
        try:
            command_payload = base64.b64decode(
                action_value.get("command_payload_base64"),
                validate=True,
            )
        except Exception as error:
            raise ValueError(
                "legacy dispatcher embodiment payload changed"
            ) from error
        action = ActionCommand(
            kind="embodiment_port",
            port_id=action_value.get("port_id"),
            command_payload=command_payload,
        )
        action.verify(max_command_bytes=MAX_ACTION_PAYLOAD_BYTES)
        request_fields = {
        ...
        if (
            ...
            or request_value.get("schema") != REQUEST_SCHEMA
        ):
            raise ValueError("legacy dispatcher request fields changed")
        old_request_payload = {
            field: item
            for field, item in request_value.items()
            if field != "authority_hmac_sha256"
        }
        _verify_hmac(
            key=self._key,
            domain=REQUEST_DOMAIN,
            payload=old_request_payload,
            signature=request_value.get("authority_hmac_sha256"),
        )
        old_intent = request_value.get("intent_receipt_sha256")
        new_intent = self._cycle.migrated_intent_receipt(old_intent)
        old_binding = value.get("binding_id")
        new_binding = self._cycle.migrated_binding_receipt(old_binding)
        if new_intent is None or new_binding is None:
            raise ValueError("legacy dispatcher lost embodied cycle authority")
        unsigned_request = {
            "action": action.as_record(),
            "intent_receipt_sha256": new_intent,
            "schema": REQUEST_SCHEMA,
            "trigger_settlement_receipt_sha256": request_value[
                "trigger_settlement_receipt_sha256"
            ],
        }
        request = ExecutorRequest(
            intent_receipt_sha256=new_intent,
            trigger_settlement_receipt_sha256=unsigned_request[
                "trigger_settlement_receipt_sha256"
            ],
            action=action,
            authority_hmac_sha256=_sign(
                self._key, REQUEST_DOMAIN, unsigned_request
            ),
        )
        acknowledgement_value = value.get("acknowledgement")
        acknowledgement = None
        if acknowledgement_value is not None:
            old_acknowledgement = _acknowledgement_from(
                acknowledgement_value,
                key=self._embodiment_executor_key,
            )
            if (
                old_acknowledgement.request_receipt_sha256
                != _authority_receipt(
                    old_request_payload,
                    request_value["authority_hmac_sha256"],
                )
            ):
                raise ValueError("legacy dispatcher acknowledgement changed")
            acknowledgement = authenticate_executor_acknowledgement(
                request=request,
                executor_id=old_acknowledgement.executor_id,
                authority_key=self._embodiment_executor_key,
                executor_action_receipt_sha256=(
                    old_acknowledgement.executor_action_receipt_sha256
                ),
                disposition=old_acknowledgement.disposition,
            )
        old_execution = value.get("execution_receipt_sha256")
        new_execution = (
            self._cycle.migrated_execution_receipt(old_execution)
            if old_execution is not None
            else None
        )
        if old_execution is not None and new_execution is None:
            raise ValueError("legacy dispatcher execution changed")
        binding_value = value.get("outcome_binding")
        outcome_binding = None
        if binding_value is not None:
            old_outcome_binding = _binding_from(
                binding_value, dispatcher_key=self._key
            )
            if old_outcome_binding.execution_receipt_sha256 != old_execution:
                raise ValueError("legacy outcome binding execution changed")
            unsigned_binding = {
                "execution_receipt_sha256": new_execution,
                "observer_attestation_receipt_sha256": (
                    old_outcome_binding.observer_attestation_receipt_sha256
                ),
                "outcome_settlement_receipt_sha256": (
                    old_outcome_binding.outcome_settlement_receipt_sha256
                ),
                "outcome_structural_fingerprint": (
                    old_outcome_binding.outcome_structural_fingerprint
                ),
                "schema": OUTCOME_BINDING_SCHEMA,
            }
            outcome_binding = OutcomeObservationBinding(
                execution_receipt_sha256=new_execution,
                outcome_settlement_receipt_sha256=(
                    old_outcome_binding.outcome_settlement_receipt_sha256
                ),
                ...
                authority_hmac_sha256=_sign(
                    self._key, OUTCOME_BINDING_DOMAIN, unsigned_binding
                ),
            )
        old_cycle_outcome = value.get("cycle_outcome_receipt_sha256")
        new_cycle_outcome = (
            self._cycle.migrated_outcome_receipt(old_cycle_outcome)
            if old_cycle_outcome is not None
            else None
        )
        if old_cycle_outcome is not None and new_cycle_outcome is None:
            raise ValueError("legacy dispatcher cycle outcome changed")
        active = _ActiveDispatch(
            phase=value.get("phase"),
            trigger_settlement_receipt_sha256=value.get(
                "trigger_settlement_receipt_sha256"
            ...
            cycle_outcome_receipt_sha256=new_cycle_outcome,
        )
    ...
    def restore_encoded(self, value: object) -> None:
        ...
        migrated_legacy_state = legacy_envelope
        latest = (
        ...
        if (
            latest is not None
            and self._cycle.last_restore_retired_speech_binding(
                latest.binding_id
            )
        ):
            latest = None
        ...
        active = None
        if active_value is not None and legacy_envelope:
            active = self._migrate_legacy_active(active_value)
            active_value = None
        if active_value is not None:
            if _retired_speech_active_record(
                active_value,
                dispatcher_key=self._key,
            ):
                active_value = None
        ...
        if active_value is not None:
            ...
            executor_key = self._embodiment_executor_key
            acknowledgement = (
