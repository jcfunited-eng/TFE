...
# at commit 31b9e8c (GL-CMD-112 Phase A, per GL-SPC-SENSORY-CATALOG-
# EVE-20260621-111 §6.2), which never merged to guala-live. Brought over
# 2026-07-04 as a test-suite dependency repair (unblocks loom_model/tests/
# test_folding_engaged.py's import chain, which has been uncollectable
# on guala-live for this reason) AND per GL-CMD-C2-WHOLE-BRAIN-EVE-
# 20260704-168-v3 A2's explicit instruction to use "the sensory
# transducer on the codex branch" for the whole-organism raising run.
# This is suite/model repair only — it is NOT wired into the live
# sensory path (same status as sensory_transducer.py's own prior port,
# GL-KB-COGNITION-ARC-RECOVERED-EVE-20260703-v2 §2.7). Content otherwise
# unchanged from the codex branch.
"""
