...
Preparation verifies one signed native stream per mounted port, enforces the
ratified invertible kernel map F = 1 + s/2 and the single native relevance
field, and runs frozen L0--L4 independently.  Each native L1 gate closes on its
own exact source timestamp.  The canonical union of those timestamps forms one
continuous sparse event sequence: only ports closing at an event contribute
evidence, while every absent topology fiber remains explicit non-evidence in
the SparseMapInjection receipt.
...
if TYPE_CHECKING:
    from .heterogeneous_l6 import HeterogeneousL6Assembly
from .model import (
    EvidenceStream,
    ReceiptError,
    ReceiptRecord,
...
class KernelNativeInputSample:
    ...
    def __post_init__(self) -> None:
        ...
        if not Fraction(1, 2) <= self.dimensionless_field <= Fraction(3, 2):
            raise ReceiptError("kernel-native field must remain in exact [1/2,3/2]")
...
class KernelNativeInputStream:
    ...
    def verify(self, stream: EvidenceStream, registry: ReceiptRegistry) -> None:
        ...
        for adapted, source in zip(self.samples, stream.samples, strict=True):
            ...
            if not Fraction(-1) <= source.signal <= Fraction(1):
                raise ReceiptError("native signal is outside the ratified [-1,1] range")
            expected_field = Fraction(1) + source.signal / 2
            if adapted.dimensionless_field != expected_field:
                raise ReceiptError(
                    "kernel input differs from the ratified exact F=1+s/2 map"
                )
            if 2 * (adapted.dimensionless_field - 1) != source.signal:
...
class ClosedExperienceProviderBundle:
    ...
    l6_production: PhysicalL6TangentProduction | HeterogeneousL6Assembly
    ...
    return _canonical_bytes(
        {
            ...
            "native_relevance_rule": "exact_source_relevance_identity",
            "schema": "glew.provider.complete_signed_port_l0_l4_trace.v3",
...
def _mount_heterogeneous_l6_production(
    *,
    production: HeterogeneousL6Assembly,
    sealed: SealedClosedExperience,
    topology: MountedFieldTopology,
    receipt_registry: ReceiptRegistry,
) -> tuple[ReceiptRegistry, Fixed42ConstraintStack]:
    _verify_registry_extension(sealed.receipt_registry, production.receipt_registry)
    _verify_registry_extension(production.receipt_registry, receipt_registry)
    production.verify(topology=topology, receipt_registry=receipt_registry)
    return receipt_registry, production.combined_stack
...
def assemble_closed_experience_provider_bundle(
    *,
    ...
    l6_production: PhysicalL6TangentProduction | HeterogeneousL6Assembly | None,
    ...
    missing = (
        ...
        (
            l6_production,
            (PhysicalL6TangentProduction, HeterogeneousL6Assembly),
    ...
    if isinstance(l6_production, HeterogeneousL6Assembly):
        working, l6_stack = _mount_heterogeneous_l6_production(
