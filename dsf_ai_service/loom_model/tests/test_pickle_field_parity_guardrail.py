...
to whatever hardcoded default someone typed into __setstate__ by hand:
  ...
  3. GL-RPT-PHASE-1-V2-REVIVE-C1-20260708-v1 (same report) / the v2
     draft: _spike_bus/_word_firing_callback left unmentioned (absent)
     after restore instead of backfilled to __init__'s real None
     default.
...
-----------------------
LoomNeuron.__getstate__ only pops three fields (_neuron_lock, _spike_bus,
_word_firing_callback) that are either unpicklable or runtime-only back-
references; __setstate__ then defensively backfills those three PLUS
...
def assert_neuron_getstate_setstate_parity():
    ...
    assert not dropped, (
        ...
        ". This is incident 3's exact bug class (GL-RPT-PHASE-1-V2-"
        "REVIVE-C1-20260708-v1: _spike_bus/_word_firing_callback left "
        "absent instead of backfilled to None)."
    )
...
def test_neuron_full_roundtrip_preserves_every_init_field():
    ...
    value, except the three fields __getstate__ deliberately excludes,
    which must come back to their documented post-restore state
    (_neuron_lock: a real, usable new Lock; _spike_bus/
    _word_firing_callback: None, re-wired later by
    Guala.wire_spike_bus())."""
    fresh = LoomNeuron("_guardrail_roundtrip_ref")
    fresh.set_spike_bus(object())
    fresh.set_word_firing_callback(lambda w, i: None)
