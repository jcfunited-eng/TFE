...
def test_step_preserves_physical_output_contract():
    n = LoomNeuron("n1")
    result = n.step([0.0, 0.25, -0.5, 0.75], tick=1)
    assert set(result.keys()) == {"committed", "n_eff", "dsf", "spike_count",
                                   "delta_eff"}
    print("test_step_preserves_physical_output_contract: PASS")
...
# ---------------------------------------------------------------------------
# Symbolic metadata retirement
# ---------------------------------------------------------------------------

def test_external_spike_metadata_has_no_word_authority():
    n = LoomNeuron("n1")
    n.membrane_threshold = 0.5
    n.receive_spike(_spike(1.0, source="_input_injection_", metadata={"word": "dog"}))
    assert "_word_firing_callback" not in n.__dict__
    assert "chi_atlas" not in n.__dict__
    assert "binding_atlas" not in n.__dict__
    assert all(
        "last_input_word" not in krimelack.__dict__
        for krimelack in n.krimelack_bank.values()
    )
    print("test_external_spike_metadata_has_no_word_authority: PASS")
...
if __name__ == "__main__":
    ...
    test_step_preserves_physical_output_contract()
    ...
    test_external_spike_metadata_has_no_word_authority()
