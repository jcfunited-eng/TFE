...
Pieces assembled here:
  ...
  REUSED (imported from validated primitives):
    ...
    10. L6_TCL               — dsf_ai_service/loom_model/neuronal_l6.py
    11. bt_div + friends     — dsf_ai_service/v4/gualaloom_mathloom_v1.py
    12. PhysicalSignalOscillator — loom_model/physical_oscillators.py
    13. PhysicalSensoryOscillatorBank — loom_model/physical_oscillators.py
...
# Reserved source_id prefix for non-synaptic physical spike sources.
# STDP only applies to real synapses: there is no connection to learn on
# for an external physical stimulus.
...
class FamiliarityFeedback:
    """Physical dead-zone attenuation register.

    The neuronal label atlas is retired, so the neuron supplies no semantic
    familiarity score.  The register remains part of the preserved physical
    state and applies only an explicitly supplied physical attenuation.
    """
    ...
    def update(self, match_score: float) -> None:
        """Update the effective dead-zone from a physical match value."""
...
class LoomNeuron:
    ...
    def __init__(self, neuron_id: str, dna_blueprint: Optional[Any] = None,
        ...
        krim_class = KRIMELACK_PRIMITIVES.get(
            primary_modality,
            PhysicalSignalOscillator,
        )
        self.krimelack = krim_class()                             # 12. Krimelack

        self.sensory_bank = PhysicalSensoryOscillatorBank()
        # 14. _grandurun_state — used as module-level function
        # 15. _SPIN_VECTOR_DIM — constant

        # Preserve every physical sensory oscillator. Cross-modal meaning is
        # settled by the causal THING mosaic, not a per-neuron label atlas.
        ...
        # Coupling signal accumulator — affects the next krimelack
        # transduction. Repeated IEEE-754 addition preserves the former
        # list's ordered sum exactly while retaining constant memory.
        self._coupling_signal_sum: float = 0.0
        self._coupling_signal_pending: bool = False
        ...
        self._last_origin_transducer: str = primary_modality
        ...
        # Structural position for propagation-delay computation.  A restored
        # state may leave this absent; the propagation function then uses its
        # preserved default delay until embryo DNA seeds a physical position.
        ...
        self._discard_symbolic_language_labels()
    ...
    def __getstate__(self):
        ...
        _mood_source: GL-CMD-MOOD-BROADCAST -- a live, runtime-only
        ...
        # Migration defense for an authenticated pre-cutover object.
    ...
    def __setstate__(self, state):
        ...
        state = dict(state)
        state.pop('_word_firing_callback', None)
        has_legacy_coupling_backlog = (
            '_coupling_signal_accum' in state
        )
        legacy_coupling_backlog = state.pop(
            '_coupling_signal_accum',
            None,
        )
        if has_legacy_coupling_backlog:
            if (
                '_coupling_signal_sum' in state
                or '_coupling_signal_pending' in state
            ):
                raise ValueError(
                    "coupling signal state contains both backlog and accumulator"
                )
            if type(legacy_coupling_backlog) is not list:
                raise TypeError("legacy coupling signal backlog is not a list")
            state['_coupling_signal_sum'] = (
                sum(legacy_coupling_backlog)
                if legacy_coupling_backlog
                else 0.0
            )
            state['_coupling_signal_pending'] = bool(
                legacy_coupling_backlog
            )
        self.__dict__.update(state)
        self._neuron_lock = threading.Lock()

        # GL-CMD-MOOD-BROADCAST backfill: a pre-this-dispatch pickle has
        # no _mood_source key at all (it was popped from state above).
        ...
        if not hasattr(self, '_coupling_signal_sum'):
            self._coupling_signal_sum = 0.0
        if not hasattr(self, '_coupling_signal_pending'):
            self._coupling_signal_pending = False
        ...
        # A pre-spike-bus state has no runtime bus reference.
    ...
    # Event-driven physical spike handling and STDP synapse plasticity.
    ...
    def _discard_symbolic_language_labels(self) -> None:
        """Remove restored word labels without changing oscillator state."""
        seen = set()
        for krimelack in (
            getattr(self, "krimelack", None),
            *getattr(self, "krimelack_bank", {}).values(),
        ):
            if krimelack is None or id(krimelack) in seen:
                continue
            seen.add(id(krimelack))
            krimelack.__dict__.pop("last_input_word", None)
    ...
    def _fire(self, now: float) -> None:
        ...
        synchronous downstream depression notification still happen
        unconditionally; only the
    ...
    def step(self, input_signal, tick: int, input_chi: Optional[int] = None) -> Dict:
        ...
        Args:
            input_signal: numeric physical signal
            tick:         current substrate tick

        ``input_chi`` remains a call-signature compatibility argument only;
        it is not read as identity, familiarity, routing, or meaning.

        Returns physical commit, L6, DSF, spike, and membrane-gate state.
        """
        if isinstance(input_signal, (str, bytes, bytearray)):
            raise TypeError(
                "LoomNeuron accepts physical numeric signals, not symbols"
            )
        try:
            physical_signal = np.asarray(
                input_signal,
                dtype=np.float64,
            ).reshape(-1)
        except (TypeError, ValueError) as error:
            raise TypeError(
                "LoomNeuron input must be a physical numeric signal"
            ) from error
        if not np.all(np.isfinite(physical_signal)):
            raise ValueError(
                "LoomNeuron physical signal contains a non-finite sample"
            )
        signal_samples = physical_signal.tolist()
        self._tick = tick

        # a. Krimelack transduces input_signal → events
        # Compute omega_override from the fixed-size ordered accumulator.
        ...
        omega_override = None
        if self._coupling_signal_pending:
            omega_shift = self._coupling_signal_sum * J_BASE / J_MAX
        ...
        if hasattr(self.krimelack, 'feed_signal'):
            self.krimelack.feed_signal(signal_samples)
        else:
            self.krimelack.feed(signal_samples)
        if omega_override is not None:
            self.krimelack.omega_0 = saved
        self._last_origin_transducer = self.primary_modality
        ...
        # b. No per-neuron identity atlas contributes to the field.
        self.familiarity.update(0.0)
        ...
        if committed:
            # The dominant ψ mode selects the physical spike intensity only.
    ...
    def receive_coupling_spike(self, neuron_id_from: str,
        ...
        # Fixed-size coupling signal accumulator — affects NEXT transduction.
        ...
        self._coupling_signal_sum += spike_contribution
        self._coupling_signal_pending = True
