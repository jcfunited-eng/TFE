...
LEGACY_SCHEMA = "guala.structural_graph.v1"
SCHEMA = "guala.structural_graph.v2"
...
def _object_specs(
    *,
    include_retired_neuronal_cognition: bool = False,
    include_legacy_coupling_backlog: bool = False,
) -> tuple[ObjectSpec, ...]:
    """Build the current physical-organism registry lazily.

    Historical LoomTapestry, wave Cell, Chi, and Binding types are excluded
    from this runtime boundary.  Their authenticated source graphs are read
    only by ``tools.guala_legacy_organism_graph_reader``.  The former numeric
    coupling backlog remains readable here because LoomNeuron converts that
    physical accumulator state without importing retired cognition.
    """
    if include_retired_neuronal_cognition:
        raise StructuralGraphError(
            "retired structural types require the explicit one-way "
            "migration registry"
        )
    ...
    neuron_runtime = _fields(
        "_neuron_lock _spike_bus _mood_source")
    brain_runtime = _fields("_spike_bus _guala_ref")
    neuron_fields = _fields(
        "neuron_id primary_modality observable psi_lattice "
        "spike_buffer couplings familiarity laws dna_site "
        "trit_register l6_tcl krimelack sensory_bank "
        "krimelack_bank _tick _last_dsf _last_events "
        "_last_commit_chi _last_commit_intensity "
        "_coupling_injection _coupling_signal_sum "
        "_coupling_signal_pending "
        ...
        "_last_origin_transducer _omega_history "
        "_expended_energy _last_energy_update_time_s "
        "_energy_block_count"
    )
    if include_legacy_coupling_backlog:
        neuron_fields = neuron_fields.union(
            {"_coupling_signal_accum"})
    embryo_fields = _fields(
        ...
        "_reflection_snapshots _identity _goal"
    )
    return (
        ObjectSpec(
            "embryo", Embryo,
        ...
        ObjectSpec(
            "language_krimelack", PhysicalSignalOscillator,
            _fields(
        ...
        ObjectSpec(
            "modal_krimelack", PhysicalModalOscillator,
            _fields(
                "label omega_0 kappa threshold dt phase winding events "
                "n_events t modality"),
        ),
        ObjectSpec(
            "base_krimelack", PhysicalBaseOscillator,
            _fields(
                "label omega_0 kappa threshold dt phase winding events "
                "n_events t"),
        ),
        ...
        ObjectSpec(
            ...
            _fields(
                "_n_events _phase events omega_0 winding "
                "_authenticated_full_field_exact_state "
                "_authenticated_full_field_lanes "
                "_authenticated_full_field_latest"
            ),
        ),
        ObjectSpec(
...
def _registry(
    *,
    ...
    specs = _object_specs(
        ...
        include_legacy_coupling_backlog=(
            include_legacy_coupling_backlog
        ),
    )
...
def _stable_scalar_probe_start(
    kind: str,
    meta: bytes,
    payload: bytes,
    namespace_size: int,
    ...
    digest.update(b"guala.structural.scalar.v2\x00")
    for part in (kind.encode("utf-8"), meta, payload):
        digest.update(struct.pack(">Q", len(part)))
        digest.update(part)
    return int.from_bytes(digest.digest()[:8], "big") % namespace_size
...
def _compressed_array_payload(
        pointer: int | None,
        payload_size: int,
) -> tuple[tempfile.SpooledTemporaryFile, int]:
    spool = tempfile.SpooledTemporaryFile(max_size=_NODE_BUFFER_BYTES)
    compressor = zlib.compressobj(level=6)
    offset = 0
    while offset < payload_size:
        length = min(1024 * 1024, payload_size - offset)
        assert pointer is not None
        encoded = compressor.compress(
            ctypes.string_at(pointer + offset, length))
        if encoded:
            spool.write(encoded)
        offset += length
    ...
    if final:
        spool.write(final)
    encoded_size = spool.tell()
    spool.seek(0)
    return spool, encoded_size


def _restore_compressed_array(
    connection: sqlite3.Connection,
    node_id: int,
    meta: Mapping[str, Any],
    encoded_size: int,
    ...
    dtype, shape, strides, origin, writeable = _validated_array_descriptor(
        meta,
        0 if math.prod(tuple(int(value) for value in meta["shape"])) == 0
        else (
            max(
                0,
                sum(
                    max(0, (int(size) - 1) * int(stride))
    ...
    backing = bytearray(max(decoded_size, dtype.itemsize, 1))
    target = memoryview(backing)
    decoded_offset = 0
    ...
    with connection.blobopen(
        ...
        while encoded_offset < encoded_size:
            encoded = blob.read(
                min(1024 * 1024, encoded_size - encoded_offset))
            if not encoded:
                raise StructuralGraphError(
                    "compressed numpy payload ended before its declared size")
            encoded_offset += len(encoded)
            pending = encoded
            while pending:
                decoded = decompressor.decompress(
                    pending,
                    decoded_size - decoded_offset + 1,
                )
                if decoded_offset + len(decoded) > decoded_size:
                    raise StructuralGraphError(
                        "compressed numpy payload exceeds its descriptor")
                target[decoded_offset:decoded_offset + len(decoded)] = decoded
                decoded_offset += len(decoded)
                pending = decompressor.unconsumed_tail
    final = decompressor.flush(decoded_size - decoded_offset + 1)
    if decoded_offset + len(final) > decoded_size:
        raise StructuralGraphError(
            "compressed numpy payload exceeds its descriptor")
    target[decoded_offset:decoded_offset + len(final)] = final
    decoded_offset += len(final)
    if (
        not decompressor.eof
        or decompressor.unused_data
        or decoded_offset != decoded_size
    ):
        raise StructuralGraphError(
            "compressed numpy payload differs from its descriptor")
    array = np.ndarray(
        shape=shape,
        dtype=dtype,
        buffer=backing,
        offset=origin if decoded_size else 0,
        strides=strides,
    )
    if not writeable:
        array.flags.writeable = False
...
class _GraphWriter:
    ...
    def _new_node(
        ...
        *,
        ...
        node_id = self._next_composite_id
        node_bytes = (
    ...
    def _scalar_node(
        ...
        *,
        ...
        if row is not None:
            ...
            return node_id
        start = _stable_scalar_probe_start(
            kind,
            lookup_meta,
            lookup_payload,
        ...
        node_id = None
        for offset in range(self.limits.max_nodes):
            candidate_id = (
                self.limits.max_nodes
                + 1
                + ((start + offset) % self.limits.max_nodes)
            )
            collision = self.connection.execute(
                "SELECT kind,meta,payload FROM scalar_work WHERE node_id=?",
                (candidate_id,),
            ).fetchone()
            if collision is None:
                node_id = candidate_id
                break
            if collision == (kind, lookup_meta, lookup_payload):
                node_id = candidate_id
                break
        if node_id is None:
            raise StructuralGraphError(
                "structural scalar identity namespace is exhausted")
        self.node_count += 1
        if self.node_count > self.limits.max_nodes:
            raise StructuralGraphError(
                "structural artifact exceeded its node capacity")
    ...
    def write_value(self, value: Any, *, depth: int = 0) -> int:
        ...
        if isinstance(value, np.ndarray):
            ...
            compressed, encoded_size = _compressed_array_payload(
                pointer, payload_size)
            try:
                if encoded_size <= _INLINE_ARRAY_BYTES:
                    payload = compressed.read()
                    if len(payload) != encoded_size:
                        raise StructuralGraphError(
                            "compressed numpy payload ended early")
    ...
    def finish(self, root: Any) -> None:
        ...
        metadata = {
            ...
            "scalar_namespace_size": self.limits.max_nodes,
        }
        self._flush_nodes()
        self._flush_edges()
        self.connection.execute(
            """
            INSERT INTO nodes(node_id,kind,type_tag,meta,payload)
            SELECT node_id,kind,NULL,NULLIF(meta,X''),payload
            FROM scalar_work
            ORDER BY node_id
            """
        )
...
class _GraphReader:
    ...
    def _validate(self) -> None:
        ...
        if (
            ...
            or user_version not in {LEGACY_USER_VERSION, USER_VERSION}
        ):
        ...
        if user_version == USER_VERSION:
            expected_metadata.add("scalar_namespace_size")
        ...
        retired_type_count = int(
            self.connection.execute(
                "SELECT COUNT(*) FROM nodes "
                "WHERE type_tag IN ('binding_atlas','chi_atlas')"
            ).fetchone()[0]
        )
        retired_field_count = int(
            self.connection.execute(
                "SELECT COUNT(*) FROM edges "
                "WHERE field_name IN ("
                "'binding_atlas','chi_atlas','_lane_P','_scP',"
                "'last_input_word','_word_firing_callback'"
                ")"
            ).fetchone()[0]
        )
        self.has_retired_neuronal_cognition = (
            retired_type_count > 0 or retired_field_count > 0
        )
        if (
            self.has_retired_neuronal_cognition
            and not {
        ...
        _, self.by_tag = _registry(
            include_legacy_coupling_backlog=(
                self.has_legacy_coupling_backlog
            ),
        )
        node_count = metadata["node_count"]
        root_id = metadata["root_id"]
        scalar_namespace_size = metadata.get("scalar_namespace_size")
        if (
        ...
        if self.schema == SCHEMA and (
            isinstance(scalar_namespace_size, bool)
            or not isinstance(scalar_namespace_size, int)
            or scalar_namespace_size <= 0
            or scalar_namespace_size > self.limits.max_nodes
        ):
            raise StructuralGraphError(
                "structural scalar namespace differs")
        ...
        if actual_count != node_count:
            raise StructuralGraphError(
                ...
                "or its node census differs")
        if self.schema == LEGACY_SCHEMA:
            if minimum != 1 or maximum != node_count:
                raise StructuralGraphError(
                    "legacy structural node ids are not contiguous")
        else:
            composite_count, composite_min, composite_max = (
                self.connection.execute(
                    "SELECT COUNT(*),MIN(node_id),MAX(node_id) "
                    "FROM nodes WHERE node_id <= ?",
                    (scalar_namespace_size,),
                ).fetchone()
            )
            if (
                composite_count <= 0
                or composite_min != 1
                or composite_max != composite_count
            ):
                raise StructuralGraphError(
                    "structural composite node ids are not contiguous")
            invalid_scalar_ids = self.connection.execute(
                "SELECT COUNT(*) FROM nodes "
                "WHERE (node_id > ? AND kind NOT IN "
                "('none','bool','int','float','complex','str','bytes',"
                "'numpy_scalar')) "
                "OR (node_id <= ? AND kind IN "
                "('none','bool','int','float','complex','str','bytes',"
                "'numpy_scalar')) "
                "OR node_id > ?",
                (
                    scalar_namespace_size,
                    scalar_namespace_size,
                    2 * scalar_namespace_size,
                ),
            ).fetchone()[0]
            if invalid_scalar_ids:
                raise StructuralGraphError(
                    "structural scalar identity class differs")
        expected_spec = next(
            (
    ...
    def read_value(self, node_id: int, *, depth: int = 0) -> Any:
        if depth > self.limits.max_depth:
            raise StructuralGraphError(
                "structural artifact exceeded its nesting depth")
        existing = self.nodes.get(node_id, _UNSET)
        ...
        if kind == "none":
            ...
            if self.schema != LEGACY_SCHEMA:
                raise StructuralGraphError(
                    "raw numpy payload is forbidden in structural v2")
            ...
            return value
        elif kind == "ndarray_zlib":
            if self.schema != SCHEMA:
                raise StructuralGraphError(
                    "compressed numpy payload is unsupported in structural v1")
            if not isinstance(meta, dict):
                raise StructuralGraphError("numpy metadata is absent")
            value = _restore_compressed_array(
                self.connection, node_id, meta, payload_size)
            self.nodes[node_id] = value
            return value
        elif kind == "dict":
