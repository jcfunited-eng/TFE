    ...
    def _advance(self, organism: FunctionalOrganism, world: Any, sensory: LeanSensoryOccurrence | None) -> SettlementResult:
        ...
        try:
            ...
            if sensory is not None and sensory.pressure_s16le is not None:
                ...
                sound_source = sensory.from_object
                if sensory.from_object is not None:
                    ...
                    speaker = next((b for b in before.bodies if b.body_id == sensory.from_object and b.body_id != before.self_body_id), None)
                    if speaker is not None and path != "door":
                        her = _self_body(before)
                        phi = math.radians(her.pose.heading_millidegrees / 1000.0)
                        # Binaural bilateral ear coordinates: left (0, +85, 200), right (0, -85, 200)
                        lx = her.pose.position.x - int(round(85.0 * math.sin(phi)))
                        ly = her.pose.position.y + int(round(85.0 * math.cos(phi)))
                        rx = her.pose.position.x + int(round(85.0 * math.sin(phi)))
                        ry = her.pose.position.y - int(round(85.0 * math.cos(phi)))
                        ez = her.pose.position.z + 200
                        sx, sy, sz = speaker.pose.position.x, speaker.pose.position.y, speaker.pose.position.z + 200
                        dist_l = max(1.0, math.dist((lx, ly, ez), (sx, sy, sz)))
                        dist_r = max(1.0, math.dist((rx, ry, ez), (sx, sy, sz)))
                        gain_l = min(1.0, float(gain * Fraction(SOUND_REFERENCE_MM, max(1, int(dist_l)))))
                        gain_r = min(1.0, float(gain * Fraction(SOUND_REFERENCE_MM, max(1, int(dist_r)))))
                        delay_samples_l = min(100, int(round(dist_l / 343_000.0 * 16_000)))
                        delay_samples_r = min(100, int(round(dist_r / 343_000.0 * 16_000)))
                        min_delay = min(delay_samples_l, delay_samples_r)
                        delay_l = delay_samples_l - min_delay
                        delay_r = delay_samples_r - min_delay
                        p_left = audioop.mul(pressure, 2, gain_l) if gain_l > 0 else None
                        p_right = audioop.mul(pressure, 2, gain_r) if gain_r > 0 else None
                        if p_left is not None and delay_l > 0:
                            p_left = (b"\x00\x00" * delay_l) + p_left[:-2 * delay_l]
