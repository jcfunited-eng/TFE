...
Provides both self-hearing transduction (mono duplicated to both ears for her own airway)
and binaural hearing transduction (distinct left and right cochlear channels from spatial acoustics).
"""
...
def _transduce_mono(
    ...
    """Transduce one mono channel into 16 ERB cochlear trajectories."""
    if not isinstance(pressure_s16le, bytes) or not pressure_s16le:
        raise ValueError("hearing pressure is empty")
    if len(pressure_s16le) % 2:
        raise ValueError("hearing pressure changed signed-16 width")
    ...
    return times, legacy, one_ear, consumed
...
def one_self_hearing_hop(
    ...
    """Transduce at most one 250 ms slice for her own airway (both ears identical)."""
    times, legacy, one_ear, consumed = _transduce_mono(pressure_s16le)
    return times, legacy, one_ear * EAR_COUNT, consumed


def one_binaural_hearing_hop(
    left_pressure_s16le: bytes,
    right_pressure_s16le: bytes,
) -> tuple[
    tuple[Fraction, ...],
    tuple[float, ...],
    tuple[tuple[float, ...], ...],
    ...
    """Transduce at most one 250 ms slice with true binaural separation (left ear: 0..15, right ear: 16..31)."""
    times, legacy, left_ear, consumed_l = _transduce_mono(left_pressure_s16le)
    _times_r, _legacy_r, right_ear, consumed_r = _transduce_mono(right_pressure_s16le)
    return times, legacy, (left_ear + right_ear), min(consumed_l, consumed_r)
