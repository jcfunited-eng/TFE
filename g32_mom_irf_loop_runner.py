...
ROOT = Path("/tmp/g32_mom_irf_loop")
RUNS_DIR = ROOT / "runs"
...
PLATEAU_EXPLORE_MAX = 0.80

NEG_FIELD_UNFAVORABLE_MAX = 400
NEG_FIELD_PROXIMITY_WEIGHT = 4.0
NEG_FIELD_UNCERTAINTY_BONUS = 1.25
NEG_FIELD_TEMPERATURE_BASE = 0.22
NEG_FIELD_TEMPERATURE_STEP = 0.01
...
def build_pool() -> List[Dict[str, str]]:
    ...
    include_irf_modes = ["1", "0,1", "0"]
    train_fracs = [
        "0.55,0.65,0.75,0.85",
        "0.60,0.70,0.80,0.90",
    ]
    test_frac = ["0.08", "0.10"]
    min_edge_values = ["0.0", "0.0005", "0.001"]
    min_winrate_values = ["0", "50", "52", "55"]
    uncertainty_penalty_values = ["0.0", "0.001", "0.002"]
    hold_bias_values = ["0.0", "0.0005", "0.001"]
...
def compute_explore_rate(stale_runs: int) -> float:
    ...
    bonus = min(0.50, 0.02 * float(stale_runs - PLATEAU_REEXPLORE_AFTER))
...
def run_one(run_id: int, cfg: Dict[str, str], fingerprint: Dict[str, Any]) -> Dict[str, Any]:
    run_name = f"momirf_{run_id:04d}_{stamp()}"
...
def main() -> None:
    ...
    try:
        ...
        start_history_size = len(history)
        log(
            "loop_start mom_irf "
        ...
        if champion_cfg is not None:
            ...
            write_ab_artifacts(ab_state=ab_state, run_id=run_id, history_size=len(history), stale_runs=stale_runs)
            completed = max(0, len(history) - start_history_size)
            if completed >= TARGET_COMPLETED_RUNS:
                log(f"target_completed_runs_reached run_id={run_id} completed={completed}")
                log("loop_complete")
                return
        else:
