...
**Authoritative deployment plan (2026-06-25):** `docs/TAO_FINANCIAL_ENGINE_PORTFOLIO_MANAGER_DESIGN_DOCUMENT_DEPLOYMENT_VERSION.md`
This is the single deployment design document. It supersedes all prior fragmentary deployment notes. It covers the full pipeline (L0-L4 kernel, Wave 1 selection, capital allocation, exit logic, bookkeeping, execution environment) with every constant cited to source file and line. Deployment sequence: Gates D1 (s_n emission) → D2 (Wave 1 filter) → D3 (exit logic v1.0) → D4 (bookkeeping fixes) → D5 (fresh ledger) → D6 (30-day paper) → D7 (live). All gates currently at NOT BUILT or NOT STARTED except execution environment (D6/D7 path). Immediate blocker: F-007a sentinel deploy (SHA 29bf93d), then s_n emission in uf_mdg_snapshot.py.

**Architectural inversion confirmed:** Wave 1 (s_n/|Δs_n| bands) is the discriminator, not the sizing modifier. The current 3WA strategist applies it after selection with no effect on entry. A 5-year walk-forward replay with v3 basin returned +1.31%/yr vs SPY +10.32% — confirming the v3 basin cannot see the Wave 1 signal (which lives in s_n, not the L4 tuple). See §8-9 of the canonical finding doc.
