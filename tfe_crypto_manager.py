...
AWS integration:
  ...
  - Otherwise (default), we pass root_key=None and rely on the existing
    ...
    def from_environment(
        ...
        Root key behavior:
          ...
          - If TFE_ENV=aws (case-insensitive), we:
              ...
              * Fetch a 32-byte root key from AWS Secrets Manager
              * Pass that key into initialize_ses_core_for_env

          - Otherwise, we pass root_key=None, which preserves the
        ...
        if env_flag == "aws":
            # AWS mode: obtain root key from Secrets Manager
            provider = AwsSecretsRootKeyProvider.from_env()
            root_key = provider.get_root_key()
        else:
            # Local/dev mode: keep existing behavior (no explicit root key)
            root_key = None
