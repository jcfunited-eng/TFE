"""Physical Sensory-to-Action Affordance & Digestible Mass Separation release, one image, one definition, one writer cutover.

Includes full release:
- AUT-ORAL-01: Oral contact distinguishes sensory tastants from declared digestible mass; mass is strictly conserved.
- AUT-ORAL-02: Scoped unsuccessful bite suppression survives refused release commands and suppresses repetitive bites on unchanged caregiver offers.
- AUT-ORAL-03: Zero-intake bite sample records episode suppression without marking the whole object currently_depleted or clearing is_food.
- AUT-ORAL-04: Matched-control twin verification demonstrates causal memory-directed pursuit of recognized food.
- Caretaker deadlock elimination: safe walk_to_region object resolution, set_down freed-hand resolution, and fallback floor delivery upon high-chair refusal.
- World authority reciprocal custody reconciliation and safe nocturnal house tidying.
...
BASE = ACCOUNT + ".dkr.ecr." + REGION + ".amazonaws.com/dsf-ai@sha256:abcd284557c37f15e0fe1f1ada43b1078d8a04313d5ffee0e6a752ecd74052a8"
...
def main():
    ...
    if not args.execute:
        return
    folder = ROOT / "backups/runtime" / ("a1-oral-lifecycle-release-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"))
    folder.mkdir()
    journal = folder / "receipt.jsonl"
    emit("release_start", commit=commit)
    context = Path(tempfile.mkdtemp(prefix="a1-oral-lifecycle-build-"))
    ...
    tag = REPOSITORY + ":a1-oral-lifecycle-" + commit[:12]
