"""Stage 4 Turn-Taking Flow release, one image, one definition, one writer cutover.

Includes full release (Stage 4 acoustic pressure clamp during speech + calibrated 250ms quiet-gap
vocal motor turn release + unconsumed target preservation + non-halting acknowledging drive).
...
OLD_TASK = "c548cf988ab14687aa19f7636a1df029"
OLD_DEFINITION = "arn:aws:ecs:us-east-1:418384447921:task-definition/dsf-ai-task:1548"
BASE = ACCOUNT+".dkr.ecr."+REGION+".amazonaws.com/dsf-ai@sha256:55fd046f41d22b26d84fa01fe34ba2fdc7f6f1b01a3ded702d79dbbe56577179"
...
FILES = {
 ...
 "dsf_ai_service/guala_functional_organism.py": "a4bae7f54ee17815fa2c27487d99802ca9c0519e8e946257f841f085093bb358",
...
def main():
    ...
    if not args.execute:
        return
    folder=ROOT/"backups/runtime"/("a1-turntaking-release-"+datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"))
    folder.mkdir()
    journal=folder/"receipt.jsonl"
    emit("release_start",commit=commit)
    context=Path(tempfile.mkdtemp(prefix="a1-turntaking-build-"))
    ...
    tag=REPOSITORY+":a1-turntaking-"+commit[:12]
