...
def main():
    ...
    band_stats = defaultdict(lambda: defaultdict(lambda: [0, 0]))
    ...
    result = {"frame": "herd-conditioned schema memory (peer-cell energy + greed)",
              "tiers": {}, "cycle_harvest": {}, "morphology_harvest": {}}
    for alpha in ("bigram", "bigram_H", "bigram_HG"):
        tier_out = {}
        for tier in TIERS:
    ...
    out = os.path.join(OUT_DIR, "ch4_uf_spectrum_herd.json")
