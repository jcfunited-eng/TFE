"""Bounded repair-only release, one image, one definition, one writer cutover.

The memory/pursuit candidate is deliberately excluded. No old checkpoint restore.
...
OLD_TASK = "1cb0c8e4eeb240a58e756f2a8570d4a4"
OLD_DEFINITION = "arn:aws:ecs:us-east-1:418384447921:task-definition/dsf-ai-task:1532"
BASE = ACCOUNT+".dkr.ecr."+REGION+".amazonaws.com/dsf-ai@sha256:e53aecd21332bb4e9fb5e03799936c9f97f846a3ba457b9c58040587df41e82e"
...
def main():
    ...
    for path in ("tools/guala_repair_actor_proof.py","tools/guala_repair_release_operator.py",__file__):
    ...
    proof=(ROOT/"backups/runtime/a1-repair-only-proof-20260924.log").read_text()
    assert "REPAIR_ONLY_MATURE_CAUSAL_PERSISTENCE_FRESH_PROCESS_PASS" in proof
    ...
    if not args.execute:
        return
    folder=ROOT/"backups/runtime"/("a1-repair-release-"+datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"))
    folder.mkdir()
    journal=folder/"receipt.jsonl"
    emit("release_start",commit=commit)
    context=Path(tempfile.mkdtemp(prefix="a1-repair-build-"))
    ...
    for source,name in (("tools/guala_repair_actor_proof.py","a1_repair_actor_proof.py"),
                        ("tools/guala_repair_release_operator.py","a1_repair_release_operator.py")):
        shutil.copyfile(ROOT/source,context/name)
        dockerfile.append("COPY "+name+" /opt/"+name)
    (context/"Dockerfile").write_text("\n".join(dockerfile)+"\n")
    tag=REPOSITORY+":a1-repair-"+commit[:12]
    ...
    assert any("REPAIR_ONLY_MATURE_CAUSAL_PERSISTENCE_FRESH_PROCESS_PASS" in line for line in messages)
    ...
    try:
        ...
        backups=records(oneoff(definition,s["networkConfiguration"],"backup"),"a1.repair.final_backup.v1")
