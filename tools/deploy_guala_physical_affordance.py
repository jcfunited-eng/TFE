"""Physical Sensory-to-Action Affordance & Invariant Food Acquisition release, one image, one definition, one writer cutover.

Includes full release:
- Excision of administrative name-whitelist gating (_is_food) from candidate generation and decide.
- Pure geometric carrying limits for grasp affordances, eliminating hidden tastant mass omniscience.
- Scoped zero-intake bite suppression to active grasp episode, clearing on release/drop.
- Retained historical intake provenance across retinal updates, distinguishing historical nourishment from current depletion.
- Soft textile compliance for floor bedding and lawful negative-space routing.
...
OLD_TASK = "ec20ff084de54d48afab9a113647fa46"
OLD_DEFINITION = "arn:aws:ecs:us-east-1:418384447921:task-definition/dsf-ai-task:1553"
BASE = ACCOUNT+".dkr.ecr."+REGION+".amazonaws.com/dsf-ai@sha256:1d088eaaf195931a315e45c7ed456d4bb028210655e52eacad27e4445161612d"
REPOSITORY = BASE.split("@")[0]
FILES = {
 "dsf_ai_service/substrate/embodiment_world.py": "fbb498d5ba9bfc8fc29780f06da0b55f54c43fd7844ebfe7bffed01619207183",
 "dsf_ai_service/guala_caretaker_hand.py": "b7c85c7162f860229f261247d274adb7f033b8437ea92ef921804ef987ac46e8",
 "dsf_ai_service/lean_production_app.py": "8131709b6ec6782533f4bd63cbbbd197be75930b4ea3210367686961be8fcef7",
 "dsf_ai_service/guala_functional_organism.py": "6dcad72ebed6511c8a5dcd766bf71346e1d611433d804170764660f7a00fd7c5",
 ...
 "dsf_ai_service/guala_home_world.py": "0821714501db1b9edaf5f7aba0c064fa3c5f8a20423954ffc14c34c49c82d778",
...
def main():
    ...
    if not args.execute:
        return
    folder=ROOT/"backups/runtime"/("a1-affordance-release-"+datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"))
    folder.mkdir()
    journal=folder/"receipt.jsonl"
    emit("release_start",commit=commit)
    context=Path(tempfile.mkdtemp(prefix="a1-affordance-build-"))
    ...
    tag=REPOSITORY+":a1-affordance-"+commit[:12]
