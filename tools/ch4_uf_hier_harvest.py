...
def main():
    ...
    # ---- causal accumulation: HIERARCHICAL SCHEMA MEMORY
    # (UF hierarchical schema/tapestry memory): the prediction comes from
    # the MOST SPECIFIC record that has earned n >= W — (species, E, G)
    # first, then (species, E), then species. Morphology targets always
    # from the stable unconditioned species store. All causal.
    obs.sort(key=lambda x: (x[0], x[5]))
    S0p, S0n = defaultdict(int), defaultdict(int)   # species
    S1p, S1n = defaultdict(int), defaultdict(int)   # species x E
    S2p, S2n = defaultdict(int), defaultdict(int)   # species x E x G
    band_stats = defaultdict(lambda: [0, 0])        # tier -> [hit, n]
    band_year = defaultdict(lambda: defaultdict(lambda: [0, 0]))
    level_used = defaultdict(int)
    cycle_open = {}
    morph_open = {}
    ...
    for d_next, big, eb, gb, disp, sym, issue_d, exit_d, issue_px in obs:
        ...
        if cand is not None:
            ...
            for tier in TIER_LIST:
                if live >= tier:
                    band_stats[tier][0] += 1 if hit else 0
                    band_stats[tier][1] += 1
                    band_year[tier][d_next[:4]][0] += 1 if hit else 0
                    band_year[tier][d_next[:4]][1] += 1
            # harvest ledger: entries at live >= 0.90 (the deepest tier
            # with decade volume in the herd run), morphology exits with
            # unconditioned targets, collapse on any established
            # DOWN-majority at the same backoff level
            mo = morph_open.get(sym)
    ...
    for tier in TIER_LIST:
        h, n = band_stats[tier]
        tier_out[str(tier)] = {"n": n,
                               "hit_pct": round(100 * h / n, 2) if n else None,
                               "by_year": {y: f"{v[0]}/{v[1]}" for y, v in sorted(band_year[tier].items())}}
