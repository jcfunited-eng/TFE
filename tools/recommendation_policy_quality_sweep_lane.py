...
def _run_learning_variant(*, variant: Variant, output_dir: Path) -> Variant:
    env = os.environ.copy()
    env["TFE_POLICY_SOURCE_MODE"] = str(variant.source_mode)
    env["TFE_POLICY_EVAL_MODE"] = str(variant.eval_mode)
    for key, value in variant.env_overrides.items():
        env[key] = str(value)

    log_path = output_dir / f"{variant.name}-learning.log"
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"learning_variant_failed:{variant.name}:rc={proc.returncode}:log={log_path}")
    ...
    if report_path is None or not report_path.exists():
        raise RuntimeError(f"learning_report_not_found:{variant.name}:log={log_path}")
    ...
    if not isinstance(candidate_policy, str) or not candidate_policy.strip():
        raise RuntimeError(f"candidate_policy_missing:{variant.name}:report={report_path}")
    ...
    copied_policy = output_dir / f"{variant.name}-candidate-policy.json"
    shutil.copy2(policy_path, copied_policy)

    variant.policy_path = copied_policy
    variant.learning_report_path = str(report_path)
    return variant
...
def _evaluate_snapshot_variant(
    *,
    ...
    return {
        ...
        "anomaly_fallback_enabled": False,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Expanded recommendation policy-quality sweep lane.")
    ...
    out_dir = (
        ...
        else REPO_ROOT / "backups" / "runtime" / f"recommendation-policy-quality-sweep-lane-{_utc_stamp()}"
    )
    ...
    variants: List[Variant] = [
        Variant(name="runtime_current", source_mode="runtime", eval_mode="runtime", env_overrides={}, policy_path=runtime_policy_path),
        Variant(name="rowtrace_action_return", source_mode="rowtrace", eval_mode="rowtrace", env_overrides={"TFE_POLICY_SELECTION_OBJECTIVE": "action_return"}),
        Variant(name="rowtrace_action_return_spider_off", source_mode="rowtrace", eval_mode="rowtrace", env_overrides={"TFE_POLICY_SELECTION_OBJECTIVE": "action_return", "TFE_POLICY_SPIDER_EYES_MODE": "off"}),
        Variant(name="rowtrace_action_return_stability_on", source_mode="rowtrace", eval_mode="rowtrace", env_overrides={"TFE_POLICY_SELECTION_OBJECTIVE": "action_return", "TFE_POLICY_INCLUDE_STABILITY_BUCKET": "1"}),
        Variant(name="rowtrace_action_return_cd_nonstable", source_mode="rowtrace", eval_mode="rowtrace", env_overrides={"TFE_POLICY_SELECTION_OBJECTIVE": "action_return", "TFE_POLICY_CD_REGIME_MODE": "nonstable_only"}),
        Variant(name="replay_action_return", source_mode="replay", eval_mode="replay", env_overrides={"TFE_POLICY_SELECTION_OBJECTIVE": "action_return"}),
    ]

    learned: List[Variant] = []
    for variant in variants:
        if variant.name == "runtime_current":
            learned.append(variant)
            continue
        learned.append(_run_learning_variant(variant=variant, output_dir=out_dir))

    rows = _load_snapshot_rows(snapshot_path)
    ranked_rows: List[Dict[str, Any]] = []

    for variant in learned:
        assert variant.policy_path is not None
    ...
    summary = {
        ...
        "winner": {**winner, "reason": winner_reason},
