...
def _verify_restored_state(staged_dir: str) -> dict:
    """Integrity verification against the STAGING dir only.

    Transient causal-experience windows have no restorable state.  A legacy
    window manifest or segment directory may remain in an old backup, but it
    is inert archive material and is not parsed into the live substrate.
    """
    report: dict = {}
    # 1. Identity parses.
    ...
    report["transient_causal_experience_state"] = "not_restorable"
    return report
