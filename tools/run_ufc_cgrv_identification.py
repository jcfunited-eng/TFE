...
RCG_SELECTED = BACKUPS / "dsf_ufc_transport_coupled_governed_selected_20260322T063923Z.json"
RCG_AUDIT = BACKUPS / "dsf_ufc_transport_coupled_governed_path_audit_20260322T063923Z.json"
...
class Candidate:
    ...
    @property
    def name(self) -> str:
        return f"cgrv_eta{self.eta:g}_lambda{self.lambda_:g}"
...
def candidate_catalog() -> list[Candidate]:
    return [Candidate(eta=eta, lambda_=lambda_) for eta in ETAS for lambda_ in LAMBDAS]
...
def invariants(point: PrimitivePoint, prev: PrimitivePoint | None) -> dict[str, float]:
    ...
    h = 1.0 - p
    q0 = 1.0 / ((1.0 + point.C_k) * (1.0 + point.P_k))
    if 0.0 <= point.R_rev_k <= 1.0:
        rev = point.R_rev_k
    else:
        rev = point.R_rev_k / (1.0 + point.R_rev_k)
    seed = cov + p - cov * p
    cons = cov * mpos
    danger = max(fail, delta)
    return {
        "s": s,
        "r": r,
        "k": k,
...
def controls(candidate: Candidate, inv: dict[str, float]) -> dict[str, float]:
    veto = (1.0 - inv["rev"]) ** candidate.eta
    ax = inv["q0"] * veto * inv["seed"]
    px = inv["q0"] * veto * inv["cons"] * inv["seed"]
    ay = inv["delta"] * inv["h"]
    ny = inv["danger"] * inv["h"] * (1.0 + inv["mneg"])
    g_couple = candidate.lambda_ * inv["delta"]
    return {"Ax": ax, "Px": px, "Ay": ay, "Ny": ny, "G": g_couple, "veto": veto}
...
def nonnegative_axis_stationary(a_term: float, linear_term: float) -> list[float]:
...
def admissible_minima(ctrl: dict[str, float]) -> list[dict[str, Any]]:
    ...
    x_roots = nonnegative_axis_stationary(ctrl["Ax"], ctrl["Px"])
    y_roots = nonnegative_axis_stationary(ctrl["Ay"], ctrl["Ny"])
    for x in x_roots:
...
def choose_global_minimum(ctrl: dict[str, float]) -> dict[str, Any]:
    ...
    return {"decision": decision, "family": chosen["family"], "chosen_minimum": chosen, "global_minima": minima}
...
def path_rule_evaluation(path_name: str, points: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if path_name == "path_A_coverage_collapse":
        ...
        if families[-1] == "red":
            failures.append("still_termination_red")
        ...
        if any(decision_seq[idx] == "Accumulate" and decision_seq[idx + 1] == "Avoid" for idx in range(len(decision_seq) - 1)):
            failures.append("direct_green_to_red")
        ...
        if decisions[-1] == "Avoid":
            failures.append("reversal_forced_red_without_collapse")
        expected = {"Accumulate", "Hold"}
        monotonicity = monotonicity_score(decisions, "nonincreasing")
    else:
        if decisions[-1] == "Accumulate":
        ...
        if decisions[-1] == "Avoid":
            failures.append("still_reversal_forced_red_without_collapse")
        expected = {"Hold"}
        monotonicity = monotonicity_score(decisions, "nonincreasing")
...
def main() -> int:
    ...
    rcg_selected = load_json(RCG_SELECTED)
    rcg_audit = load_json(RCG_AUDIT)
    ...
    verdict = "CGRV-UFC is viable and safer than baseline" if achieved_six else "Elegant UFC not recovered under CGRV-UFC."

    best_path_failures = {
        path_name: audit["acceptance_failures"]
        for path_name, audit in best["path_audit"].items()
        if audit["acceptance_failures"]
    }
    diagnosis = {
        "start_not_accumulate_disappeared": "start_not_accumulate" not in best["failure_histogram"],
        "path_A_end_red": best["path_audit"]["path_A_coverage_collapse"]["accepted"] or "end_not_avoid" not in best["path_audit"]["path_A_coverage_collapse"]["acceptance_failures"],
        "reversal_suppression_stopped_green": "reversal_failed_to_suppress_accumulate" not in best["failure_histogram"],
        "path_D_not_mainly_avoid": "mild_damage_region_mainly_avoid" not in best["path_audit"]["path_D_one_sided_contested_carry_sweep"]["acceptance_failures"],
    }

    audit_json = {
        "generated_at_utc": utc_iso(),
        "branch_name": "CGRV-UFC",
        "diagnostic_input": {
            "rcg_forced_conclusion": rcg_selected["forced_conclusion"],
            "rcg_best_candidate": rcg_selected["best_candidate"],
            "rcg_best_score": rcg_selected["best_score"],
        },
        ...
        "best_candidate_failure_reasons": best_path_failures,
        "diagnosis": diagnosis,
        "scorecards": scorecards,
    }

    audit_md = (
        "# CGRV-UFC Strict Path Audit\n\n"
        "## Concise Verdict\n"
        f"- `{verdict}`\n\n"
        "## Best Combo\n"
        f"- name: `{best['candidate']['name']}`\n"
        f"- eta: `{best['candidate']['eta']}`\n"
        ...
        + "\n\n## Best Combo Path Failures\n"
        + "\n".join(f"- `{name}` -> `{fails}`" for name, fails in best_path_failures.items())
        + "\n\n## Short Structural Diagnosis\n"
        f"- start_not_accumulate disappeared: `{diagnosis['start_not_accumulate_disappeared']}`\n"
        f"- path_A finally ended red: `{diagnosis['path_A_end_red']}`\n"
        f"- reversal suppression stopped green: `{diagnosis['reversal_suppression_stopped_green']}`\n"
        f"- path_D stayed out of mainly-Avoid: `{diagnosis['path_D_not_mainly_avoid']}`\n"
    )

    audit_json_path = BACKUPS / f"dsf_ufc_cgrv_path_audit_{stamp}.json"
    audit_md_path = BACKUPS / f"dsf_ufc_cgrv_path_audit_{stamp}.md"
    selected_path = BACKUPS / f"dsf_ufc_cgrv_selected_{stamp}.json"
