...
OUT_DIR="$ROOT/backups/runtime/recommendations-consistency-probe-${TS}"
mkdir -p "$OUT_DIR"

USERNAME="codex_reco_cons_${TS//[^0-9]/}"
...
set +e
node "$ROOT/web/scripts/recommendations_consistency_probe.mjs" \
  --baseUrl "$BASE_URL" \
  --username "$USERNAME" \
  --password "$PASSWORD" \
