...
GUALA_OPERATOR_API_ORIGIN="${GUALA_OPERATOR_API_ORIGIN:?GUALA_OPERATOR_API_ORIGIN is required}"
TFE_PUBLIC_BASE_URL="${TFE_PUBLIC_BASE_URL:?TFE_PUBLIC_BASE_URL is required}"
GUALA_OPERATOR_API_SECRET_ID="${GUALA_OPERATOR_API_SECRET_ID:?GUALA_OPERATOR_API_SECRET_ID is required}"
CLERK_SECRET_ID="${CLERK_SECRET_ID:?CLERK_SECRET_ID is required}"
...
tools/tfe_web_operator_task_contract.py
LIST
...
resolve_required_runtime_secret() {
  local secret_id="$1"
  local evidence_name="$2"
  local evidence_path="${EVIDENCE_DIR}/${evidence_name}.json"
  local secret_arn

  if ! aws secretsmanager describe-secret \
    --secret-id "${secret_id}" \
    --region "${REGION}" \
    --output json >"${evidence_path}"; then
    echo "Operator listening deploy contract failed: required secret is absent or unreadable: ${evidence_name}" >&2
    exit 1
  fi
  if jq -e 'has("DeletedDate") or (.ARN | type != "string" or length == 0)' "${evidence_path}" >/dev/null; then
    echo "Operator listening deploy contract failed: required secret is deleted or has no ARN: ${evidence_name}" >&2
    exit 1
  fi
  secret_arn="$(jq -r '.ARN' "${evidence_path}")"
  printf '%s' "${secret_arn}"
}

verify_operator_runtime_secret_authority() {
  local execution_role_arn
  local kms_key_id
  local kms_key_arn
  local kms_index

  GUALA_OPERATOR_API_SECRET_ARN="$(
    resolve_required_runtime_secret \
      "${GUALA_OPERATOR_API_SECRET_ID}" \
      "guala-operator-api-secret"
  )"
  CLERK_SECRET_ARN="$(
    resolve_required_runtime_secret \
      "${CLERK_SECRET_ID}" \
  ...
  execution_role_arn="$(
    jq -r '.taskDefinition.executionRoleArn // empty' \
      "${EVIDENCE_DIR}/taskdef-current.json"
  )"
  if [ -z "${execution_role_arn}" ]; then
    echo "Operator listening deploy contract failed: ECS executionRoleArn is absent." >&2
    exit 1
  fi
  ...
  aws iam simulate-principal-policy \
    ...
    --resource-arns \
      ...
      "${CLERK_SECRET_ARN}" \
    --region "${REGION}" \
    --output json >"${EVIDENCE_DIR}/operator-secret-iam-simulation.json"
  if ! jq -e '
    (.EvaluationResults | length) == 2
    and all(
      .EvaluationResults[];
      .EvalDecision == "allowed"
      and ((.MissingContextValues // []) | length) == 0
    )
  ' "${EVIDENCE_DIR}/operator-secret-iam-simulation.json" >/dev/null; then
    echo "Operator listening deploy contract failed: ECS execution role cannot read both required secrets." >&2
    exit 1
  fi

  kms_index=0
  while IFS= read -r kms_key_id; do
    [ -z "${kms_key_id}" ] && continue
    kms_index=$((kms_index + 1))
    aws kms describe-key \
      --key-id "${kms_key_id}" \
      --region "${REGION}" \
      --output json >"${EVIDENCE_DIR}/operator-secret-kms-${kms_index}.json"
    kms_key_arn="$(
      jq -r '.KeyMetadata.Arn // empty' \
        "${EVIDENCE_DIR}/operator-secret-kms-${kms_index}.json"
    )"
    if [ -z "${kms_key_arn}" ]; then
      echo "Operator listening deploy contract failed: custom secret KMS key has no ARN." >&2
      exit 1
    fi
    aws iam simulate-principal-policy \
      --policy-source-arn "${execution_role_arn}" \
      ...
      --region "${REGION}" \
      --output json >"${EVIDENCE_DIR}/operator-secret-kms-iam-${kms_index}.json"
    if ! jq -e '
      (.EvaluationResults | length) == 1
      and .EvaluationResults[0].EvalDecision == "allowed"
      and ((.EvaluationResults[0].MissingContextValues // []) | length) == 0
    ' "${EVIDENCE_DIR}/operator-secret-kms-iam-${kms_index}.json" >/dev/null; then
      echo "Operator listening deploy contract failed: ECS execution role cannot decrypt a required secret." >&2
      exit 1
    fi
  done < <(
    jq -r '.KmsKeyId // empty' \
      "${EVIDENCE_DIR}/guala-operator-api-secret.json" \
      "${EVIDENCE_DIR}/clerk-secret.json" \
      | sort -u
  )
}
...
jq \
  ...
  "${EVIDENCE_DIR}/taskdef-current.json" >"${EVIDENCE_DIR}/taskdef-register-input-base.json"

python3 "${REPO_ROOT}/tools/tfe_web_operator_task_contract.py" \
  --mode apply \
  --input "${EVIDENCE_DIR}/taskdef-register-input-base.json" \
  --output "${EVIDENCE_DIR}/taskdef-register-input.json" \
  --operator-secret-arn "${GUALA_OPERATOR_API_SECRET_ARN}" \
  --clerk-secret-arn "${CLERK_SECRET_ARN}" \
  --operator-api-origin "${GUALA_OPERATOR_API_ORIGIN}" \
  --public-base-url "${TFE_PUBLIC_BASE_URL}"
...
jq '.taskDefinition' \
  ...
  >"${EVIDENCE_DIR}/operator-task-contract-post.json"
python3 "${REPO_ROOT}/tools/tfe_web_operator_task_contract.py" \
  --mode verify \
  --input "${EVIDENCE_DIR}/operator-task-contract-post.json" \
  --operator-secret-arn "${GUALA_OPERATOR_API_SECRET_ARN}" \
  --clerk-secret-arn "${CLERK_SECRET_ARN}" \
  --operator-api-origin "${GUALA_OPERATOR_API_ORIGIN}" \
  --public-base-url "${TFE_PUBLIC_BASE_URL}"
