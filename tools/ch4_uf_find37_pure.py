"""
ch4_uf_find37_pure.py — pure-structure finder at the coarse scale
=================================================================
...
def main():
    ...
    for i, sym in enumerate(uni):
        ...
        fine_dirs = {m: leg_dirs(closes, m)[0] for m in FINE}
        moves = np.abs(np.diff(closes))
        n = len(closes)
        open_f = None
        for t in range(1, n):
            if open_f is not None:
                ...
                if side == 1 and closes[t] >= open_f["tgt"]:
                    ...
                    finds.append(open_f); open_f = None
            if flips_c[t] != 0 and open_f is None and closes[t] >= PRICE_FLOOR:
                side = int(flips_c[t])
                w0 = max(1, t - 5)
                ...
                agree = int(all(fine_dirs[m][t] == side for m in FINE))
    ...
    result = {
        "frame": "pure-structure coarse-scale finder (rung-16 flips; "
                 "completion inside the same structure)",
        "all": table(closed),
