...
QUALITY_AUDIT_PROBE_PREFIX = "recommendation-quality-audit-probe"
...
def _learning_report_latest_override_path(*, source_mode: str, eval_mode: str, output_dir: Path) -> Path:
    return output_dir / f"l5-policy-learning-latest-{source_mode}-{eval_mode}.json"


def _run_learning_variant(*, source_mode: str, eval_mode: str, output_dir: Path) -> PolicyVariant:
    env = os.environ.copy()
    env["TFE_POLICY_SOURCE_MODE"] = source_mode
    env["TFE_POLICY_EVAL_MODE"] = eval_mode
    env["TFE_L5_REPORT_LATEST_PATH"] = str(
        _learning_report_latest_override_path(
    ...
    log_path = output_dir / f"learning-{source_mode}-{eval_mode}.log"
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"learning_variant_failed:{source_mode}:{eval_mode}:rc={proc.returncode}:log={log_path}")

    report_path: Path | None = None
    for line in (proc.stdout or "").splitlines():
        candidate = line.strip()
        if "l5-policy-learning-report-" in candidate and candidate.endswith(".json"):
            p = Path(candidate)
            report_path = p if p.is_absolute() else (REPO_ROOT / p)
            break

    if report_path is None or not report_path.exists():
        raise RuntimeError(f"learning_report_not_found:{source_mode}:{eval_mode}:log={log_path}")

    report = json.loads(report_path.read_text(encoding="utf-8"))
    candidate_policy = report.get("artifacts", {}).get("candidate_policy_path")
    if not isinstance(candidate_policy, str) or not candidate_policy.strip():
        raise RuntimeError(f"candidate_policy_missing:{source_mode}:{eval_mode}:report={report_path}")

    policy_path = Path(candidate_policy)
    if not policy_path.is_absolute():
        policy_path = REPO_ROOT / policy_path
    if not policy_path.exists():
        raise RuntimeError(f"candidate_policy_not_found:{policy_path}")

    return PolicyVariant(
        name=f"policy_{source_mode}_{eval_mode}",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run bounded recommendation quality audit matrix.")
    ...
    out_dir = (
        ...
        else REPO_ROOT / "backups" / "runtime" / f"{QUALITY_AUDIT_PROBE_PREFIX}-{_utc_stamp()}"
    )
    ...
    learning_variants: List[PolicyVariant] = [
        PolicyVariant(
            name="policy_runtime_current",
    ...
    replay_variant = _run_learning_variant(source_mode="replay", eval_mode="replay", output_dir=out_dir)
    rowtrace_variant = _run_learning_variant(source_mode="rowtrace", eval_mode="rowtrace", output_dir=out_dir)
    learning_variants.extend([replay_variant, rowtrace_variant])
    ...
    ranked_rows.sort(
        key=lambda r: (
            int(r["gates"]["total_gate_count"]),
            int(r["gates"]["quality_gate_count"]),
            float(r["avg_outcome_over_index_pct"]),
            float(r["coverage_rate"]),
            -float(r["fallback_rate"]),
        ),
        reverse=True,
    )
    ...
    if winner is None:
        raise RuntimeError("quality_audit_matrix_empty")

    reason = (
        "Winner has highest total gate count; among ties it has stronger 5/20/60+avg quality profile and better reliability "
        "(higher coverage, lower fallback)."
    )
    ...
    summary = {
        ...
        "inputs": {
            ...
            "notes": {
                "bar_threshold_effect_on_5_20_60": "row-trace dataset has no historical bar_count, so bar-threshold variants are evaluated on live snapshot reliability only",
        ...
        "winner": {
            ...
            "eval_mode": winner["eval_mode"],
            "min_bars": winner["min_bars"],
            "policy_path": winner["policy_path"],
            "gates": winner["gates"],
            "horizon_outcome_over_index_pct": winner["horizon_outcome_over_index_pct"],
            "avg_outcome_over_index_pct": winner["avg_outcome_over_index_pct"],
            "coverage_rate": winner["coverage_rate"],
            "fallback_rate": winner["fallback_rate"],
            "reason": reason,
        },
    ...
    with csv_path.open("w", encoding="utf-8", newline="") as f:
        ...
        for idx, row in enumerate(ranked_rows, start=1):
            g = row["gates"]
            h = row["horizon_outcome_over_index_pct"]
            writer.writerow(
                [
                    idx,
                    row["variant_name"],
                    ...
                    row["min_bars"],
                    g["total_gate_count"],
                    g["quality_gate_count"],
                    g["reliability_gate_count"],
                    g["gate_5day"],
                    g["gate_20day"],
                    g["gate_60day"],
                    g["gate_sp_plus_4_proxy_avg"],
                    g["gate_coverage"],
                    g["gate_fallback"],
                    h["5"],
                    h["20"],
