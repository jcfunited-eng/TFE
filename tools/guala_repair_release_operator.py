...
if mode == "rehearse":
    ...
    subprocess.run([sys.executable, "/opt/a1_repair_actor_proof.py"], check=True, timeout=170)
elif mode == "backup":
    ...
    destination = directory/("a1-repair-"+os.environ["GIT_SHA"]+"-"+str(time.time_ns())+".zip")
    ...
    assert check == pair
    assert store.read_pointer() == pair.pointer
    print(json.dumps({"schema":"a1.repair.final_backup.v1","path":str(destination),
