...
class NativeEvidenceCustodyOwner:
    ...
    def _add_witness_locked(
        ...
        *,
        ...
        if any(
            ...
            raise RuntimeError(
                "native evidence inverse changed canonical receipt custody"
            )
        witnesses[digest] = witness

    def prepare_admission(
        self,
        witness: NativeEvidenceCustodyWitness,
    ) -> PreparedNativeEvidenceAdmission:
        """Issue temporary custody without publishing learned evidence."""
        ...
        with self._lock:
            ...
            state = _NativeEvidenceAdmissionState(phase="prepared")
            prepared = PreparedNativeEvidenceAdmission(
                witness=witness,
                _token=token,
                _state=state,
    ...
    def commit_prepared_admission(
        ...
        """Publish one prepared witness and return its exact event inverse."""
        ...
        with self._lock:
            ...
            if current is not None:
                ...
                def retire_oldest() -> None:
                    nonlocal retired
                    _digest_value, oldest = witnesses.popitem(last=False)
                ...
                for record in witness.receipt_records:
                    ...
                    canonical = mounted if mounted is not None else record
                    records[record.digest] = canonical
                    refcounts[record.digest] = (
                        refcounts.get(record.digest, 0) + 1
                    )
                ...
                canonical_record_tuple = (
                    ...
                    else tuple(canonical_records)
                )
                canonical_witness = NativeEvidenceCustodyWitness(
                    original_profile_binding_sha256=(
                        witness.original_profile_binding_sha256
                    ),
                    ports=witness.ports,
                    receipt_records=canonical_record_tuple,
            ...
            commit_token = sha256_digest(
                receipt_sha256(_canonical({
                ...
                "native evidence admission commit token",
            )
    ...
    def admit(
        ...
        """Publish one witness through the same owner transaction."""
